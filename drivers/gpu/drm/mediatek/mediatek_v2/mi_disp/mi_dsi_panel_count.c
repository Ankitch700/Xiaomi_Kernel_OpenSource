...
static char *hwmon_key_fps[FPS_MAX_NUM] = { "1fps_times",  "10fps_times",
					    "24fps_times", "30fps_times",
					    "40fps_times", "48fps_times",
					    "50fps_times", "60fps_times",
					    "90fps_times", "120fps_times",
					    "144fps_times" };
static const u32 dynamic_fps[FPS_MAX_NUM] = { 1,  10, 24, 30,  40, 48,
					      50, 60, 90, 120, 144 };
...
void dsi_panel_state_count(struct lcm *lcm, int enable)
{
	...
	if (enable) {
		...
		if (time_after64(jiffies_time, timestamp_panelon))
			lcm->mi_count.panel_active +=
				jiffies_time - timestamp_panelon;
		snprintf(ch, sizeof(ch), "%llu",
			 lcm->mi_count.panel_active / HZ);
		update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_ACTIVE,
				       ch);

		memset(ch, 0, sizeof(ch));
		snprintf(ch, sizeof(ch), "%llu",
			 lcm->mi_count.kickoff_count / 60);
		update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_REFRESH,
				       ch);
		...
		update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_BOOTTIME,
				       ch);
		...
		update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_DAYS,
				       ch);
...
void dsi_panel_backlight_count(struct lcm *lcm, u32 bl_lvl)
{
	...
	if (last_bl_level > 0) {
		lcm->mi_count.bl_level_integral +=
			last_bl_level * (bl_level_end - last_bl_start);
		lcm->mi_count.bl_duration += (bl_level_end - last_bl_start);
	}

	/* backlight level 3071 ==> 450 nit */
	if (last_bl_level > (lcm->bl_max_level * 3 / 4)) {
		lcm->mi_count.bl_highlevel_duration +=
			(bl_level_end - last_bl_start);
	} else if (last_bl_level > 0) {
		/* backlight level (0, 3071] */
		lcm->mi_count.bl_lowlevel_duration +=
			(bl_level_end - last_bl_start);
	}
	...
	if (bl_lvl == 0) {
		...
		update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_BL_HIGH,
				       ch);

		memset(ch, 0, sizeof(ch));
		snprintf(ch, sizeof(ch), "%llu",
			 lcm->mi_count.bl_lowlevel_duration / HZ);
		update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_BL_LOW,
				       ch);
	}
	...
	if (i < FPS_MAX_NUM || !fps) {
		if (i != timming_index && timming_index < FPS_MAX_NUM) {
			...
			if (time_after64(jiffies_time, timestamp_fps))
				lcm->mi_count.fps_times[timming_index] +=
					(jiffies_time - timestamp_fps) / HZ;
			snprintf(ch, sizeof(ch), "%llu",
				 lcm->mi_count.fps_times[timming_index]);
			update_hw_monitor_info(HWMON_CONPONENT_NAME,
					       hwmon_key_fps[timming_index],
					       ch);
...
void dsi_panel_count_init(struct lcm *lcm)
{
	...
	memset((void *)lcm->mi_count.fps_times, 0,
	       sizeof(lcm->mi_count.fps_times));
	...
	for (i = 0; i < FPS_MAX_NUM; i++)
		add_hw_monitor_info(HWMON_CONPONENT_NAME, hwmon_key_fps[i],
				    "0");
...
int dsi_panel_disp_count_set(struct lcm *lcm, const char *buf)
{
	...
	result = sscanf(buf,
			...
			"fps144_times=%llu\n"
			"record_end=%llu\n",
			&panel_active, &kickoff_count, &kernel_boottime,
			&kernel_rtctime, &kernel_days, &bl_duration,
			&bl_level_integral, &bl_highlevel_duration,
			&bl_lowlevel_duration, &hbm_duration, &hbm_times,
			&fps_times[FPS_1], &fps_times[FPS_10],
			&fps_times[FPS_24], &fps_times[FPS_30],
			&fps_times[FPS_40], &fps_times[FPS_48],
			&fps_times[FPS_50], &fps_times[FPS_60],
			&fps_times[FPS_90], &fps_times[FPS_120],
			&fps_times[FPS_144], &record_end);
	...
	if (rtctime.tv_sec > kernel_rtctime) {
		if (rtctime.tv_sec - kernel_rtctime > 10 * 365 * DAY_SECS) {
			...
			if (rtctime.tv_sec - kernel_rtctime > DAY_SECS) {
				delta_days = (rtctime.tv_sec - kernel_rtctime) /
					     DAY_SECS;
				lcm->mi_count.bootRTCtime =
					rtctime.tv_sec -
					((rtctime.tv_sec - kernel_rtctime) %
					 DAY_SECS);
			} else {
	...
	if (lcm->mi_count.bl_duration > 0) {
		snprintf(count_str, sizeof(count_str), "%llu",
			 lcm->mi_count.bl_level_integral /
				 lcm->mi_count.bl_duration);
		update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_BL_AVG,
				       count_str);
	}
	...
	snprintf(count_str, sizeof(count_str), "%llu",
		 lcm->mi_count.hbm_duration);
	update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_HBM_DRUATION,
			       count_str);

	memset(count_str, 0, sizeof(count_str));
	lcm->mi_count.hbm_times = hbm_times;
	snprintf(count_str, sizeof(count_str), "%llu", lcm->mi_count.hbm_times);
	update_hw_monitor_info(HWMON_CONPONENT_NAME, HWMON_KEY_HBM_TIMES,
			       count_str);
...
ssize_t dsi_panel_disp_count_get(struct lcm *lcm, char *buf)
{
	...
	ret = scnprintf(
		...
		"fps144_times=%llu\n"
		"record_end=%llu\n",
		lcm->mi_count.panel_active / HZ, lcm->mi_count.kickoff_count,
		lcm->mi_count.boottime + boot_time, lcm->mi_count.bootRTCtime,
		lcm->mi_count.bootdays, lcm->mi_count.bl_duration / HZ,
		lcm->mi_count.bl_level_integral / HZ,
		lcm->mi_count.bl_highlevel_duration / HZ,
		lcm->mi_count.bl_lowlevel_duration / HZ,
		lcm->mi_count.hbm_duration, lcm->mi_count.hbm_times,
		lcm->mi_count.fps_times[FPS_1], lcm->mi_count.fps_times[FPS_10],
		...
		lcm->mi_count.fps_times[FPS_144], record_end);
