	...
	if (lcm_led_i2c_read_write.read_enable) {
		...
		if (panel_ext->funcs->led_i2c_reg_op)
			...
			retval = panel_ext->funcs->led_i2c_reg_op(
				lcm_led_i2c_read_write.buffer, LM36273_REG_READ,
...
int mi_dsi_panel_set_doze_brightness(struct mtk_dsi *dsi, int doze_brightness)
{
	...
	if (dsi->mi_cfg.lhbm_en && doze_brightness) {
		...
		/* N6 code for HQ-339903 by zhangyundan at 2023/10/25 start */
		if (!(panel_ext && panel_ext->funcs &&
		      panel_ext->funcs->set_only_aod_backlight)) {
			pr_info("%s set_only_aod_backlight func not defined");
			ret = 0;
		} else {
			panel_ext->funcs->set_only_aod_backlight(
				dsi->panel, doze_brightness);
		}
		/* N6 code for HQ-339903 by zhangyundan at 2023/10/25 end */
		mutex_unlock(&dsi->dsi_lock);
		return ret;
	} else if (dsi->mi_cfg.aod_en == false && doze_brightness == 0) {
		/* N6 code for HQ-341763 by zhangyundan at 2023/10/31 start */
		if (!(panel_ext && panel_ext->funcs &&
		      panel_ext->funcs->set_only_aod_backlight)) {
			pr_info("%s set_only_aod_backlight func not defined");
			ret = 0;
		...
		/* N6 code for HQ-341763 by zhangyundan at 2023/10/31 end */
		mutex_unlock(&dsi->dsi_lock);
		return ret;
	}
				...
				struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_FOD_CALIBRATION_BRIGHTNESS:
		if (ctl->feature_val == -1) {
			...
			if (panel_ext->funcs->backlight_for_calibration)
				panel_ext->funcs->backlight_for_calibration(
					dsi->panel, ctl->feature_val);
			mi_cfg->in_fod_calibration = false;
		} else {
			/* N6 code for HQ-341519 by zhangyundan at 2023/10/31 start */
			if (ctl->feature_val >= 0 && ctl->feature_val <= 4095) {
				/* N6 code for HQ-341519 by zhangyundan at 2023/10/31 end */
	...
	case DISP_FEATURE_BIST_MODE:
		...
		if (panel_ext->funcs->panel_set_bist_enable) {
			panel_ext->funcs->panel_set_bist_enable(
				dsi->panel, ctl->feature_val);
			mi_cfg->feature_val[DISP_FEATURE_BIST_MODE] =
				ctl->feature_val;
		}
		break;
	case DISP_FEATURE_BIST_MODE_COLOR:
		if (ctl->tx_ptr && panel_ext->funcs->panel_set_bist_color) {
			if (ctl->tx_ptr[0] >= 0 && ctl->tx_ptr[0] <= 255 &&
				...
				panel_ext->funcs->panel_set_bist_color(
					dsi->panel, ctl->tx_ptr);
				DISP_INFO("set bist color to R:%d G:%d B:%d\n",
					  ctl->tx_ptr[0], ctl->tx_ptr[1],
					  ctl->tx_ptr[2]);
			}
		}
		break;
	case DISP_FEATURE_ROUND_MODE:
		...
		if (panel_ext->funcs->panel_set_round_enable) {
			panel_ext->funcs->panel_set_round_enable(
				dsi->panel, ctl->feature_val);
		}
		break;
	...
	case DISP_FEATURE_LOCAL_HBM:
		...
		if (panel_ext->funcs->set_lhbm_fod) {
			...
			DISP_INFO("LHBM value is %d\n", ctl->feature_val);
		}
		break;
	/* N6 code for HQ-308399 by zhangyundan at 2023/7/21 end */
	/* N6 code for HQ-310724 by zhangyundan at 2023/7/25 end */
	...
	case DISP_FEATURE_GIR:
		...
		if (mi_cfg->gir_state == ctl->feature_val) {
			DISP_ERROR("GIR is the same, return\n");
			break;
		}
		if (mi_cfg->feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] !=
		    DOZE_TO_NORMAL) {
			DISP_ERROR("Has enter doze, gir return\n");
			break;
		}
		switch (ctl->feature_val) {
		case GIR_ON:
