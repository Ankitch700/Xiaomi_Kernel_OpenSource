...
static char *hw_item_print(hw_item *item, int ident, int *offset)
{
	...
	if (ident == 0) {
		for (i = 0; i < ident * 4; i++) {
			size += snprintf(print_buf + size, PAGE_SIZE - size,
					 "%s", " ");
		}
	...
	while (item) {
		...
		if (item->child) {
			...
			ident_child = (strlen(item->name) == 0) ? (ident + 1) :
								  (ident + 2);
			*offset = size;
...
int update_hw_monitor_info(char *component_name, char *mon_key, char *mon_value)
{
	...
	component = hw_item_get_child(info_manager->hw_monitor, component_name);
	if (!component) {
...
int register_hw_monitor_info(char *component_name)
{
	...
	hw_item_add_child(info_manager->hw_monitor, component_name, NULL);
	pr_debug("%s: %s\n", __func__, hw_item_dump());
	return 0;
}
...
static ssize_t hw_info_store(struct kobject *kobj, struct kobj_attribute *attr,
...
static int hw_info_decrypt_test(char *buf, size_t len)
{
	...
	int i;
	int ret;
	info_manager->tfm = crypto_alloc_cipher(
		"aes", CRYPTO_ALG_TYPE_BLKCIPHER, CRYPTO_ALG_ASYNC);
	...
	for (i = 0; i < len; i += blocksize)
		crypto_cipher_decrypt_one(info_manager->tfm, &dest[i], &buf[i]);
	pr_debug("%s: %s\n", __func__, dest);
	kfree(dest);
free_cipher:
 ...
 *
 */
static ssize_t hw_info_show(struct kobject *kobj, struct kobj_attribute *attr,
			    char *buf)
{
#if 0
	int ret;
...
static ssize_t hw_mon_store(struct kobject *kobj, struct kobj_attribute *attr,
			    const char *buf, size_t count)
{
	return count;
}

static ssize_t hw_mon_show(struct kobject *kobj, struct kobj_attribute *attr,
			   char *buf)
{
	mutex_lock(&hw_mon_notifier_lock);
	raw_notifier_call_chain(&hw_mon_notifier_list, 0, NULL);
...
int hwconf_init(void)
{
	...
	info_manager->hwconf_check = debugfs_create_file(
		"hwconf_check", 0644, NULL, NULL, &hwconf_check_fops);
...
void hwconf_exit(void)
{
	if (info_manager) {
		hw_item_free(info_manager->hw_monitor);
		if (info_manager->hwconf_kobj) {
			sysfs_remove_group(info_manager->hwconf_kobj,
					   &attr_group);
			kobject_del(info_manager->hwconf_kobj);
		}
