...
static void print_special_ldo(struct device *dev, bool suspend)
{
	struct qcom_spmi_dev *ctx = dev_get_drvdata(dev);
        ...
        if (!strcmp(dev_name(dev), "0-01")) { //PM8550-1
                //L13B DISPLAY VCI 3.0v
                regmap_read(ctx->regmap, 0xcd08, &val);
                pr_info("%s: display vci %s, 0x%x", suspend?"suspend":"resume", (val & 0x80)?"on":"off", val);

        } else if (!strcmp(dev_name(dev), "0-0c")) { //PM8010_M
                //L2M DVDD 1.0v
                regmap_read(ctx->regmap, 0x4108, &val);
                pr_info("%s: rear aon camera %s, 0x%x", suspend?"suspend":"resume", (val & 0x80)?"on":"off", val);
        } else if (!strcmp(dev_name(dev), "0-0d")) { //PM8010_N
                //L1N DVDD 1.1v
                regmap_read(ctx->regmap, 0x4008, &val);
                pr_info("%s: front aon camera %s, 0x%x", suspend?"suspend":"resume", (val & 0x80)?"on":"off", val);
        }
}

static int pmic_spmi_suspend(struct device *dev)
{
	print_special_ldo(dev, true);
