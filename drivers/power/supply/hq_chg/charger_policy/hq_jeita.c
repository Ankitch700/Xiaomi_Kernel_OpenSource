...
struct hq_jeita_info {
	...
	struct step_jeita_cfg jeita_fcc_eu_cfg[STEP_JEITA_TUPLE_NUM];
	struct step_jeita_cfg jeita_fcc_eu_cfg_limit_minutes[STEP_JEITA_TUPLE_NUM];
...
static int handle_jeita(struct hq_jeita_info *chip)
{
	...
	manager = power_supply_get_drvdata(chip->batt_psy);
	if (IS_ERR_OR_NULL(manager)) {
		pr_err("manager is_err_or_null\n");
		return 0;
	}
	hq_err("charger_region:%d,manager->normal_pps_adapter_temp_status:%d\n",manager->charger_region,manager->normal_pps_adapter_temp_status);
	#if IS_ENABLED(CONFIG_PD_BATTERY_SECRET)
	...
	if(chip->pd_verifed) {
		...
		if(!manager->charge_limit) {
			hq_info("EU country start 10A at 15~48 temp\n");
			vote(chip->total_fcc_votable, JEITA_VOTER, true, ((chip->jeita_fcc_eu_cfg[chip->jeita_index].value) + curr_offset));
		}
		else {
			hq_info("EU country After 3min limit 6.6A at 15~48 temp\n");
			vote(chip->total_fcc_votable, JEITA_VOTER, true, ((chip->jeita_fcc_eu_cfg_limit_minutes[chip->jeita_index].value) + curr_offset));
		}

		if (manager->normal_pps_adapter_temp_status == Warning) {
			hq_info("EU country set 15~48 temp,adapter temp warning 10*2/3 A\n");
			vote(chip->total_fcc_votable, NORMAL_PPS_VOTER, true, chip->jeita_fcc_eu_cfg_limit_minutes[4].value);
		} else if (manager->normal_pps_adapter_temp_status == Over_temperature) {
			hq_info("EU country set 15~48 temp,adapter temp Over_temperature 1 A\n");
			vote(chip->total_fcc_votable, NORMAL_PPS_VOTER, true, 1000);
		}
#else
		if (manager->charger_region)
		{
			if(!manager->charge_limit) {
				hq_info("9 country start 10A at 15~48 temp\n");
				vote(chip->total_fcc_votable, JEITA_VOTER, true, ((chip->jeita_fcc_eu_cfg[chip->jeita_index].value) + curr_offset));
			}
			else {
				hq_info("9 country After 3min limit 6.6A at 15~48 temp\n");
				vote(chip->total_fcc_votable, JEITA_VOTER, true, ((chip->jeita_fcc_eu_cfg_limit_minutes[chip->jeita_index].value) + curr_offset));
			}

			if (manager->normal_pps_adapter_temp_status == Warning) {
				hq_info("9 country set 15~48 temp,adapter temp warning 10*2/3 A\n");
				vote(chip->total_fcc_votable, NORMAL_PPS_VOTER, true, chip->jeita_fcc_eu_cfg_limit_minutes[4].value);
			} else if (manager->normal_pps_adapter_temp_status == Over_temperature) {
				hq_info("9 country set 15~48 temp,adapter temp Over_temperature 1 A\n");
				vote(chip->total_fcc_votable, NORMAL_PPS_VOTER, true, 1000);
			}
		}
		else {
			hq_info("not EU country and 9 country\n");
			vote(chip->total_fcc_votable, JEITA_VOTER, true, ((chip->jeita_fcc_cfg[chip->jeita_index].value) + curr_offset));
		}
#endif
	}
	...
	if (chip->pd_verifed && (g_policy->sm == PM_STATE_CHARGERPUMP_CC_CV || g_policy->cp_charge_done)) {
		...
		if (vol_now >= HEAVY_LOAD_VOLTAGE && curr_now <= CURRENT_NOW_1A) {
			ret = power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_CURRENT_NOW, &pval);
			if (ret < 0) {
				hq_err("Couldn't read batt current_now, ret=%d\n", ret);
			}
			curr_now = pval.intval / 1000;
			hq_info("read batt current_now, ret=%d\n", curr_now);
			chip->fv = (chip->fv - TERM_DELTA_CV * 2);
		}
	} else {
		fuel_gauge_set_fastcharge_mode(chip->fuel_gauge, false);
	}
	...
	hq_info("pd_verifed = %d, cp_charge_done = %d, jeita_index = %d, curr_offset = %d, is_charge_done = %d charge_limit = %d \n",
				chip->pd_verifed, g_policy->cp_charge_done, chip->jeita_index, curr_offset, chip->is_charge_done,manager->charge_limit);
	hq_info("step_chg_fcc = %d, jeita_fcc = %d, jeita_fv = %d, jeita_iterm = %d  jeita_fcc_eu_cfg = %d jeita_fcc_eu_cfg_limit_minutes = %d \n",
				chip->step_chg_cfg[i].value, ((chip->jeita_fcc_cfg[chip->jeita_index].value) + curr_offset), chip->fv, chip->iterm_curr,((chip->jeita_fcc_eu_cfg[chip->jeita_index].value) + curr_offset),((chip->jeita_fcc_eu_cfg_limit_minutes[chip->jeita_index].value) + curr_offset));
	return 0;
}
...
void normal_adapter_detect_plugin_lower_fcc_work(struct work_struct *work)
{
	struct hq_jeita_info *chip = the_chip;
	struct charger_manager *manager = container_of(work, struct charger_manager, normal_adapter_detect_plugin_lower_fcc_work.work);
	hq_info("normal pps adapter plug in enter\n");
	chip->pd_adapter = get_adapter_by_name("pd_adapter");
	if (!chip->pd_adapter)
		hq_err("failed to pd_adapter\n");
	else
	...
	if (chip->jeita_index == INDEX_15_to_35 || chip->jeita_index == INDEX_35_to_48)
	{
		if ((manager->pd_active == CHARGE_PD_PPS_ACTIVE) && !chip->pd_verifed)
		{
			hq_info("plug in wait 3min lower fcc 10*2/3 A\n");
			vote(chip->total_fcc_votable, JEITA_VOTER, true, chip->jeita_fcc_eu_cfg_limit_minutes[chip->jeita_index].value);
			manager->charge_limit = true;
		}
...
static bool hq_parse_jeita_dt(struct device_node *node, struct hq_jeita_info *chip)
{
	...
	total_length = of_property_count_elems_of_size(node, "jeita_fcc_eu_cfg", sizeof(u32));
	if (total_length < 0) {
		hq_err("failed to read total_length of jeita_fcc_eu_cfg\n");
		return 0;
	}

	ret |= of_property_read_u32_array(node, "jeita_fcc_eu_cfg", (u32 *)chip->jeita_fcc_eu_cfg, total_length);
	if (ret) {
		hq_err("failed to parse jeita_fcc_eu_cfg\n");
		return 0;
	}

	total_length = of_property_count_elems_of_size(node, "jeita_fcc_eu_cfg_limit_minutes", sizeof(u32));
	if (total_length < 0) {
		hq_err("failed to read total_length of jeita_fcc_eu_cfg_limit_minutes\n");
		return 0;
	}

	ret |= of_property_read_u32_array(node, "jeita_fcc_eu_cfg_limit_minutes", (u32 *)chip->jeita_fcc_eu_cfg_limit_minutes, total_length);
	if (ret) {
		hq_err("failed to parse jeita_fcc_eu_cfg_limit_minutes\n");
		return 0;
	}
	...
	for (i = 0; i < STEP_JEITA_TUPLE_NUM; i++)
		hq_info("[jeita_fcc_cfg]%d %d %d [jeita_fv_cfg]%d %d %d [jeita_fcc_eu_cfg]%d %d %d [jeita_fcc_eu_cfg_limit_minutes]%d %d %d \n",
					...
					chip->jeita_fcc_eu_cfg[i].low_threshold, chip->jeita_fcc_eu_cfg[i].high_threshold, chip->jeita_fcc_eu_cfg[i].value,
					chip->jeita_fcc_eu_cfg_limit_minutes[i].low_threshold, chip->jeita_fcc_eu_cfg_limit_minutes[i].high_threshold, chip->jeita_fcc_eu_cfg_limit_minutes[i].value);

	ret |= of_property_read_u32(node, "iterm", &chip->iterm);
	ret |= of_property_read_u32(node, "iterm_ffc_15_to_35", &chip->iterm_ffc_15_to_35);
