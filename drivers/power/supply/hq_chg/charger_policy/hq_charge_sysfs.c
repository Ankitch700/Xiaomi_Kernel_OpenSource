...
#endif
/* N6R code for HQ-430292 at 2024/11/17 by p-mazhuang3 start */
				if(manager->input_suspend)
					val->intval = POWER_SUPPLY_STATUS_UNKNOWN;
				else
					val->intval = status;
/* N6R code for HQ-430292 at 2024/11/17 by p-mazhuang3 end */
		break;
...
static void manual_set_cc_toggle(struct charger_manager *manager, bool en)
{
	...
	if(en && !manager->cid_status)
	{
		...
		hq_info("ui set cc toggle : start hrtimer\n");
	}else{
		ret = alarm_try_to_cancel(&manager->rust_det_work_timer);
		if (ret < 0) {
			hq_err("callback was running, skip timer\n");
...
static int get_apdo_max(struct charger_manager *manager) {
	...
	ret = adapter_get_cap(adapter, &g_policy->cap);
		if (ret < 0) {
			hq_info("adapter get cap failed\n");
			goto done;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (manager->apdo_max >= INPUT_POWER_OVER_66W)
...
static ssize_t charger_region_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct power_supply *batt_psy = NULL;
	struct charger_manager *manager = NULL;
...
static ssize_t charger_region_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct charger_manager *manager = NULL;
	struct power_supply *batt_psy = NULL;
	...
	manager->charger_region = val;
	hq_info("manager->charger_region = %d\n", manager->charger_region);
	return count;
}

static struct device_attribute charger_region_attr =
	__ATTR(charger_region, 0644, charger_region_show, charger_region_store);
