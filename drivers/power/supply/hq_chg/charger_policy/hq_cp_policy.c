...
static int switch2_1_fcc_set = 6000;
static int switch1_1_fcc_set = 4000;
#endif
...
static void pdm_multi_mode_switch(struct chargerpump_policy *policy)
{
   ...
   if (policy == NULL)
	...
	hq_info("N6:ibat_request = %d, 2_1 mode = %d, 1_1 mode = %d, soc = %d\n",
...
static bool update_smart_chg_cp_workmode(struct chargerpump_policy *policy)
{
		...
		if (manager != NULL){
			...
			if(manager->pps_fast_mode){
				if (policy->switch2_1_enable && (!((policy->m_cp_mode == 0) && (policy->s_cp_mode == 0)))) {
					...
					hq_info("N6:low_fast working, set switch2_1_enable\n");
				}
				hq_info("N6:low_fast working, cp_mode not switch\n");
				return false;
			}
		}
		else
	...
	if (policy->switch2_1_enable && (!((policy->m_cp_mode == 0) && (policy->s_cp_mode == 0)))) {
		...
		chargerpump_set_cp_workmode(policy->master_cp, 0);
		chargerpump_set_cp_workmode(policy->slave_cp, 0);
		hq_info("N6:set switch2_1_enable\n");
		return true;
		...
		hq_info("N6:set switch1_1_enable\n");
		return true;
		...
		msleep(1000);
		chargerpump_set_cp_workmode(policy->master_cp, 0);
		hq_info("N6:set switch2_1_single_enable\n");
		return true;
		...
		msleep(1000);
		chargerpump_set_cp_workmode(policy->master_cp, 1);
		hq_info("N6:set switch1_1_single_enable\n");
		return true;
	}
	return false;
}
...
static int monitor_normal_pps_adapter_status(struct chargerpump_policy *policy)
{
	struct power_supply *psy = NULL;
	struct chg_info *info = &policy->info;
	struct charger_manager *manager;
	...
	if (psy != NULL) {
		...
		if (manager != NULL){
			if (manager->pd_active == CHARGE_PD_PPS_ACTIVE)
				manager->normal_pps_adapter_temp_status = info->tadapter;
		}
		else
			return false;
	}
		...
		return false;
	hq_info("monitor_normal_pps_adapter_status:%d\n",manager->normal_pps_adapter_temp_status);
	return 0;
}
...
static int __chargerpump_cc_cv_algo(struct chargerpump_policy *policy)
{
	...
	if ((info->tbat >= 151 && info->tbat <= 480) && (info->ffc_cv >= 4490))
		final_cv = info->ffc_cv - TERM_DELTA_CV - CHARGERPUMP_POLICY_FFC_CV_OFFSET;
	else
...
static int dv_policy_state_chargerpump_exit(struct chargerpump_policy *policy)
{
	...
	if (policy->request_volt != CHARGERPUMP_POLICY_REQUEST_SAFE5V) {
		policy->request_volt = CHARGERPUMP_POLICY_REQUEST_SAFE5V;
	}
	...
	if (info->m_cp_chging || (cfg->s_cp_enable && info->s_cp_chging)) {
		...
		adapter_set_cap(policy->adapter, policy->cap_nr, policy->request_volt, 3000);
		goto out;
	}
...
static int chargerpump_policy_thread_fn(void *data)
{
	...
	while (true) {
		...
		monitor_normal_pps_adapter_status(policy);
#if IS_ENABLED(CONFIG_XIAOMI_SMART_CHG)
		if (info->total_fcc > 0) {
			...
			if (need_switch) {
				policy->sm = PM_STATE_INIT;
				policy->initial_vbus_flag = false;
				hq_info("N6:need switch cp mode\n");
			}
		} else
			policy->initial_vbus_flag = false;
#endif
...
int chargerpump_policy_stop(struct chargerpump_policy *policy)
{
	...
	if (cpu_latency_qos_request_active(&cp_qos_request)) {
		hq_info("qos_remove_request\n");
