...
int handle_smart_chg_functype_status(struct charger_manager *manager)
{
	...
	for(i = SMART_CHG_FEATURE_MIN_NUM; i <= SMART_CHG_FEATURE_MAX_NUM; i++){  //handle bit1 ~ bit SMART_CHG_FEATURE_MAX_NUM
		...
		dev_err(manager->dev, "type:%d, en_ret=%d, active_status=%d,func_val=%d, all_func_status=%#X\n",
			i, manager->smart_charge[i].en_ret, manager->smart_charge[i].active_status, manager->smart_charge[i].func_val,all_func_status);
	}

	dev_err(manager->dev, "all_func_status:%#X\n", all_func_status);
...
void monitor_smart_chg(struct charger_manager *manager)
{
	...
	dev_err(manager->dev, "N6 monitor_smart_chg, en_ret = %d, fun_val = %d, active_status = %d, smart_soc = %d, ui_soc =%d, smart_ctrl_en = %d, state = %d\n",
	...
	if(manager->smart_charge[SMART_CHG_NAVIGATION].en_ret && manager->soc >= manager->smart_charge[SMART_CHG_NAVIGATION].func_val && !manager->smart_charge[SMART_CHG_NAVIGATION].active_status)
	{
		...
		if (g_policy->state == POLICY_RUNNING){
			...
			dev_err(manager->dev, "N6 monitor disable cp\n");
		}
		manager->smart_ctrl_en = true;
		dev_err(manager->dev, "N6 monitor disable charger,soc >= %d\n", manager->smart_charge[SMART_CHG_NAVIGATION].func_val);
	}
...
void monitor_night_charging(struct charger_manager *manager)
{
		...
		if (g_policy->state == POLICY_RUNNING){
			chargerpump_policy_stop(g_policy);
			dev_err(manager->dev, "%s N6 monitor disable cp\n", __func__);
		}
		...
		if ((manager->pd_active == CHARGE_PD_PPS_ACTIVE) && (g_policy->state == POLICY_NO_START)){
			chargerpump_policy_start(g_policy);
			dev_err(manager->dev, "%s N6 monitor enable cp\n", __func__);
		}
	}
}
		...
		if (manager->system_temp_level > manager->pd_thermal_levels) {
			...
			goto err;
                ...
                if(fast_flag) {  //stay fast strategy
                        ...
                        if((manager->soc > 38) && (manager->thermal_board_temp > 395)){
                        ...
                        if((manager->soc > 38) && (manager->thermal_board_temp > 385)){
                                if(manager->low_fast_ffc >= 2950){
                                        manager->low_fast_ffc -= 800;
                                }
                                else{
                                        manager->low_fast_ffc = 2150;
                                }
                        ...
                        else if((manager->soc > 35) && (manager->thermal_board_temp > 409)){
                                manager->low_fast_ffc = 2150;
                                pr_err("%s stay fast but cool down, manager->thermal_board_temp = %d, manager->low_fast_ffc = %d\n", __func__, manager->thermal_board_temp, manager->low_fast_ffc);
                        }
                                ...
                                if(manager->low_fast_ffc >= 5150){
                                        manager->low_fast_ffc -= 3000;
                                }
                                else{
                                        manager->low_fast_ffc = 2150;
                                }
                                pr_err("%s stay fast but decrease 3, manager->thermal_board_temp = %d, manager->low_fast_ffc = %d\n", __func__, manager->thermal_board_temp, manager->low_fast_ffc);
                        }
                        else if((manager->soc > 30) && (manager->soc < 39) && (manager->thermal_board_temp < 399)){
                                if(manager->low_fast_ffc <= 11200){
                                        manager->low_fast_ffc += 1000;
                                }
                                else{
                                        manager->low_fast_ffc = 12200;
                                }
                                pr_err("%s stay fast but add 1, manager->thermal_board_temp = %d, manager->low_fast_ffc = %d\n", __func__, manager->thermal_board_temp, manager->low_fast_ffc);
                        }
                        else if((manager->soc <= 30) && (manager->thermal_board_temp < 366)){
                                if(manager->low_fast_ffc <= 11000){
                                        manager->low_fast_ffc += 1200;
                                }
                                else{
                                        manager->low_fast_ffc = 12200;
                                }
                                pr_err("%s stay fast but add 1.2, manager->thermal_board_temp = %d, manager->low_fast_ffc = %d\n", __func__, manager->thermal_board_temp, manager->low_fast_ffc);
                        }
...
void xm_charge_work(struct work_struct *work)
{
	...
	dev_err(chip->dev, "N6:check xm_charge_work\n");
