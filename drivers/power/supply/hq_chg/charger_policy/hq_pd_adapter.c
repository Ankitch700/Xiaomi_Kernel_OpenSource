...
int pd_get_temp(struct adapter_dev *dev,
	uint8_t *temp)
{
	struct pd_adapter_info *info;
	int ret;
	...
	if(ret < 0)
		return ret;
	*temp = (pps_status.real_time_flags & 0x6) >> 1;//bit 1~2
	hq_err("adapter temp status :%d,real_time_flags:%x\n",*temp,pps_status.real_time_flags);
