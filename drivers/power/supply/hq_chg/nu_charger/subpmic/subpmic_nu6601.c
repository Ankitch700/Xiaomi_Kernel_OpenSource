...
static int _nu6601_get_vbus_type(struct nu6601_charger *chg)
{
	...
	int type;
	uint8_t val = 0;
	unsigned int boot_mode = chg->bootmode;
	...
	if ((type == BC12_TYPE_DCP) && (chg->state.vbus_type == BC12_TYPE_NONE)){
		/**delay report DCP for NU66xx, because MI PD adapter would happen hardreset**/
		if (boot_mode == 8 || boot_mode == 9)
			pr_info("%s: bc1.2 DCP delay 0!!\n", __func__);
		else {
			pr_info("%s: bc1.2 DCP delay 500!!\n", __func__);
			mdelay(500);
		}
	}
	pr_err("%s: bc1.2= %x\n", __func__, type);
            ...
            enum sc_adc_channel channel, uint32_t *value)
{
    ...
    case ADC_GET_VSYS:
        ...
        break;
    case ADC_GET_VBAT:
		ret = nu6601_get_adc(chg, ADC_VBAT) / 1000;
        break;
    case ADC_GET_VAC:
		...
		struct nu6601_charger *chg)
{
	...
	np = of_parse_phandle(dev->of_node, "bootmode", 0);
  	if (!np)
  		...
  		if (!tag)
 			...
 			chg->bootmode = tag->bootmode;
 			chg->boottype = tag->boottype;
 		}
  	}
