...
/* N6R code for HQ-425190 at 2024/10/25 by p-mazhuang3 start */
    [F_CID_EN]          = REG_FIELD(SUBPMIC_REG_HK_CTRL + 1, 3, 3),
/* N6R code for HQ-425190 at 2024/10/25 by p-mazhuang3 end */
...
{
/* N6R code for HQ-429228 at 2024/11/03 by p-mazhuang3 start */
    int ret = 0;
    dev_info(sc->dev, "%s ma =%d input_suspend = %d", __func__, ma,sc->sc_charger->input_suspend);
    if (!sc->sc_charger->input_suspend) {
        if (ma < 100) {
            ret = subpmic_chg_field_write(sc, F_PERFORMANCE_EN, true);
        } else {
...
/* N6R code for HQ-429228 at 2024/11/03 by p-mazhuang3 end */   
...
static int subpmic_chg_hw_init(struct subpmic_chg_device *sc)
{
    ...
    if (sc->tcpc) {
        tcpci_set_continuous_time(sc->tcpc);
    }
...
/* N6R code for HQ-425190 at 2024/10/25 by p-mazhuang3 end */
    ...
    ret |= subpmic_chg_write_byte(sc, 0x75, 0x9); // EU certification,add delay to meet transmitter eye diagram
