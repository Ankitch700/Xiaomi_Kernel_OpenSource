...
static int bq_battery_soc_smooth_tracking_new(struct bq_fg_chip *bq, int raw_soc, int batt_soc, int batt_ma)
{
	...
	ktime_t tmp_time = 0;
	/* N6 code for HQ-346740 by p-wuwencheng at 2023/12/22 -start */
	static ktime_t start_time = 0; // record start time
	static bool keep_100; // If true, keep uisoc at 100
	static int cnt; // use for count
	static int tmp_cnt; // Used for adding and subtracting uisoc count
	unsigned long long duration; // Record interval time
	/* N6 code for HQ-346740 by p-wuwencheng at 2023/12/22 -end */
#if IS_ENABLED(CONFIG_XIAOMI_SMART_CHG)
	...
	if (!raw_soc) {
        ...
        if (!is_full_votable) {
                ...
                if(charging_status == POWER_SUPPLY_STATUS_FULL){
                        fg_info("%s smooth_new: report full, is_full_flag = %d\n", __func__, is_full_flag);
                        is_full_flag = true;
                        vote(is_full_votable, SMOOTH_NEW_VOTER, true, 0);
                }

                if(is_full_flag && (raw_soc >= 9700)){
                        fg_info("%s smooth_new: report full, old raw_soc = %d\n", __func__, raw_soc);
                        raw_soc = 100000;
                        fg_info("%s smooth_new: report full, new raw_soc = %d\n", __func__, raw_soc);
                } else {
                        is_full_flag = false;
                        vote(is_full_votable, SMOOTH_NEW_VOTER, false, 0);
                }
                fg_info("%s smooth_new:is_full_flag = %d\n", __func__, is_full_flag);
        }
#endif
	...
	/* N6 code for HQ-346740 by p-wuwencheng at 2023/12/22 -start */
	/* If the last report 100 by keep_100 flag hold, use this logic for smoothing after unplugging */
	if ((charging_status == POWER_SUPPLY_STATUS_DISCHARGING || charging_status == POWER_SUPPLY_STATUS_CHARGING || charging_status == POWER_SUPPLY_STATUS_NOT_CHARGING) && keep_100) {
		fg_info("%s  Repair uisoc DISCHARGING: system_soc = %d last_system_soc =%d \n", __func__, system_soc,last_system_soc);
		if ( system_soc == (100 - tmp_cnt)) {
			keep_100 = false;
		}
		/* tmp_cnt add 1 after every 15 cycles */
		if (system_soc < (100 - tmp_cnt) && cnt++ < 15) {
			system_soc = 100 - tmp_cnt;
			fg_info("%s Repair uisoc DISCHARGING: system_soc = %d  cnt =%d \n", __func__, system_soc,cnt);
		} else {
			tmp_cnt++;
			cnt = 0;
		}
	...
	/*Repair that the UISOC after cut-off under low-temperature conditions is not 100  */
	if (charging_status == POWER_SUPPLY_STATUS_FULL && bq->tbat < 0 && system_soc < 100) {
		if (!keep_100) {
			if (start_time == 0) {
				start_time = ktime_get();
				fg_info("%s Repair uisoc: start_time = %lld \n", __func__, start_time);
			}

			system_soc+=tmp_cnt;
			tmp_time = ktime_sub(ktime_get(), start_time);
			duration = (unsigned long long)ktime_to_ms(tmp_time);
			fg_info("%s Repair uisoc: duration = %d \n", __func__, duration);
			if (duration > 60000 && system_soc < 100) {
				tmp_cnt++;
				start_time = 0;
				system_soc += tmp_cnt;
				fg_info("%s Repair uisoc: duration system_soc = %d last_system_soc =%d \n", __func__, system_soc,last_system_soc);
				if (system_soc > 99) {
					keep_100 = true;
					tmp_cnt = 0;
					fg_info("%s Repair uisoc: keep_100 = %d tmp_cnt =%d \n", __func__, keep_100,tmp_cnt);
				}
			}
		}
		if (keep_100)
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	bq->i2c_err_flag = fg_read_volt(bq);
	fg_read_status(bq);
...
static void low_vbat_power_off(struct bq_fg_chip *bq)
{
	...
	if (rc < 0)
		...
		volt = pval.intval / 1000;

	if ((volt < (SHUTDOWN_DELAY_VOL_LOW - 50)) && volt != 0) {
		if (count < 5)
			count ++;
		else {
			if (bq->charger != NULL) {
				rc = charger_reset(bq->charger);
				if (rc < 0)
