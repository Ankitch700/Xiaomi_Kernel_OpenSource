...
/* N6 code for HQ-304403 by zhangyundan at 2023/07/10 start */
#include <linux/string.h>
/* N6 code for HQ-304403 by zhangyundan at 2023/07/10 end */
...
/* N6 code for HQ-304097 by zhangyundan at 2023/07/26 start */
#if IS_ENABLED(CONFIG_MI_DISP_NOTIFIER)
#include "../../../gpu/drm/mediatek/mediatek_v2/mi_disp/mi_disp_notifier.h"
#endif
/* N6 code for HQ-304097 by zhangyundan at 2023/07/26 end */

/*N6 code for HQ-302671 by dingying at 2023/08/08 start*/
static struct xiaomi_touch_interface xiaomi_touch_interfaces;
#include "focaltech_mi_custom.c"
/*N6 code for HQ-302671 by dingying at 2023/08/08 end*/
...
/* N6 code for tp_info by zhangyundan at 2023/07/10 start */
extern int32_t fts_extra_proc_init(void);
extern void fts_extra_proc_deinit(void);
/* N6 code for tp_info by zhangyundan at 2023/07/10 end */
/*****************************************************************************
* Static function prototypes
*****************************************************************************/
static int fts_ts_suspend(struct device *dev);
static int fts_ts_resume(struct device *dev);
/* N6 code for HQ-304378 by zhangyundan at 2023/07/12 start */
int fts_gestures_status = 0;
/* N6 code for HQ-304378 by zhangyundan at 2023/07/12 end */

/*N6 code for HQ-304124  by dingying at 2023/07/26 start*/
...
/*N6 code for HQ-304124  by dingying at 2023/07/26 end*/
/*N6 code for HQ-302671 by dingying at 2023/08/08 start*/
...
{
    FTS_DEBUG("tp reset");
    /* N6 code for HQ-301696 by liaoxianguo at 2023/8/25 start */
    fts_write_reg(FTS_REG_IDE_PARA_STATUS, FTS_REG_IDE_PARA_STATUS_EN);
    msleep(20);
    /* N6 code for HQ-301696 by liaoxianguo at 2023/8/25 end */
...
/* N6 code for HQ-310178 by zhangyundan at 2023/08/02 start */
    if (fts_data->fod_status != -1) {
        FTS_INFO("fod_status = %d\n", fts_data->fod_status);
        fts_fod_recovery();
    }
/* N6 code for HQ-310178 by zhangyundan at 2023/08/02 end */
    return 0;
}
...
static int fts_read_and_report_foddata(struct fts_ts_data *data)
{
    ...
    case 0x25:
        ...
        break;
    case 0x26:
        /* N6 code for HQ-334349 by zhangyundan at 2023/11/02 start */
        /*N6 code for HQ-357548 by liaoxianguo at 2023/12/12 start*/
        x = ((buf[4] << 8) | buf[5]);
        y = ((buf[6] << 8) | buf[7]);
        /*N6 code for HQ-357548 by liaoxianguo at 2023/12/12 end*/
        /* N6 code for HQ-334349 by zhangyundan at 2023/11/02 end */
        z = buf[3];
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    case TOUCH_PROTOCOL_v2:
        ...
        for (i = 0; i < event_num; i++) {
            ...
            /*N6 code for HQ-303888 by dingying at 2023/08/02 start*/
            /*N6 code for HQ-357548 by liaoxianguo at 2023/12/12 start*/
            events[i].x = events[i].x * 16  / FTS_HI_RES_X_MAX;
            events[i].y = events[i].y * 16 / FTS_HI_RES_X_MAX;
            /*N6 code for HQ-357548 by liaoxianguo at 2023/12/12 end*/
            /*N6 code for HQ-303888 by dingying at 2023/08/02 end*/
            events[i].area = touch_buf[FTS_TOUCH_OFF_AREA + base];
...
#endif
/* N6 code for HQ-304097 by zhangyundan at 2023/7/26 start */
    input_set_capability(input_dev, EV_KEY, KEY_WAKEUP);
    input_set_capability(input_dev, EV_KEY, KEY_GOTO);
/* N6 code for HQ-304097 by zhangyundan at 2023/7/26 end */
/* N6 code for HQ-310178 by zhangyundan at 2023/08/02 start */
...
*/
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            ...
            /* N6 code for HQ-301696 by liaoxianguo at 2023/8/25 start */
            FTS_DEBUG("ts_data->pdata->avdd_gpio=%d", ts_data->pdata->avdd_gpio);
            gpio_direction_output(ts_data->pdata->avdd_gpio, 1);
            FTS_DEBUG("ts_data->pdata->iovdd_gpio=%d", ts_data->pdata->iovdd_gpio);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    FTS_INFO("max touch number:%d, irq gpio:%d, reset gpio:%d, avdd gpio:%d, iovdd gpio:%d",
             pdata->max_touch_number, pdata->irq_gpio, pdata->reset_gpio, pdata->avdd_gpio, pdata->iovdd_gpio);
    /*N6 code for HQ-302671 by dingying at 2023/08/08 start*/
    ...
    if ((ts_data->fod_status != -1 && ts_data->fod_status != 100)) {
        if (ts_data->double_click == false) {
            if (ts_data->fod_status == 0) {
                FTS_INFO("fod_status:%d,clear CF reg",ts_data->fod_status);
                ret = fts_fod_reg_write(FTS_REG_GESTURE_FOD_ON, false);
                if (ret < 0)
                    FTS_ERROR("%s fts_fod_reg_write failed\n", __func__);
            } else {
            ...
            if (ts_data->fod_status == 0) {
                ...
                ret = fts_fod_reg_write(FTS_REG_GESTURE_ALL_ON, true);
                if (ret < 0)
                    FTS_ERROR("%s fts_fall_reg_write failed\n", __func__);
            }
            fts_gesture_reg_write(FTS_REG_GESTURE_DOUBLETAP_ON, true);
            if (ret < 0)
                FTS_ERROR("%s fts_all_reg_write failed\n", __func__);
        }
    }
/* N6 code for HQ-347795 by liaoxianguo at 2023/11/21 end */
...
/* N6 code for HQ-335337 by liaoxianguo at 2023/10/29 start */
/* N6 code for HQ-347795 by liaoxianguo at 2023/11/21 start */
    if (ts_data->fod_status != 0 && ts_data->fod_status != -1) {
        ts_data->gesture_support = 1;
    }
/* N6 code for HQ-347795 by liaoxianguo at 2023/11/21 end */
/* N6 code for HQ-335337 by liaoxianguo at 2023/10/29 end */

    if (ts_data->gesture_support) {
        fts_gesture_suspend(ts_data);
        fts_gestures_status = 1;
    } else {
        fts_irq_disable();
        fts_gestures_status = 0;
...
static int fts_ts_resume(struct device *dev)
{
    ...
    /* N6 code for HQ-301696 by liaoxianguo at 2023/8/25 end */
    FTS_FUNC_ENTER();
/* N6 code for HQ-310178 by zhangyundan at 2023/08/10 start */
#ifdef KERNEL_FACTORY_BUILD
    ts_data->fod_status = 1;
    fts_write_reg(FTS_REG_GESTURE_SUPPORT ,FTS_REG_GESTURE_FOD_ON);
    FTS_INFO("FACTORY fod always on");
#endif
/* N6 code for HQ-310178 by zhangyundan at 2023/08/10 start */
    if (!ts_data->suspended) {
...
/* N6 code for HQ-310178 by zhangyundan at 2023/08/02 end */
    ...
    for(i = 0; i < retry_time; i++) {
        fts_read_reg(FTS_REG_CHIP_ID, &id);
        if(id == FTS_REG_CHIP_ID_EN) {
            break;
        }
        msleep(10);
    }
                ...
                unsigned long event, void *data)
{
    ...
    if (!(event == MI_DISP_DPMS_EARLY_EVENT ||
        ...
        FTS_ERROR("event(%lu) do not need process", event);
        return 0;
    }
    if (evdata && evdata->data && ts_data) {
        blank = *(int *)(evdata->data);
        FTS_ERROR("event:%lu,blank:%u", event, blank);
...
static int fts_notifier_callback_init(struct fts_ts_data *ts_data)
{
 ...
 /*
    ...
    if (1) {
        ret = mtk_disp_notifier_register("FT3683G Touch", &ts_data->fb_notif);
...
#endif //CONFIG_DRM*/
    ts_data->fb_notif.notifier_call = fb_notifier_callback;
    FTS_INFO("init notifier with mi_disp_register_client");
#if IS_ENABLED(CONFIG_MI_DISP_NOTIFIER)
...
#endif //CONFIG_DRM*/
#if IS_ENABLED(CONFIG_MI_DISP_NOTIFIER)
    ret = mi_disp_unregister_client(&ts_data->fb_notif);
    if (ret) {
        FTS_ERROR("[FB]Unable to unregister fb_notifier: %d", ret);
    }
#endif
    FTS_FUNC_EXIT();
    return 0;
...
/* N6 code for HQ-304097 by zhangyundan at 2023/07/26 end */
/*N6 code for HQ-326418 by liaoxianguo at 2023/09/25 start*/
static int fts_fod_test_store(int value)
{
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    if (value) {
        ...
        input_report_abs(input_dev, ABS_MT_POSITION_Y, FOD_TEST_POSITION_X);
        input_sync(input_dev);
    } else {
        input_mt_slot(input_dev, 0);
...
/*N6 code for HQ-326418 by liaoxianguo at 2023/09/25 end*/
/* N6 code for HQ-304097 by zhangyundan at 2023/7/26 start */
void fts_update_gesture_state(struct fts_ts_data *ts_data, int bit, bool enable)
{
    ...
    if (ts_data->suspended) {
        ...
        FTS_INFO("AOD: %d", ts_data->gesture_status>>1 & 0x01);
...
/* N6 code for HQ-334004 by liaoxianguo at 2023/10/28 start */
        fts_data->gesture_support = 1;
        if (!fts_data->finger_in_fod && fts_data->fod_status == 0) {
            fts_fod_reg_write(FTS_REG_GESTURE_FOD_ON, false);
        }
        if (fts_data->fod_status == 1 || fts_data->fod_status == 2) {
            fts_fod_recovery();
        }
/* N6 code for HQ-334004 by liaoxianguo at 2023/10/28 end */
        return 0;
    }
/* N6 code for HQ-310178 by zhangyundan at 2023/08/02 end */
/*N6 code for HQ-302671 by dingying at 2023/08/08 start*/
...
static void fts_init_xiaomi_touchfeature(struct fts_ts_data *ts_data)
{
    ...
    /*N6 code for HQ-326418 by liaoxianguo at 2023/09/25 start*/
    xiaomi_touch_interfaces.fod_test_store = fts_fod_test_store;
    /*N6 code for HQ-326418 by liaoxianguo at 2023/09/25 end*/
...
/* N6 code for HQ-310178 by zhangyundan at 2023/08/02 start */
    ...
    ts_data->fod_status = -1;
/* N6 code for HQ-310178 by zhangyundan at 2023/08/02 stop */
    ...
    if (ret){
        FTS_ERROR("set reset gpio to hight failed");
    }
    msleep(100);
    /* N6 code for HQ-301696 by liaoxianguo at 2023/8/25 end */
#endif
...
/* N6 code for HQ-304232 by zhangyundan at 2023/07/10 start */
    ret = fts_hw_info_add(ts_data);
    if (ret){
        FTS_ERROR("hw_info add fail");
    }
/* N6 code for HQ-304232 by zhangyundan at 2023/07/10 end */
    if (ts_data->ts_workqueue) {
...
#endif
/* N6 code for HQ-317632 by liaoxianguo at 2023/8/18 start */
    if (IS_ERR_OR_NULL(ts_data->pdata)){
        return -EINVAL;
    }
/* N6 code for HQ-317632 by liaoxianguo at 2023/8/18 end */
