...
int start_selftest(int type)
{
    ...
    if (!tdata) {
        ...
        return -TP_SELFTEST_FAIL;
    }

    FTS_TEST_INFO("test item:0x%x", tdata->ic.mc_sc.u.tmp);

    ret = fts_test_read_reg(FACTORY_REG_LCD_NOISE_TTHR, &tdata->fre_num);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read fre_num fails");
        return ret;
    }
    ...
    /* entry factory mode */
    ft5672_spi_test(tdata, &test_result);
    if (false == test_result) {
        ret = -TP_SELFTEST_FAIL;
        fts_ftest->test_item[FTS_ENTER_FACTORY_MODE].testresult = RESULT_NG;
    } else {
        fts_ftest->test_item[FTS_ENTER_FACTORY_MODE].testresult = RESULT_PASS;
    }
    ...
    /* short test */
    if (type == TEST_SHORT) {
        ft5672_short_test(tdata, &test_result);
        if (false == test_result) {
            ret = -TP_SELFTEST_FAIL;
            fts_ftest->test_item[FTS_SHORT_CIRCUIT_TEST].testresult = RESULT_NG;
        } else {
    ...
    /* open test */
    if (type == TEST_OPEN) {
        ft5672_panel_differ_test(tdata, &test_result);
        if (false == test_result) {
            ret = -TP_SELFTEST_FAIL;
            fts_ftest->test_item[FTS_OPEN_TEST].testresult = RESULT_NG;
        } else {
