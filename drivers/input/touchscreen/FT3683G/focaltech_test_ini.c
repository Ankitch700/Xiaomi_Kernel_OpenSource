...
/* N6 code for HQ-324069 by liaoxianguo at 2023/09/08 start */
    FTS_TEST_INFO("get ini file,size=%d, last buf=%c %c", ini->length, ini->data[ini->length-1], ini->data[ini->length-2]);
/* N6 code for HQ-324069 by liaoxianguo at 2023/09/08 end */
    return ret;
#else
...
static int ini_get_line(char *filedata, char *line_data, int *line_len, int pos, int ini_max_size)
{
    ...
    for (i = 0; i < MAX_INI_LINE_LEN; i++) {
        if (('\n' == filedata[i]) || ('\r' == filedata[i])) {
            ...
            FTS_TEST_INFO("ini-line,pos=%d,line data=%s", pos + i, line_data);
...
static int ini_parse_keyword(struct ini_data *ini, char *line_buffer)
{
    ...
    /* N6 code for HQ-324069 by liaoxianguo at 2023/09/08 start */
    FTS_TEST_INFO("keyname:%s,value:%s keyword offset:%d", ini->tmp[offset].name, ini->tmp[offset].value, ini->keyword_num_total);
    /* N6 code for HQ-324069 by liaoxianguo at 2023/09/08 end */
    section->keyword_num++;
...
static int ini_init_inidata(struct ini_data *ini)
{
    ...
    while (pos < ini->length) {
        /* N6 code for HQ-324069 by liaoxianguo at 2023/09/08 start */
        FTS_TEST_INFO("ini data pos=%d", pos);
