...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    if (upg->ts_data->fw_loading){
        FTS_INFO("fw upgrade in process, don't upgrade again");
        return;
    }
    ...
    /*N6 code for HQ-322876 by dingying at 2023/08/29 start*/
    ...
    upg->ts_data->fw_loading = 0;
    /*N6 code for HQ-322876 by dingying at 2023/08/29 start*/
    ret = fts_hw_info_add(fts_data);
    if (ret){
        FTS_ERROR("hw_info add fail");
    }
    /*N6 code for HQ-322876 by dingying at 2023/08/29 end*/
}
...
static int32_t fts_tp_info_show(struct seq_file *m, void *v)
{
        ...
        if (ret < 0){
                FTS_ERROR("Read fw version failed");
                return ret;
        }
        seq_printf(m, "[Vendor]:Huaxing [TP-IC]:FT3683G [FW]:0x%x\n", fw_version);
        return 0;
}
...
int fts_hw_info_add(struct fts_ts_data *ts_data)
{
    ...
    sprintf(tp_version_info, "[Vendor]Huaxing [TP-IC]:FT3683G [FW]0x%x\n", fw_version);
...
int lct_get_lockdown_info(void)
{
    ...
    ret = fts_flash_read(0x3f000, lockdown_values, 8);
    if (ret)
