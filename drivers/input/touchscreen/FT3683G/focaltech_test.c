...
/*N6 code for HQ-302302 ITO by dingying at 2023/07/10 start*/
...
/*N6 code for HQ-302302 ITO by dingying at 2023/07/10 end*/
/*N6 code for HQ-304306 by liaoxianguo at 2023/10/11 start*/
static struct proc_dir_entry *fts_tp_selftest_proc = NULL;
/*N6 code for HQ-304306 by liaoxianguo at 2023/10/11 end*/
/*N6 code for HQ-304290 tp_data_dump by dingying at 2023/07/18 start*/
...
#endif
/*N6 code for HQ-304290 tp_data_dump by dingying at 2023/07/18 end*/
...
static int tp_data_dump_proc_open(struct inode *inode, struct file *file)
{
    ...
    if (ret < 0) {
        FTS_TEST_ERROR("lct_tp_get_diffdata() failed, ret = %d", ret);
        goto err_get_data_fail;
    }
    goto exit;
err_get_data_fail:
...
/* N6 code for HQ-301654 by dingying at 2023/07/20 start */
/* N6 code for HQ-302343 by dingying at 2023/08/10 start */
...
/* N6 code for HQ-302343 by dingying at 2023/08/10 end */
/* N6 code for HQ-301654 by dingying at 2023/07/20 start */
    return 0;
}
...
/*N6 code for HQ-304306 by liaoxianguo at 2023/10/11 start*/
//-------------------/proc/tp_selftest begin-----------------
int fts_tp_selftest_start(int type)
{
    int testresult = 0;

    testresult = start_selftest(type);
    return testresult;
}

static int fts_tp_selftest_entry(char *ini_file_name, int type)
{
    int ret = 0;
    ...
    /* Start testing according to the test configuration */
    ret = fts_tp_selftest_start(type);
    if (ret < 0) {
        FTS_TEST_SAVE_INFO("\n\n=======Tp test failure.\n");
        fts_ftest->tp_selftest = TP_SELFTEST_FAIL;
    } else {
        FTS_TEST_SAVE_INFO("\n\n=======Tp test pass.\n");
        fts_ftest->tp_selftest = TP_SELFTEST_PASS;
    }
...
{
    int ret = 0;
    int tp_selftest_type = 0;
    char tmp[FILE_NAME_LENGTH] = { 0 };
    char fwname[FILE_NAME_LENGTH] = "focaltech_mp_fw_huaxin.ini";
    struct fts_ts_data *ts_data = fts_ftest->ts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    ...
    FTS_TEST_DBG("tmp:%s.", tmp);
    FTS_TEST_DBG("fwname:%s.", fwname);

    if (memcmp(tmp, "i2c", IIC_TEST_LEN) == 0) {
        ret = fts_wait_tp_to_valid();
        if (ret < 0)
            fts_ftest->tp_selftest = TP_SELFTEST_FAIL;
        else
            fts_ftest->tp_selftest = TP_SELFTEST_PASS;
    } else if (memcmp(tmp, "short", SHORT_TEST_LEN) == 0) {
        tp_selftest_type = TEST_SHORT;
        mutex_lock(&input_dev->mutex);
        fts_irq_disable();
        fts_esdcheck_switch(ts_data, DISABLE);

        ret = fts_enter_test_environment(true);
        if (ret < 0)
            FTS_ERROR("enter test environment fail");
        else
            fts_tp_selftest_entry(fwname, tp_selftest_type);
        ret = fts_enter_test_environment(false);
        if (ret < 0)
            FTS_ERROR("enter normal environment fail");
        ...
        fts_irq_enable();
        mutex_unlock(&input_dev->mutex);
    } else if (memcmp(tmp, "open", OPEN_TEST_LEN) == 0) {
        tp_selftest_type = TEST_OPEN;
        mutex_lock(&input_dev->mutex);
        fts_irq_disable();
        fts_esdcheck_switch(ts_data, DISABLE);
        ...
        fts_ftest->tp_selftest = TP_SELFTEST_INVALID;
    }
...
{
    char tmp[FTS_TP_SELFTEST_TMP_LENGTH] = { 0 };
    int cnt = 0;

    if (*pos != 0)
        return 0;

    cnt = snprintf(tmp, sizeof(fts_ftest->tp_selftest), "%d\n",
           fts_ftest->tp_selftest);

    FTS_TEST_INFO("tmp = %s, fts_ftest->tp_selftest = %d", tmp,
          fts_ftest->tp_selftest);
...
const struct proc_ops fts_tp_selftest_ops = {
    .proc_read = fts_tp_selftest_read,
    .proc_write = fts_tp_selftest_write,
};
//-------------------/proc/tp_selftest end-----------------
/*N6 code for HQ-304306 by liaoxianguo at 2023/10/11 end*/
...
/*N6 code for HQ-304290 tp_data_dump by dingying at 2023/07/18 start*/
    ...
    lct_tp_data_dump_p->tp_data_dump_proc = proc_create(FTS_PROC_TP_DATA_DUMP, 0664, NULL,
...
/*N6 code for HQ-304306 by liaoxianguo at 2023/10/11 start*/
    fts_tp_selftest_proc = proc_create(FTS_TP_SELFTEST, 0664, NULL,
        &fts_tp_selftest_ops);
    if (!fts_tp_selftest_proc) {
        FTS_ERROR("Failed to create tp_selftest");
        return -EINVAL;
    } else {
        FTS_INFO("Sucess to creat proc/tp_selftest");
    }
/*N6 code for HQ-304306 by liaoxianguo at 2023/10/11 end*/
