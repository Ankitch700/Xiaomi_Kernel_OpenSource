...
int fts_gesture_switch(struct input_dev *dev, unsigned int type, unsigned int code, int value)
{
    ...
    if (type == EV_SYN && code == SYN_CONFIG) {
        if (value == WAKEUP_OFF) {
            ...
            FTS_INFO("gesture disabled:%d", ts_data->gesture_support);
            fts_update_gesture_state(fts_data, GESTURE_DOUBLETAP, false);
            ts_data->double_click = false;
        } else if (value == WAKEUP_ON) {
            ts_data->gesture_support = ENABLE;
            FTS_INFO("gesture enabled:%d", ts_data->gesture_support);
            fts_update_gesture_state(fts_data, GESTURE_DOUBLETAP,true);
            ts_data->double_click = true;
        }
    }
    FTS_FUNC_EXIT();
    return 0;
