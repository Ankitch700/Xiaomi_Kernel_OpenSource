...
/*N6 code for HQ-305074 by dingying at 2023/10/07 start*/
#define GTP_SHORT_TEST 4
/*N6 code for HQ-305074 by dingying at 2023/10/07 end*/
...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	/*N6 code for HQ-305074 by dingying at 2023/09/27 start*/
	sprintf(limit_file, "%s_%d_VSN.csv", GOODIX_TEST_FILE_NAME,
...
static int goodix_analysis_rawdata(struct goodix_ts_test *ts_test)
{
	...
	if (err_cnt * 100 > times * 100 * 1 / 10) {
		ts_info("rawdata more than 10%%(%d) fail, need test again",
			err_cnt);
		return 1;
	}
...
int inspect_module_init(struct goodix_ts_core *core_data)
{
	...
	proc_entry = proc_create_data("self_test", 0664, gt_android_touch_proc,
