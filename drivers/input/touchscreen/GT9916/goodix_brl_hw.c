...
/* N6 code for HQ-361771 by p-chenkunying at 2024/1/25 start */
	kfree(tmp_buf);
/* N6 code for HQ-361771 by p-chenkunying at 2024/1/25 end */
	return ret;
}
			 ...
			 struct goodix_ts_event *ts_event)
{
	...
	/*N6 code for HQ-340465 by huangshiquan at 2023/11/03 start*/
	if (checksum_cmp(pre_buf, IRQ_EVENT_HEAD_LEN, CHECKSUM_MODE_U8_LE)) {
		/*avoiding abnormal reset of flag bits due to read-write conflicts*/
		if (pre_buf[0] == 0) {
			pre_buf[0] = GESTURE_WITHOUT_FINGERPRINT;
			if (checksum_cmp(pre_buf, IRQ_EVENT_HEAD_LEN, CHECKSUM_MODE_U8_LE)) {
				pre_buf[0] = SCREENOFF_TOUCH;
				if (checksum_cmp(pre_buf, IRQ_EVENT_HEAD_LEN,
						 CHECKSUM_MODE_U8_LE)) {
					ts_err("touch head checksum err[%*ph]",
	...
	/*N6 code for HQ-340465 by huangshiquan at 2023/11/03 end*/
...
static int brl_game(struct goodix_ts_core *cd, u8 data0, u8 data1, bool on)
{
	...
	if (cd->hw_ops->send_cmd(cd, &cmd)) {
		ts_err("failed send game cmd, data0 = 0x%x, data1 = 0x%x, on = %d",
		       cmd.data[0], cmd.data[1], on);
		return -EINVAL;
	}
	ts_info("game data0:0x%x, data1:0x%x, game mode %s", cmd.data[0],
		cmd.data[1], (on == true) ? "on" : "off");
	return 0;
}
/*N6 code for HQ-305061 by zhangzhijian5 at 2023/10/30 end*/

/*N6 code for HQ-305055 by zhangzhijian5 at 2023/11/1 start*/
static int brl_edge_mode_set(struct goodix_ts_core *cd, int value)
{
	struct goodix_ts_cmd cmd = {0};
...
static int brl_hdle_mode_set(struct goodix_ts_core *cd, bool value)
{
	...
	if (!cd || !cd->hw_ops || !cd->hw_ops->send_cmd) {
		ts_err("cd or cd->hw_ops or cd->hw_ops->send_cmd is NULL\n");
		return -EINVAL;
	}

	memset(&cmd, 0, sizeof(struct goodix_ts_cmd));
        cmd.cmd = GOODIX_HDLE_MODE_CMD;
        cmd.len = GOODIX_HDLE_MODE_CMD_LENGTH;
