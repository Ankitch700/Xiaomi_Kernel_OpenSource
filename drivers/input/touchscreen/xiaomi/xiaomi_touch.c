...
int xiaomitouch_register_modedata(int touchId, struct xiaomi_touch_interface *data)
{
	...
	if (data->touch_edge_mode_set)
		touch_data->touch_edge_mode_set = data->touch_edge_mode_set;
	/*N6 code for HQ-302671 by dingying at 2023/08/08 end*/
	/* N6 code for HQ-330294 by liaoxianguo at 2023/9/21 start */
	if (data->touch_hdle_mode_set)
		touch_data->touch_hdle_mode_set = data->touch_hdle_mode_set;
	/* N6 code for HQ-330294 by liaoxianguo at 2023/9/21 end */
	/* N6 code for HQ-326418 by liaoxianguo at 2023/9/25 start */
	if (data->fod_test_store)
		touch_data->fod_test_store = data->fod_test_store;
	/* N6 code for HQ-326418 by liaoxianguo at 2023/9/25 end */
...
/*N6 code for HQ-302671 by dingying at 2023/08/08 start*/
static ssize_t touch_edge_mode_set(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int value = 0;
	int game_mode_enable = 0;
	struct xiaomi_touch_pdata *pdata = dev_get_drvdata(dev);
	...
	if (pdata->touch_data[0]->touch_edge_mode_set) {
		if (pdata->touch_data[0]->getModeValue)
			game_mode_enable =
                  	pdata->touch_data[0]->getModeValue(Touch_Game_Mode, GET_CUR_VALUE);
		if(!game_mode_enable)
			pdata->touch_data[0]->touch_edge_mode_set(value);
		else
			pr_info("%s game mode enabled, Normal edge failed:%d\n", __func__, value);
	}
	else {
		pr_err("%s has not edge\n", __func__);
	}
...
/*N6 code for HQ-302671 by dingying at 2023/08/08 end*/
/* N6 code for HQ-330294 by liaoxianguo at 2023/9/21 start */
static ssize_t touch_hdle_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int value = 0;
	struct xiaomi_touch_pdata *pdata = dev_get_drvdata(dev);
	...
	if (pdata->touch_data[0]->touch_hdle_mode_set)
		pdata->touch_data[0]->touch_hdle_mode_set(!!value);
	else {
		pr_err("%s has not implement\n", __func__);
	}
	pr_info("%s value:%d\n", __func__, !!value);
...
/*N6 code for HQ-302671 by dingying at 2023/08/08 start*/
static DEVICE_ATTR(touch_edge, (S_IRUGO | S_IWUSR | S_IWGRP), NULL, touch_edge_mode_set);
/*N6 code for HQ-302671 by dingying at 2023/08/08 end*/

/* N6 code for HQ-330294 by liaoxianguo at 2023/9/21 start */
static DEVICE_ATTR(touch_hdle, (S_IRUGO | S_IWUSR | S_IWGRP), NULL, touch_hdle_store);
/* N6 code for HQ-330294 by liaoxianguo at 2023/9/21 end */
...
static struct attribute *touch_attr_group[] = {
	...
	/*N6 code for HQ-302671 by dingying at 2023/08/08 start*/
	&dev_attr_touch_edge.attr,
	/*N6 code for HQ-302671 by dingying at 2023/08/08 end*/
	/* N6 code for HQ-330294 by liaoxianguo at 2023/9/21 start */
	&dev_attr_touch_hdle.attr,
	/* N6 code for HQ-330294 by liaoxianguo at 2023/9/21 end */
	/*N6 code for HQ-326418 by liaoxianguo at 2023/09/25 start*/
