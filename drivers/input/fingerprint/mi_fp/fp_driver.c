...
static ssize_t get_fplive_event(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct fp_device *fp_dev = &fp;
...
static ssize_t set_fplive_event(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	struct fp_device *fp_dev = &fp;
	...
	if (!strncmp(buf, "1", strlen("1"))) {
		fp_dev->fplive = 1;
		dev_info(dev, "%s set fplive 1 \n", __func__);
		sysfs_notify(&fp_dev->driver_device->dev.kobj, NULL, "fplive");
	}
	else if (!strncmp(buf, "0", strlen("0"))) {
		fp_dev->fplive = 0;
		dev_info(dev, "%s set fplive 0 \n", __func__);
		sysfs_notify(&fp_dev->driver_device->dev.kobj, NULL, "fplive");
	}
	else {
		dev_err(dev,"failed to set fplive\n");
		return -EINVAL;
	}
	return count;
}
static DEVICE_ATTR(fplive, S_IRUSR | S_IWUSR, get_fplive_event, set_fplive_event);

static struct attribute *attributes[] = {
#ifdef FP_ULTRA_QCOM
	&dev_attr_intr2.attr,
	&dev_attr_fingerdown.attr,
	&dev_attr_fplive.attr,
#endif
#ifdef CONFIG_SIDE_FINGERPRINT
	&dev_attr_irq.attr,
	&dev_attr_fingerdown.attr,
