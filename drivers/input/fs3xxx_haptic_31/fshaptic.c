...
static int foursemi_i2c_read(struct foursemi *foursemi, unsigned char reg_addr,
...
static int foursemi_i2c_write(struct foursemi *foursemi, unsigned char reg_addr,
			      unsigned char reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
	...
	while (cnt < FS_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(foursemi->i2c, reg_addr,
						reg_data);
		if (ret < 0) {
...
static int select_pin_ctl(struct foursemi *foursemi, const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(foursemi->pinctrl_state); i++) {
		...
		if (!strncmp(n, name, strlen(n))) {
			rc = pinctrl_select_state(foursemi->foursemi_pinctrl,
						  foursemi->pinctrl_state[i]);
			if (rc)
...
static int foursemi_parse_dt(struct device *dev, struct foursemi *foursemi,
			     struct device_node *np)
{
	...
	val = of_property_read_u32(np, "fs3001_i2c_addr",
				   &foursemi->fs3001_i2c_addr);
	if (val)
...
static int foursemi_i2c_probe(struct i2c_client *i2c,
			      const struct i2c_device_id *id)
{
	...
	for (i = 0; i < ARRAY_SIZE(foursemi->pinctrl_state); i++) {
		...
		struct pinctrl_state *state =
			pinctrl_lookup_state(foursemi->foursemi_pinctrl, n);
		if (!IS_ERR(state)) {
	...
	if (gpio_is_valid(foursemi->irq_gpio)) {
		...
		ret = devm_gpio_request_one(&i2c->dev, foursemi->irq_gpio,
					    GPIOF_DIR_IN, "foursemi_int");
		if (ret) {
	...
	//alloc fs3001 struct
	if (foursemi->name == FS3001_A1 || foursemi->name == FS3001_A2 ||
	    foursemi->name == FS3001_A3) {
		foursemi->fs3001 = devm_kzalloc(
			&i2c->dev, sizeof(struct fs3001), GFP_KERNEL);
		if (foursemi->fs3001 == NULL) {
			if (gpio_is_valid(foursemi->irq_gpio))
				devm_gpio_free(&i2c->dev, foursemi->irq_gpio);
			if (gpio_is_valid(foursemi->reset_gpio) &&
			    (!foursemi->enable_pin_control))
		...
		//irq
		if (gpio_is_valid(foursemi->fs3001->irq_gpio) &&
			...
			ret = devm_request_threaded_irq(
				&i2c->dev,
				gpio_to_irq(foursemi->fs3001->irq_gpio), NULL,
				fs3001_irq, irq_flags, "fs3001",
				foursemi->fs3001);
			if (ret != 0) {
				pr_err("%s:failed to request IRQ %d: %d\n",
				       FSERROR,
				       gpio_to_irq(foursemi->fs3001->irq_gpio),
				       ret);
				goto err_fs3001_irq;
			}
		} else {
		...
		if (rc < 0) {
			dev_err(foursemi->fs3001->dev,
				"create FF input device failed, rc=%d\n", rc);
			return rc;
		} else {
			pr_info("input_ff_create ok, rc=%d\n", rc);
		}

		foursemi->fs3001->work_queue = create_singlethread_workqueue(
			"fs3001_vibrator_work_queue");
		if (!foursemi->fs3001->work_queue) {
			dev_err(&i2c->dev,
				"Error creating fs3001_vibrator_work_queue\n");
			goto err_fs3001_sysfs;
		}
		INIT_WORK(&foursemi->fs3001->set_gain_work,
			  fs3001_haptics_set_gain_work_routine);
		ff = input_dev->ff;
...
static int foursemi_i2c_remove(struct i2c_client *i2c)
{
	...
	if (foursemi->name == FS3001_A1 || foursemi->name == FS3001_A2 ||
		...
		free_pages((unsigned long)foursemi->fs3001->start_buf,
			   HAPSTREAM_MMAP_PAGE_ORDER);
		...
		sysfs_remove_group(&foursemi->fs3001->i2c->dev.kobj,
...
static const struct i2c_device_id foursemi_i2c_id[] = { { FOURSEMI_I2C_NAME,
							  0 },
							{} };
	...
	.driver =
		{
			...
			.of_match_table = of_match_ptr(
				foursemi_dt_match), //match with dts "compatible"
		},
