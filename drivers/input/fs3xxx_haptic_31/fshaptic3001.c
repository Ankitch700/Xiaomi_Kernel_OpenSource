...
static void
fs3001_haptic_set_diagnostic_reg_to_default_value(struct fs3001 *fs3001);
...
static void pm_qos_enable(struct fs3001 *fs3001, bool b_enable)
{
	...
	sprintf(str, "%s,OVER_5_10 enable=%d, Qos_time=%d\n", __func__,
		b_enable, fs3001->Qos_time);
	fs3001_debug_message(fs3001, str);
	if (b_enable) {
		cpu_latency_qos_add_request(&fs3001_pm_qos_req_vb,
					    fs3001->Qos_time);
	} else {
	...
	fs3001_debug_message(fs3001, str);
	if (b_enable) {
		pm_qos_add_request(&fs3001_pm_qos_req_vb,
				   PM_QOS_CPU_DMA_LATENCY, fs3001->Qos_time);
	} else {
...
static int fs3001_i2c_write(struct fs3001 *fs3001, unsigned char reg_addr,
			    unsigned char reg_data)
{
	...
	while (cnt < FS3001_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(fs3001->i2c, reg_addr,
						reg_data);
		if (ret < 0) {
		...
		cnt++;
		usleep_range(FS3001_I2C_RETRY_DELAY * 1000,
			     FS3001_I2C_RETRY_DELAY * 1000 + 500);
	}
	return ret;
}

static int fs3001_i2c_read(struct fs3001 *fs3001, unsigned char reg_addr,
			   unsigned char *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
...
static int fs3001_i2c_writes(struct fs3001 *fs3001, unsigned char reg_addr,
...
static int fs3001_i2c_write_bits_1(struct fs3001 *fs3001,
				   unsigned char reg_addr,
				   unsigned char reg_data,
				   unsigned char i_start, unsigned char i_stop)
{
	int ret = -1;
...
static unsigned char fs3001_i2c_read_bits(struct fs3001 *fs3001,
					  unsigned char reg_addr, int i_start,
					  int i_stop)
{
	int ret = -1;
	unsigned char reg_val = 0;
...
int fs3001_parse_dt(struct device *dev, struct fs3001 *fs3001,
		    struct device_node *np)
{
	...
	//fs3001_f0_ref
	val = of_property_read_u32(np, "fs3001_f0_ref",
	...
	//fs3001_auto_brake
	val = of_property_read_u32(np, "fs3001_auto_brake",
	...
	//fs3001_f0_cali_mode
	val = of_property_read_u32(np, "fs3001_f0_cali_mode",
	...
	val = of_property_read_u32(np, "fs3001_cont_drv1_lvl",
				   &fs3001->dts_info.fs3001_cont_drv1_lvl);
	if (val != 0) {
		pr_err("%s:fs3001_cont_drv1_lvl not found\n", FSERROR);
		fs3001->dts_info.fs3001_cont_drv1_lvl =
			FS3001_CONT_DRV1_LVL_DEFAULT;
	} else {
	...
	val = of_property_read_u32(np, "fs3001_cont_drv2_lvl",
				   &fs3001->dts_info.fs3001_cont_drv2_lvl);
	if (val != 0) {
		pr_err("%s:fs3001_cont_drv2_lvl not found\n", FSERROR);
		fs3001->dts_info.fs3001_cont_drv2_lvl =
			FS3001_CONT_DRV2_LVL_DEFAULT;
	} else {
	...
	val = of_property_read_u32(np, "fs3001_cont_drv1_time",
				   &fs3001->dts_info.fs3001_cont_drv1_time);
	if (val != 0) {
		pr_err("%s:fs3001_cont_drv1_time not found\n", FSERROR);
		fs3001->dts_info.fs3001_cont_drv1_time =
			FS3001_CONT_DRV1_TIME_DEFAULT;
	} else {
	...
	val = of_property_read_u32(np, "fs3001_cont_drv2_time",
				   &fs3001->dts_info.fs3001_cont_drv2_time);
	if (val != 0) {
		pr_err("%s:fs3001_cont_drv2_time not found\n", FSERROR);
		fs3001->dts_info.fs3001_cont_drv2_time =
			FS3001_CONT_DRV2_TIME_DEFAULT;
	} else {
	...
	val = of_property_read_u32(np, "fs3001_cont_1_period",
				   &fs3001->dts_info.fs3001_cont_1_period);
	if (val != 0) {
		pr_err("%s:fs3001_cont_1_period not found\n", FSERROR);
		fs3001->dts_info.fs3001_cont_1_period =
			FS3001_CONT_1_PERIOD_DEFAULT;
	} else {
	...
	//fs3001_brk_slopeth
	val = of_property_read_u32(np, "fs3001_brk_slopeth",
				   &fs3001->dts_info.fs3001_brk_slopeth);
	if (val != 0) {
		pr_err("%s:fs3001_brk_slopeth not found\n", FSERROR);
		fs3001->dts_info.fs3001_brk_slopeth =
			FS3001_BRK_SLOPETH_DEFAULT;
	} else {
	...
	//fs3001_brk_gain
	val = of_property_read_u32(np, "fs3001_brk_gain",
	...
	//fs3001_brk_times
	val = of_property_read_u32(np, "fs3001_brk_times",
	...
	val = of_property_read_u32(np, "fs3001_brk_noise_gate",
				   &fs3001->dts_info.fs3001_brk_noise_gate);
	if (val != 0) {
		pr_err("%s:fs3001_brk_noise_gate not found\n", FSERROR);
		fs3001->dts_info.fs3001_brk_noise_gate =
			FS3001_BRK_NOISE_GATE_DEFAULT;
	} else {
	...
	//fs3001_brk_1_period
	val = of_property_read_u32(np, "fs3001_brk_1_period",
				   &fs3001->dts_info.fs3001_brk_1_period);
	if (val != 0) {
		pr_err("%s:fs3001_brk_1_period not found\n", FSERROR);
		fs3001->dts_info.fs3001_brk_1_period =
			FS3001_BRK_1_PERIOD_DEFAULT;
	} else {
	...
	//fs3001_brk_pgagain
	val = of_property_read_u32(np, "fs3001_brk_pgagain",
				   &fs3001->dts_info.fs3001_brk_pgagain);
	if (val != 0) {
		pr_err("%s:fs3001_brk_pgagain not found\n", FSERROR);
		fs3001->dts_info.fs3001_brk_pgagain =
			FS3001_BRK_PGAGAIN_DEFAULT;
	} else {
	...
	//fs3001_brk_margin
	val = of_property_read_u32(np, "fs3001_brk_margin",
	...
	val = of_property_read_u32(np, "fs3001_play_ram_srate",
				   &fs3001->dts_info.fs3001_play_ram_srate);
	if (val != 0) {
		pr_err("%s:fs3001_play_ram_srate not found\n", FSERROR);
		fs3001->dts_info.fs3001_play_ram_srate =
			FS3001_PLAY_RAM_SRATE_DEFAULT;
	} else {
	...
	val = of_property_read_u32(np, "fs3001_play_rtp_srate",
				   &fs3001->dts_info.fs3001_play_rtp_srate);
	if (val != 0) {
		pr_err("%s:fs3001_play_rtp_srate not found\n", FSERROR);
		fs3001->dts_info.fs3001_play_rtp_srate =
			FS3001_PLAY_RTP_SRATE_DEFAULT;
	} else {
	...
	val = of_property_read_u32(np, "fs3001_default_vib_mode",
	...
	//	fs3001_vbat_mode
	val = of_property_read_u32(np, "fs3001_vbat_mode",
	...
	// fs3001_lk_f0_cali
	val = of_property_read_u32(np, "fs3001_lk_f0_cali",
	...
	val = of_property_read_u32(np, "fs3001_bypass_system_gain",
				   &fs3001->dts_info.fs3001_bypass_system_gain);
	if (val != 0) {
		pr_err("%s:fs3001_bypass_system_gain not found\n", FSERROR);
		fs3001->dts_info.fs3001_bypass_system_gain =
			FS3001_BYPASS_SYSTEM_GAIN_DEFAULT;
	} else {
	...
	val = of_property_read_u32(np, "fs3001_f0_cali_data_mode",
				   &fs3001->dts_info.fs3001_f0_cali_data_mode);
	if (val != 0) {
		pr_err("%s:fs3001_f0_cali_data_mode not found\n", FSERROR);
		fs3001->dts_info.fs3001_f0_cali_data_mode =
			FS3001_F0_CALI_DATA_MODE_DEFAULT;
	} else {
	...
	//	fs3001_lr_pgagain
	val = of_property_read_u32(np, "fs3001_lr_pgagain",
	...
	val = of_property_read_u32_array(np, "fs3001_duration_time",
					 duration_time,
					 ARRAY_SIZE(duration_time));
	if (val != 0) {
		pr_err("%s:fs3001_duration_time not found\n", FSERROR);
		for (i = 0; i < ARRAY_SIZE(duration_time); i++) {
			duration_time[i] = FS3001_DURATION_TIME_DEFAULT * i +
					   FS3001_DURATION_TIME_DEFAULT;
		}
	} else {
		for (i = 0; i < ARRAY_SIZE(duration_time); i++) {
	...
	memcpy(fs3001->dts_info.fs3001_duration_time, duration_time,
	       sizeof(duration_time));

	//fs3001_reg_inits = < 0x5f0001 0x5f3411 0x5f7080 >;
	val = of_property_read_u32_array(np, "fs3001_reg_inits", reg_inits,
					 ARRAY_SIZE(reg_inits));
	if (val != 0) {
	...
	val = of_property_read_u32(np, "fs3001_rtp_id_boundary",
				   &fs3001->dts_info.fs3001_rtp_id_boundary);
	if (val != 0) {
		pr_err("%s:fs3001_rtp_id_boundary not found\n", FSERROR);
		fs3001->dts_info.fs3001_rtp_id_boundary =
			FS3001_RTP_ID_BOUNDARY_DEFAULT;
	} else {
	...
	//	fs3001_rtp_max
	val = of_property_read_u32(np, "fs3001_rtp_max",
	...
	//fs3001_rtp_time
	val = of_property_read_u32_array(np, "fs3001_rtp_time", rtp_time,
					 ARRAY_SIZE(rtp_time));
	if (val != 0) {
	...
	fs3001->constant.pattern = devm_kcalloc(
		fs3001->dev, HAP_WAVEFORM_BUFFER_MAX, sizeof(u8), GFP_KERNEL);
	if (!fs3001->constant.pattern)
		return -ENOMEM;

	tmp = of_get_available_child_count(np);
	//devm_kcalloc alloc tmp size of sizeof(*fs3001->predefined)
	fs3001->predefined = devm_kcalloc(
		fs3001->dev, tmp, sizeof(*fs3001->predefined), GFP_KERNEL);
	...
	for_each_available_child_of_node (np, child_node) {
		...
		if (rc < 0) {
			...
			sprintf(str,
				"%s,effect_id %d,qcom,wf-pattern count =0x%x \n",
		...
		effect->pattern =
			devm_kcalloc(fs3001->dev, effect->pattern_length,
...
static void fs3001_haptic_upload_lra(struct fs3001 *fs3001, unsigned int flag)
{
	...
	case FS3001_F0_CALI:
		...
		fs3001_debug_message(fs3001, str);
		fs3001_i2c_write(fs3001, FS3001_FTUNECFG,
				 (char)fs3001->f0_cali_data);
		break;
...
static int fs3001_haptic_set_gain(struct fs3001 *fs3001, unsigned char gain)
{
	...
	case FS3001_HAPTIC_VBAT_COMP_PLAY_AUTO_BRK_ENABLE:
		sprintf(str, "%s, vbat mode = %d, gain=0x%x", __func__,
			fs3001->dts_info.fs3001_vbat_mode, gain);
		fs3001_debug_message(fs3001, str);
	...
	case FS3001_HAPTIC_VBAT_COMP_PLAY_GAIN_BRK_DISABLE:
		...
		sprintf(str,
			"%s, vbat mode = %d, vbat=%d, vbat_min=%d, vbat_ref=%d",
			__func__, fs3001->dts_info.fs3001_vbat_mode,
			fs3001->vbat, FS3001_VBAT_MIN, FS3001_VBAT_REFER);
		fs3001_debug_message(fs3001, str);
		...
		sprintf(str,
			"%s, enable vbat comp, level = %x comp level = %x\n",
	...
	case FS3001_HAPTIC_VBAT_COMP_PLAY_REG_BRK_ENABLE:
		...
		sprintf(str, "%s, vbat mode = %d, vbat=%d, VBD=%d\n", __func__,
			fs3001->dts_info.fs3001_vbat_mode, fs3001->vbat,
			uc_VBD);
		fs3001_debug_message(fs3001, str);
		if (uc_VBD > 0) {
			fs3001_i2c_write_bits_1(fs3001, FS3001_VCOMPCFG, uc_VBD,
						5, 0);
		}
		break;
	default:
...
//vbat mode
static int fs3001_haptic_vbat_mode_config(struct fs3001 *fs3001,
					  unsigned int mode)
{
	...
	default:
		pr_info("unsupported vbat mode (0x%x)\n", mode);
		fs3001->dts_info.fs3001_vbat_mode =
			FS3001_HAPTIC_VBAT_COMP_DISABLE;
		fs3001_i2c_write_bits_1(fs3001, FS3001_VCOMPCTRL, 0x00, 4, 0);
		break;
	}
...
static int fs3001_haptic_ram_vbat_compensate(struct fs3001 *fs3001, bool flag)
{
	...
	if (flag)
		fs3001_haptic_vbat_mode_config(
			fs3001, fs3001->dts_info.fs3001_vbat_mode);
	else
		fs3001_haptic_vbat_mode_config(fs3001,
					       FS3001_HAPTIC_VBAT_COMP_DISABLE);

	return 0;
}

static int fs3001_haptic_play_mode(struct fs3001 *fs3001,
...
static int fs3001_haptic_play_go(struct fs3001 *fs3001)
{
	...
	fs3001_i2c_write(
		fs3001, FS3001_OPCTRL,
		0x00); //Do a stop action regardless of whether the flag is false
	...
	fs3001_i2c_read(fs3001, FS3001_STATUS,
			&reg_val); //read 0x0 to ensure osc off
...
static int fs3001_haptic_set_wav_seq(struct fs3001 *fs3001, unsigned char wav,
				     unsigned char seq)
{
...
static int fs3001_haptic_set_wav_loop(struct fs3001 *fs3001, unsigned char wav,
				      unsigned char loop)
{
	...
	if (wav % 2 == 0) {
		fs3001_i2c_write_bits_1(fs3001, FS3001_WFSLOOP1 + (wav / 2),
					loop, 3, 0);
	} else {
		fs3001_i2c_write_bits_1(fs3001, FS3001_WFSLOOP1 + (wav / 2),
					loop, 7, 4);
...
static int fs3001_haptic_rtp_init(struct fs3001 *fs3001)
{
	...
	while ((!fs3001_haptic_rtp_get_fifo_afs_0xAF(fs3001)) &&
	       (fs3001->play_mode == FS3001_HAPTIC_RTP_MODE) &&
	       !atomic_read(&fs3001->exit_in_rtp_loop)) {
		...
		if (fs3001->rtp_cnt < (fs3001->ram.base_addr)) {
			if ((fs3001->rtp_container->len - fs3001->rtp_cnt) <
			    (fs3001->ram.base_addr)) {
				buf_len = fs3001->rtp_container->len -
					  fs3001->rtp_cnt;
			} else {
				buf_len = fs3001->ram.base_addr;
			}
		} else if ((fs3001->rtp_container->len - fs3001->rtp_cnt) <
			   (fs3001->ram.base_addr >> 2)) {
		...
		fs3001_i2c_writes(fs3001, FS3001_RTPWDATA,
				  &fs3001->rtp_container->data[fs3001->rtp_cnt],
				  buf_len);
		fs3001->rtp_cnt += buf_len;
		fs3001_i2c_read(fs3001, FS3001_DIGSTAT,
				&DIGSTAT_OPS); //DIGSTAT_OPS == 0x00  inactive
		DIGSTAT_OPS = DIGSTAT_OPS >> 4;
		if ((fs3001->rtp_cnt == fs3001->rtp_container->len) ||
		    (DIGSTAT_OPS == FS3001_DIGSTAT_B7_4_OPS_OFF)) {
			if (fs3001->rtp_cnt == fs3001->rtp_container->len)
	...
	if (fs3001->play_mode == FS3001_HAPTIC_RTP_MODE &&
	    !atomic_read(&fs3001->exit_in_rtp_loop))
...
static int fs3001_haptic_set_repeat_wav_seq(struct fs3001 *fs3001,
					    unsigned char seq)
{
...
static int fs3001_haptic_set_pgagain(struct fs3001 *fs3001,
...
static void fs3001_rtp_work_routine(struct work_struct *work)
{
	...
	sprintf(str,
		"%s,enter fs3001->effect_id=%d,fs3001->dts_info.fs3001_rtp_id_boundary=%d, fs3001->dts_info.fs3001_rtp_max=%d,fs3001->dts_info.fs3001_bypass_system_gain=%d\n",
	...
	while (atomic_read(&fs3001->is_in_rtp_loop)) {
		...
		ret = wait_event_interruptible(
			fs3001->wait_q,
			atomic_read(&fs3001->is_in_rtp_loop) == 0);
		pr_info("wakeup \n");
		if (ret == -ERESTARTSYS) {
	...
	if (fs3001->state) {
		...
		if (fs3001->rtp_file_num >
		    ((sizeof(fs3001_rtp_name) / FS3001_RTP_NAME_MAX) - 1))
			fs3001->rtp_file_num = (sizeof(fs3001_rtp_name) /
						FS3001_RTP_NAME_MAX) -
					       1;
		...
		ret = request_firmware(&rtp_file,
				       fs3001_rtp_name[fs3001->rtp_file_num],
				       fs3001->dev);
		if (ret < 0) {
		...
		sprintf(str, "%s,rtp file:[%s] size = %dbytes\n", __func__,
			fs3001_rtp_name[fs3001->rtp_file_num],
			fs3001->rtp_container->len);
		fs3001_debug_message(fs3001, str);

		memcpy(fs3001->rtp_container->data, rtp_file->data,
		       rtp_file->size);
		release_firmware(rtp_file);
		fs3001->rtp_init =
			1; //memcpy(fs3001->rtp_container->data, rtp_file->data, rtp_file->size);
		fs3001_haptic_upload_lra(fs3001, FS3001_OSC_CALI);
		fs3001_haptic_set_pwm(
			fs3001,
			fs3001->dts_info.fs3001_play_rtp_srate); //20210601
		//gain
		...
		if (fs3001->dts_info.fs3001_bypass_system_gain == 0) {
			...
			fs3001_haptic_set_gain(fs3001,
					       fs3001->level); //20210716
		}
		...
		while (cnt) {
			fs3001_i2c_read(fs3001, FS3001_DIGSTAT, &reg_val);
			if ((reg_val >> 4) ==
			    FS3001_DIGSTAT_B7_4_OPS_GO) //DIGSTAT_OPS == 0x20  go
			{
				cnt = 0;
				rtp_work_flag = true;
...
static int fs3001_rtp_osc_calibration(struct fs3001 *fs3001)
{
	...
	//haptic go
	fs3001_haptic_play_go(fs3001);
	fs3001_interrupt_clear(
		fs3001); //need to do it immediately, otherwise fs3001_haptic_rtp_get_fifo_afs will get wrong result
	...
	while (1) {
		...
		if (!fs3001_haptic_rtp_get_fifo_afs_0xAF(fs3001)) {
			...
			if (fs3001->rtp_cnt != fs3001->rtp_container->len) {
				...
				fs3001->rtp_update_flag = fs3001_i2c_writes(
					fs3001, FS3001_RTPWDATA,
					&fs3001->rtp_container
						 ->data[fs3001->rtp_cnt],
					buf_len);
				fs3001->rtp_cnt += buf_len;
			}
		...
		ktime_get_real_ts64(&fs3001->end);
		fs3001->microsecond =
			(fs3001->end.tv_sec - fs3001->start.tv_sec) * 1000000 +
			(fs3001->end.tv_nsec - fs3001->start.tv_nsec) / 1000000;
...
static int fs3001_osc_trim_calculation(unsigned long int theory_time,
				       unsigned long int real_time)
{
	...
	if (theory_time == real_time) {
		...
		lra_code =
			(100 * (real_time - theory_time) + 257064068) / 1776596;
	}
...
static int fs3001_haptic_get_lra_resistance(struct fs3001 *fs3001)
{
	...
	fs3001_i2c_write(
		fs3001, FS3001_SWDIAG1,
		0xFA); //0xB5 = 0xFA		// Turn of S2F, turn on RL detection current sink
	usleep_range(
		1000,
		2000); //usleep_range(100, 110); //Wait > 100 us(100);                                                           //Wait > 100 us

	ui_BCBD = 0;
	for (i = 0; i < FS3001_OFFSET_RETRIES; i++) {
		fs3001_i2c_write(
			fs3001, FS3001_SARCTRL,
			0x90); //0xBB = 0x90		// SAR capture data
		usleep_range(1000, 2000); //usleep_range(10, 20); //Wait > 10 us
		fs3001_i2c_read(fs3001, FS3001_SARTS_L,
				&uc_0xBC); //0xBC//low 8 offset value
		fs3001_i2c_read(fs3001, FS3001_SARTS_L,
				&uc_0xBC); //0xBC//low 8 offset value
		uc_0xBD = fs3001_i2c_read_bits(fs3001, FS3001_SARTS_H, 1,
					       0); //0xBD//high 2 offset value
	...
	fs3001->lra = (fs3001->offset - ui_BCBD) * 428600 /
		      (1024 * get_d2sgain(uc_0x2A));
	fs3001->lra =
		fs3001->lra *
		100; //need to * 100 (In get_d2sgain function, gain has been amplified by 100 times )
...
static int fs3001_haptic_juge_RTP_is_going_on(struct fs3001 *fs3001)
{
	...
	if (((mode & 0x3) == FS3001_SYSCTRL_B1_0_OPMODE_RTP) &&
	    ((glb_st >> 4) == FS3001_DIGSTAT_B7_4_OPS_GO)) {
		rtp_state = 1;
	}
	return rtp_state;
}

static int fs3001_container_update(struct fs3001 *fs3001,
				   struct fs3001_container *fs3001_cont)
{
	unsigned char reg_val = 0;
	...
	fs3001->ram.base_addr =
		(unsigned int)((fs3001_cont->data[0 + shift] << 8) |
			       (fs3001_cont->data[1 + shift]));

	//BASE_ADDRH  WFSBASE_H
	fs3001_i2c_write_bits_1(fs3001, FS3001_WFSBASE_H,
				fs3001_cont->data[0 + shift], 3, 0);

	//BASE_ADDRL  WFSBASE_L
	fs3001_i2c_write(fs3001, FS3001_WFSBASE_L,
			 fs3001_cont->data[1 + shift]);

	//FIFO_AEH  1/2 of fifo
	fs3001_i2c_write_bits_1(
		fs3001, FS3001_RTPFIFOAE_H,
		(unsigned char)(((fs3001->ram.base_addr >> 1) >> 8) & 0x0F), 3,
		0);
	//FIFO AEL
	fs3001_i2c_write(
		fs3001, FS3001_RTPFIFOAE_L,
		(unsigned char)(((fs3001->ram.base_addr >> 1) & 0xFF)));

	// FIFO_AFH  1/4 of fifo
	fs3001_i2c_write_bits_1(
		fs3001, FS3001_RTPFIFOAF_H,
		(unsigned char)(((fs3001->ram.base_addr -
				  (fs3001->ram.base_addr >> 2)) >>
				 8) &
				0x0F),
		3, 0);
	// FIFO_AFL
	fs3001_i2c_write(fs3001, FS3001_RTPFIFOAF_L,
			 (unsigned char)(((fs3001->ram.base_addr -
					   (fs3001->ram.base_addr >> 2)) &
					  0xFF)));
	...
	//RAMADDR_H
	fs3001_i2c_write_bits_1(fs3001, FS3001_RAMADDR_H,
				fs3001_cont->data[0 + shift], 3, 0);
	//RAMADDR_L
	fs3001_i2c_write(fs3001, FS3001_RAMADDR_L,
			 fs3001_cont->data[1 + shift]);

	shift = fs3001->ram.ram_shift;
	pr_info("ram_len = %d\n", fs3001_cont->len - shift);

	fs3001->ram_update_flag = fs3001_i2c_writes(fs3001, FS3001_RAMWDATA,
						    &fs3001_cont->data[shift],
						    fs3001_cont->len - shift);
...
#ifdef FS_READ_BIN_FLEXBALLY
		if (load_cont <= 20) {
			schedule_delayed_work(&fs3001->ram_work,
...
static int fs3001_ram_update(struct fs3001 *fs3001)
{
	...
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				       fs3001_ram_name, fs3001->dev, GFP_KERNEL,
				       fs3001, fs3001_ram_loaded);
}
...
static int fs3001_rtp_trim_lra_calibration(struct fs3001 *fs3001)
{
	...
	lra_trim_code =
		fs3001_osc_trim_calculation(theory_time, fs3001->microsecond);
	if (lra_trim_code >= 0) {
...
static int fs3001_haptic_play_repeat_seq(struct fs3001 *fs3001,
					 unsigned char flag)
{
	pr_info("enter\n");
...
//motor protect
static int fs3001_haptic_swicth_motor_protect_config(struct fs3001 *fs3001,
						     bool flag)
{
	pr_info("enter\n");
...
{
	unsigned int i_temp =
		FS3001_BASE_FRE * 10 / fs3001->dts_info.fs3001_f0_ref;
	pr_info("enter\n");
	fs3001_i2c_write(fs3001, FS3001_F0SET_L, i_temp & 0xFF);
	fs3001_i2c_write(fs3001, FS3001_F0SET_H, (i_temp >> 8) & 0xFF);
}

static unsigned int fs3001_haptic_cal_0x7B_value(long long ll_f0,
						 long long ll_f0_ref)
{
...
static int fs3001_haptic_f0_calibration(struct fs3001 *fs3001)
{
	...
	fs3001_i2c_write(fs3001, FS3001_FTUNECFG,
			 0x90); //cali will fail if  FTUNECFG == 0 or 0xff
	...
	} else if (fs3001->dts_info.fs3001_f0_cali_mode ==
		   FS3001_F0_CALI_MODE_FORMULA) {
		// enter DETECT_MODE
		fs3001_haptic_play_mode(fs3001, FS3001_HAPTIC_F0_DETECT_MODE);

		fs3001_haptic_cont_get_f0(fs3001); //get f0
		ui_0x7B = fs3001_haptic_cal_0x7B_value(
			fs3001->f0, fs3001->dts_info.fs3001_f0_ref);
		//save f0_cali_data
...
fs3001_haptic_brightness_get(struct led_classdev *cdev)
{
...
static void fs3001_haptic_brightness_set(struct led_classdev *cdev,
...
static int
fs3001_haptic_audio_ctr_list_insert(struct haptic_audio *haptic_audio,
...
static int fs3001_haptic_audio_ctr_list_clear(struct haptic_audio *haptic_audio)
{
...
static void fs3001_haptic_audio_work_routine(struct work_struct *work)
{
	struct fs3001 *fs3001 =
		container_of(work, struct fs3001, haptic_audio.work);
	...
	if (ctr_list_flag == 1) {
		...
		if (ctr_list_diff_cnt > 2) {
			list_for_each_entry_safe_reverse (
				...
				list) {
				if ((p_ctr->play == FS3001_HAPTIC_CMD_NULL) &&
				    (FS3001_HAPTIC_CMD_ENABLE ==
				     (FS3001_HAPTIC_CMD_HAPTIC & p_ctr->cmd))) {
					list_del(&p_ctr->list);
					kfree(p_ctr);
					ctr_list_del_cnt++;
				}
	...
	if (fs3001->haptic_audio.ctr.play) {
		pr_info("cnt=%d, cmd=%d, play=%d, wavseq=%d, loop=%d, gain=%d\n",
			...
			fs3001->haptic_audio.ctr.cmd,
			fs3001->haptic_audio.ctr.play,
			fs3001->haptic_audio.ctr.wavseq,
			fs3001->haptic_audio.ctr.loop,
			fs3001->haptic_audio.ctr.gain);
	}
	...
	//haptic play control
	if (FS3001_HAPTIC_CMD_ENABLE ==
	    (FS3001_HAPTIC_CMD_HAPTIC & fs3001->haptic_audio.ctr.cmd)) {
		if (FS3001_HAPTIC_PLAY_ENABLE ==
		    fs3001->haptic_audio.ctr.play) {
			...
			fs3001_haptic_stop(fs3001);
			fs3001_haptic_play_mode(fs3001, FS3001_HAPTIC_RAM_MODE);
			fs3001_haptic_set_wav_seq(
				fs3001, 0x00, fs3001->haptic_audio.ctr.wavseq);
			fs3001_haptic_set_wav_loop(
				fs3001, 0x00, fs3001->haptic_audio.ctr.loop);
			fs3001_haptic_set_gain(fs3001,
					       fs3001->haptic_audio.ctr.gain);
			fs3001_haptic_start(fs3001);
			mutex_unlock(&fs3001->lock);
		} else if (FS3001_HAPTIC_PLAY_STOP ==
			   fs3001->haptic_audio.ctr.play) {
			mutex_lock(&fs3001->lock);
			fs3001_haptic_stop(fs3001);
			mutex_unlock(&fs3001->lock);
		} else if (FS3001_HAPTIC_PLAY_GAIN ==
			   fs3001->haptic_audio.ctr.play) {
			mutex_lock(&fs3001->lock);
			fs3001_haptic_set_gain(fs3001,
					       fs3001->haptic_audio.ctr.gain);
...
// cat f0
// 1723
static ssize_t fs3001_f0_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
...
//cat cali
//2350
static ssize_t fs3001_cali_show(struct device *dev,
...
static ssize_t fs3001_cali_store(struct device *dev,
...
//cat f0_save
//f0_cali_data = 0x83
static ssize_t fs3001_f0_save_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
//cat cali_mode
//cali_mode = 0
static ssize_t fs3001_cali_mode_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
//cali_mode = 1
static ssize_t fs3001_cali_mode_store(struct device *dev,
...
//cat f0_save
//f0_cali_data = 0x90
static ssize_t fs3001_f0_save_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat osc_save
//osc_cali_data = 0x90
static ssize_t fs3001_osc_save_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
//cat osc_save
//osc_cali_data = 0x8F
static ssize_t fs3001_osc_save_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat f0_ref
//f0_ref = 2350
static ssize_t fs3001_f0_ref_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
//f0_ref = 2222
static ssize_t fs3001_f0_ref_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat osc_cali
//osc_cali_data = 0x90
static ssize_t fs3001_osc_cali_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_state_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_state_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	return count;
...
static ssize_t fs3001_activate_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	return snprintf(buf, PAGE_SIZE,
			"fs3001->activate_mode = %d fs3001->state=%d\n",
...
static ssize_t fs3001_activate_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_activate_mode_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_activate_mode_store(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_cont_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
// Make a cont go
//echo 1 > cont
static ssize_t fs3001_cont_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_rtp_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
static ssize_t fs3001_rtp_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	//fs3001_haptic_set_rtp_aei(fs3001, false);
	//fs3001_interrupt_clear(fs3001);
	if (val < (sizeof(fs3001_rtp_name) /
		   FS3001_RTP_NAME_MAX)) //xxxx_please refer to basic-51
	{
		fs3001->effect_id = val;
		pr_info("fs3001_rtp_name[%d]: %s\n", (val - 20),
			fs3001_rtp_name[val - 20]);
...
//cat register
static ssize_t fs3001_reg_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
static ssize_t fs3001_reg_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		fs3001_i2c_write(fs3001, (unsigned char)databuf[0],
...
static ssize_t fs3001_regs_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%x %x %x %x %x %x %x %x %x %x", &databuf[0],
		   &databuf[1], &databuf[2], &databuf[3], &databuf[4],
		   &databuf[5], &databuf[6], &databuf[7], &databuf[8],
		   &databuf[9]) == 10) {
		for (i = 0; i < ARRAY_SIZE(databuf); i++) {
			if (databuf[i] != 0xffff) {
				fs3001_i2c_write(
					fs3001,
					(unsigned char)((databuf[i] >> 8) &
							0xff),
					(unsigned char)(databuf[i] & 0xff));
			}
		}
	} else
...
static ssize_t fs3001_reg_inits_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
static ssize_t fs3001_duration_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ktime_t time_rem;
	...
	return snprintf(
		buf, PAGE_SIZE,
		"fs3001->timer remaining = %lld, fs3001->duration = %d\n",
...
//echo 1 > activate
static ssize_t fs3001_duration_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//seq7: 0x00
//seq8: 0x00
static ssize_t fs3001_seq_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	size_t count = 0;
...
static ssize_t fs3001_seq_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		fs3001_haptic_set_wav_seq(fs3001, (unsigned char)databuf[0],
					  fs3001->seq[databuf[0]]);
		mutex_unlock(&fs3001->lock);
	}
	return count;
}
...
//seq7_loop = 0x00
//seq8_loop = 0x00
static ssize_t fs3001_loop_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	size_t count = 0;
...
//seq7_loop = 0x00
//seq8_loop = 0x00
static ssize_t fs3001_loop_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		fs3001_haptic_set_wav_loop(fs3001, (unsigned char)databuf[0],
					   fs3001->loop[databuf[0]]);
		mutex_unlock(&fs3001->lock);
	}
	return count;
}
...
static ssize_t fs3001_index_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	unsigned char reg_val = 0;
...
//seq7_loop = 0x00
//seq8_loop = 0x00
static ssize_t fs3001_index_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat sram_size
//sram_size = 3K
static ssize_t fs3001_sram_size_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	pr_info("enter\n");
...
//cat gain
//0x80
static ssize_t fs3001_gain_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	unsigned char reg = 0;
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat gain
//0x7E
static ssize_t fs3001_gain_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//...
//...
static ssize_t fs3001_ram_update_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
	...
	fs3001_i2c_write_bits_1(fs3001, FS3001_RAMADDR_H,
				(unsigned char)(fs3001->ram.base_addr >> 8), 3,
				0);
	fs3001_i2c_write(fs3001, FS3001_RAMADDR_L,
			 (unsigned char)(fs3001->ram.base_addr & 0x00ff));
...
//echo 1 > ram_update
static ssize_t fs3001_ram_update_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat vbat_mode
//vbat_mode = 0
static ssize_t fs3001_vbat_mode_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
//cat vbat_mode
//vbat_mode = 1
static ssize_t fs3001_vbat_mode_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cont_drv1_lvl = 0x7F
//cont_drv2_lvl = 0x36
static ssize_t fs3001_cont_drv_lvl_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cont_drv1_lvl = 0x7F
//cont_drv2_lvl = 0x44
static ssize_t fs3001_cont_drv_lvl_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		fs3001_i2c_write(fs3001, FS3001_DRVCFG1,
				 fs3001->dts_info.fs3001_cont_drv1_lvl);
		fs3001_i2c_write(fs3001, FS3001_DRVCFG2,
				 fs3001->dts_info.fs3001_cont_drv2_lvl);
	}
	return count;
}
...
//cont_drv1_lvl = 0x04
//cont_drv2_time = 0x14
static ssize_t fs3001_cont_drv_time_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cont_drv1_time = 0x04
//cont_drv2_time = 0x06
static ssize_t fs3001_cont_drv_time_store(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		fs3001_i2c_write(fs3001, FS3001_DRVCFG3,
				 fs3001->dts_info.fs3001_cont_drv1_time);
		fs3001_i2c_write(fs3001, FS3001_DRVCFG4,
				 fs3001->dts_info.fs3001_cont_drv2_time);
	}
	return count;
}
...
//cat cont_brk_times
//cont_brk_times = 0x06
static ssize_t fs3001_brk_times_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
//echo 5 > brk_times
//brk_times = 0x05
static ssize_t fs3001_brk_times_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%x", &databuf[0]) == 1) {
		...
		fs3001_i2c_write(fs3001, FS3001_BRKCFG5,
				 fs3001->dts_info.fs3001_brk_times);
	}
	return count;
}
...
//cat vbat_monitor
//vbat_monitor = 3433
static ssize_t fs3001_vbat_monitor_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat lra_resistance
//lra_resistance = 226
static ssize_t fs3001_lra_resistance_show(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (fs3001->dts_info.fs3001_lr_pgagain !=
	    fs3001->dts_info.fs3001_brk_pgagain) {
		fs3001_haptic_set_pgagain(fs3001,
					  fs3001->dts_info.fs3001_lr_pgagain);
...
//cat gun_type
//0xff
static ssize_t fs3001_gun_type_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	pr_info("enter\n");
...
//cat gun_type
//0xfe
static ssize_t fs3001_gun_type_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat version
//v0.1.0.2
static ssize_t fs3001_version_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	pr_info("enter\n");
...
//cat bullet_nr
//0x00
static ssize_t fs3001_bullet_nr_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	pr_info("enter\n");
...
//cat bullet_nr
//0x01
static ssize_t fs3001_bullet_nr_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//cat haptic_audio
//0
static ssize_t fs3001_haptic_audio_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//echo 1 1 1 1 0
static ssize_t fs3001_haptic_audio_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%d %d %d %d %d %d", &databuf[0], &databuf[1],
		...
		if (databuf[2] != FS3001_HAPTIC_CMD_NULL) {
			...
			fs3001_haptic_audio_ctr_list_insert(
				&fs3001->haptic_audio, hap_ctr, fs3001->dev);
			if (hap_ctr->cmd == 0xff) {
				pr_info("haptic_audio stop\n");
				if (hrtimer_active(
					    &fs3001->haptic_audio.timer)) {
...
static ssize_t fs3001_bypass_system_gain_show(struct device *dev,
					      struct device_attribute *attr,
					      char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_bypass_system_gain_store(struct device *dev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
//reset the chip
//echo 1 > reset
static ssize_t fs3001_reset_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	unsigned int val = 0;
...
static ssize_t fs3001_debug_enable_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_debug_enable_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	...
	if (sscanf(buf, "%x", &databuf[0]) == 1) {
		...
		if (databuf[0] != 0 && databuf[0] != 1) {
			pr_err("1:input value=%d out of range\n", databuf[0]);
			return count;
		}
...
static ssize_t fs3001_effect_id_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
static ssize_t fs3001_effect_id_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_buf_size_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
static ssize_t fs3001_buf_size_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_Qos_time_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
static ssize_t fs3001_Qos_time_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static ssize_t fs3001_auto_brake_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
	ssize_t len = 0;
...
static ssize_t fs3001_auto_brake_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	struct fs3001 *fs3001 = g_foursemi->fs3001;
...
static DEVICE_ATTR(activate_mode, 0644, fs3001_activate_mode_show,
		   fs3001_activate_mode_store);
...
static DEVICE_ATTR(ram_update, 0644, fs3001_ram_update_show,
		   fs3001_ram_update_store);
...
static DEVICE_ATTR(vbat_mode, 0644, fs3001_vbat_mode_show,
		   fs3001_vbat_mode_store);
static DEVICE_ATTR(cali, 0644, fs3001_cali_show, fs3001_cali_store);
static DEVICE_ATTR(cali_mode, 0644, fs3001_cali_mode_show,
		   fs3001_cali_mode_store);
static DEVICE_ATTR(cont_drv_lvl, 0644, fs3001_cont_drv_lvl_show,
		   fs3001_cont_drv_lvl_store);
static DEVICE_ATTR(cont_drv_time, 0644, fs3001_cont_drv_time_show,
		   fs3001_cont_drv_time_store);
static DEVICE_ATTR(brk_times, 0644, fs3001_brk_times_show,
		   fs3001_brk_times_store);
...
static DEVICE_ATTR(bullet_nr, 0644, fs3001_bullet_nr_show,
		   fs3001_bullet_nr_store);
static DEVICE_ATTR(haptic_audio, 0644, fs3001_haptic_audio_show,
		   fs3001_haptic_audio_store);
static DEVICE_ATTR(bypass_system_gain, 0644, fs3001_bypass_system_gain_show,
		   fs3001_bypass_system_gain_store);
...
static DEVICE_ATTR(debug_enable, 0644, fs3001_debug_enable_show,
		   fs3001_debug_enable_store);
static DEVICE_ATTR(effect_id, 0644, fs3001_effect_id_show,
		   fs3001_effect_id_store);
...
static DEVICE_ATTR(auto_brake, 0644, fs3001_auto_brake_show,
		   fs3001_auto_brake_store);
...
static int fs3001_haptic_play_effect_seq(struct fs3001 *fs3001,
					 unsigned char flag)
{
	sprintf(str,
		"%s, enter flag = %d,fs3001->effect_id =%d,fs3001->dts_info.fs3001_rtp_id_boundary = %d,fs3001->activate_mode =%d,fs3001->dts_info.fs3001_bypass_system_gain=%d\n",
		__func__, flag, fs3001->effect_id,
		fs3001->dts_info.fs3001_rtp_id_boundary, fs3001->activate_mode,
		fs3001->dts_info.fs3001_bypass_system_gain);
	fs3001_debug_message(fs3001, str);
	...
	if (flag) {
		if (fs3001->activate_mode == FS3001_HAPTIC_ACTIVATE_RAM_MODE) {
			if ((fs3001->effect_id == 5) ||
			    (fs3001->effect_id == 6)) {
				fs3001->effect_id = 6;
			} else if (fs3001->effect_id == 1) {
				fs3001->effect_id = 3;
			} else if ((fs3001->effect_id >= 2) &&
				   (fs3001->effect_id <= 9)) {
				fs3001->effect_id = 8;
			}

			fs3001_haptic_set_wav_seq(fs3001, 0x00,
						  (char)fs3001->effect_id +
							  1); //set seq0
			fs3001_haptic_set_pwm(
				fs3001,
				fs3001->dts_info
					.fs3001_play_ram_srate); //20210601
			fs3001_haptic_set_wav_seq(fs3001, 0x01,
						  0x00); //set seq1 stop
		...
		if (fs3001->activate_mode ==
		    FS3001_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			...
			fs3001_haptic_set_repeat_wav_seq(
				fs3001,
				(fs3001->dts_info.fs3001_rtp_id_boundary + 1));
			fs3001_haptic_set_pwm(
				fs3001,
				fs3001->dts_info
					.fs3001_play_ram_srate); //20210601
...
{
	struct fs3001 *fs3001 =
		container_of(work, struct fs3001, long_vibrate_work);

	sprintf(str, "%s, enter,state=%d activate_mode = %d duration = %d\n",
		__func__, fs3001->state, fs3001->activate_mode,
		fs3001->duration);
	fs3001_debug_message(fs3001, str);
	...
	if (fs3001->state) {
		if (fs3001->activate_mode == FS3001_HAPTIC_ACTIVATE_RAM_MODE) {
			fs3001_haptic_ram_vbat_compensate(fs3001,
							  false); //20210716
			//fs3001_haptic_play_repeat_seq(fs3001, true);
			fs3001_haptic_play_effect_seq(fs3001, true);
		} else if (fs3001->activate_mode ==
			   FS3001_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			...
			hrtimer_start(
				&fs3001->timer,
				ktime_set(fs3001->duration / 1000,
					  (fs3001->duration % 1000) * 1000000),
				HRTIMER_MODE_REL);
			pm_stay_awake(fs3001->dev);
			fs3001->wk_lock_flag = 1;
		} else if (fs3001->activate_mode ==
...
static int fs3001_hapstream_i2c_writes(struct fs3001 *fs3001,
				       struct mmap_buf_format *hapstream_buf)
{
	int ret = -1, i = 0, len = 0;
	char str[200] = { 0 };
	...
	ret = i2c_master_send(fs3001->i2c, &(hapstream_buf->reg_addr),
			      hapstream_buf->length + 1);
...
static inline unsigned int fs3001_get_sys_msecs(
	void) //static inline unsigned int fs3001_get_sys_msecs() -- transsion platform
{
...
static void fs3001_rtp_work_hapstream(struct work_struct *work)
{
	...
	while (true) {
		...
		if (((reg_val_00 & 0x01) != 0x01) &&
		    ((reg_val_11 & 0x03) != 0x01)) {
		...
		if (reg_val_00 & 0x00 ||
		    ...
		    (hapstream_buf->status == MMAP_BUF_DATA_INVALID)) {
			pr_info("hapstream break_2, reg_00 = 0x%02X, hapstream_stop_flag = %d, hapstream_buf->status = 0x%02X\n",
				reg_val_00, fs3001->hapstream_stop_flag,
				hapstream_buf->status);
			break;
		} else if (hapstream_buf->status == MMAP_BUF_DATA_VALID &&
			   (reg_val_AF & 0x01)) {
			pr_info("buf_cnt = %d, bit = %d, length = %d!\n",
				buf_cnt, hapstream_buf->bit,
				hapstream_buf->length);
...
static long fs3001_hapstream_unlocked_ioctl(struct file *file, unsigned int cmd,
					    unsigned long arg)
{
...
static ssize_t fs3001_buf_write_proc(struct file *filp,
...
static int fs3001_file_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	/* N6 code for HQ-347095 by p-dongfeiju1 at 2023/11/14 start */
	if (!vma) {
		return -EINVAL;
	}
	/* N6 code for HQ-347095 by p-dongfeiju1 at 2023/11/14 end */
	...
	if (vma && ((vma->vm_end - vma->vm_start) !=
		    (PAGE_SIZE << HAPSTREAM_MMAP_PAGE_ORDER))) {
...
int fs3001_vibrator_init(struct fs3001 *fs3001)
{
	...
	ret = sysfs_create_group(&fs3001->i2c->dev.kobj,
				 &fs3001_vibrator_attribute_group);
	if (ret < 0) {
	...
	ret = sysfs_create_link(&fs3001->vib_dev.dev->kobj,
				&fs3001->i2c->dev.kobj, "vibrator");
	if (ret < 0) {
	...
	fs3001->fs_config_proc =
		proc_create(FS_HAPSTREAM_NAME, 0666, NULL, &config_proc_ops);
	...
	fs3001->start_buf = (struct mmap_buf_format *)__get_free_pages(
		GFP_KERNEL, HAPSTREAM_MMAP_PAGE_ORDER);
...
static void fs3001_hack_init(struct fs3001 *fs3001)
{
	...
	if (fs3001->name == FS3001_A1) {
		...
		fs3001_i2c_write_bits_1(
			fs3001, FS3001_ANACTRL, 1, 6,
			6); //Just in case of special circumstances, so enable it
		fs3001_i2c_write(fs3001, FS3001_OTPPG0W2B4,
				 0x00); //for A1 short
		fs3001_i2c_write_bits_1(fs3001, FS3001_ANACTRL, 0, 6, 6);
...
{
	fs3001_haptic_enable_key(fs3001, true); //enable key
	fs3001_i2c_write_bits_1(fs3001, FS3001_INTCTRL, 0, 7,
				7); // Disable interrupt output
	fs3001_i2c_write(fs3001, FS3001_ANACTRL,
			 0x40); //0xC0 = 0x40		// Enable OSC
	fs3001_i2c_write(
		fs3001, FS3001_SWDIAG1,
		0xC0); //0xB5 = 0xC0		// Enable Software diagnose mode and VCM buffer
	usleep_range(1000, 2000); //usleep_range(10, 20); //Wait > 5 us
	fs3001_i2c_write(
		fs3001, FS3001_SWDIAG1,
		0xD2); //0xB5 = 0xD2		// Turn on S1FN & S1FP & PGA
	usleep_range(
		1000,
		2000); //usleep_range(100, 150);//Wait > 100 us														 //Wait > 100 us  ÕâÀï¸ø100ms
}
...
fs3001_haptic_set_diagnostic_reg_to_default_value(struct fs3001 *fs3001)
{
	...
	fs3001_i2c_write_bits_1(fs3001, FS3001_INTCTRL, 1, 7,
				7); // enable interrupt output
	fs3001_haptic_enable_key(
		fs3001,
		false); //disable key													 //Wait > 100 us  ÕâÀï¸ø100ms
}
...
static int fs3001_haptic_offset_calibration(struct fs3001 *fs3001)
{
	...
	//outp
	fs3001_i2c_write(fs3001, FS3001_SARCTRL,
			 0x92); //0xBB = 0x92		// SAR capture data
	usleep_range(1000, 2000); //usleep_range(10, 20); //Wait > 5 us
	fs3001_i2c_read(fs3001, FS3001_SARTS_L,
			&uc_0xBC); //0xBC//low 8 offset value
	fs3001_i2c_read(fs3001, FS3001_SARTS_L,
	...
	if (100 <= ui_BCBD &&
	    ui_BCBD <=
		    350) //Check if SAR_OUTN_READING ¡Ê[100, 350] (TBD, 0.58V ~2.05V).
	{
		pr_info("outp = %d\n", ui_BCBD);
	} else {
	...
	//outn
	fs3001_i2c_write(fs3001, FS3001_SARCTRL,
			 0x93); //0xBB = 0x93		// SAR capture data
	usleep_range(1000, 2000); //usleep_range(10, 20); //Wait > 5 us
	fs3001_i2c_read(fs3001, FS3001_SARTS_L,
			&uc_0xBC); //0xBC//low 8 offset value
	fs3001_i2c_read(fs3001, FS3001_SARTS_L,
	...
	//offset
	fs3001_i2c_write(fs3001, FS3001_SWDIAG1,
			 0xDE); //0xB5 = 0xDE		// Turn on S2F
	usleep_range(1000, 2000); //usleep_range(10, 20); //Wait > 5 us
	ui_BCBD = 0;
	for (i = 0; i < FS3001_OFFSET_RETRIES; i++) {
		fs3001_i2c_write(
	...
	if (ui_BCBD > (512 + 6) || ui_BCBD < (512 - 6)) {
		...
		fs3001_i2c_write_bits_1(fs3001, FS3001_DIAGLRA_H,
					(ui_BCBD >> 8) & 0x3, 1, 0);
	}
...
{
	struct fs3001 *fs3001 =
		container_of(work, struct fs3001, ram_work.work);
...
fs3001_haptic_audio_timer_func(struct hrtimer *timer)
{
	struct fs3001 *fs3001 =
		container_of(timer, struct fs3001, haptic_audio.timer);
	...
	hrtimer_start(
		&fs3001->haptic_audio.timer,
		ktime_set(fs3001->haptic_audio.timer_val / 1000000,
			  (fs3001->haptic_audio.timer_val % 1000000) * 1000),
		HRTIMER_MODE_REL);
	return HRTIMER_NORESTART;
}

static void fs3001_haptic_auto_brk_enable(struct fs3001 *fs3001,
					  unsigned int flag)
{
	pr_info("enter, flag = %d\n", flag);

	fs3001_i2c_write_bits_1(fs3001, FS3001_BRKCTRL, (unsigned char)flag, 7,
				7);
}
...
void fs3001_reg_init(struct fs3001 *fs3001)
{
	...
	for (i = 0; i < ARRAY_SIZE(fs3001->dts_info.fs3001_reg_inits); i++) {
		if (fs3001->dts_info.fs3001_reg_inits[i] != 0xffffff) {
			uc_reg_addr =
				(fs3001->dts_info.fs3001_reg_inits[i] >> 16) &
				0xff;
			uc_reg_val =
				fs3001->dts_info.fs3001_reg_inits[i] & 0xff;
			uc_end = (fs3001->dts_info.fs3001_reg_inits[i] >> 8) &
				 0xf;
			uc_start =
				((fs3001->dts_info.fs3001_reg_inits[i] >> 12) &
				 0xf) +
				uc_end;

			fs3001_i2c_write_bits_1(fs3001, uc_reg_addr, uc_reg_val,
						uc_start, uc_end);
		}
	}
	mutex_unlock(&fs3001->lock);
...
void fs3001_f0_cali_setting_init(struct fs3001 *fs3001)
{
	...
	fs3001_i2c_write(fs3001, FS3001_DRVCFG4,
			 fs3001->dts_info.fs3001_cont_drv2_time);
	//  cont_1_period
	fs3001_i2c_write(fs3001, FS3001_TRKCFG2,
			 fs3001->dts_info.fs3001_cont_1_period & 0xff);
	fs3001_i2c_write_bits_1(fs3001, FS3001_TRKCFG3,
				fs3001->dts_info.fs3001_cont_1_period >> 8, 4,
				0);
	//	brk_slopeth
	fs3001_i2c_write(fs3001, FS3001_BRKCFG1,
			 fs3001->dts_info.fs3001_brk_slopeth & 0xff);
	//	brk_gain
	fs3001_i2c_write(fs3001, FS3001_BRKCFG4,
			 fs3001->dts_info.fs3001_brk_gain & 0xff);
	//	brk_times
	fs3001_i2c_write_bits_1(fs3001, FS3001_BRKCFG5,
				fs3001->dts_info.fs3001_brk_times, 4, 0);
	//	brk_noise_gate
	fs3001_i2c_write(fs3001, FS3001_BEMFDCFG4,
			 fs3001->dts_info.fs3001_brk_noise_gate & 0xff);
	//	brk_1_period
	fs3001_i2c_write(fs3001, FS3001_TRKCFG6,
			 fs3001->dts_info.fs3001_brk_1_period & 0xff);
	fs3001_i2c_write_bits_1(fs3001, FS3001_TRKCFG7,
				fs3001->dts_info.fs3001_brk_1_period >> 8, 4,
				0);
	//	brk_pgagain
	fs3001_haptic_set_pgagain(fs3001, fs3001->dts_info.fs3001_brk_pgagain);
	//	brk_margin
	fs3001_i2c_write(fs3001, FS3001_TRKCFG4,
			 fs3001->dts_info.fs3001_brk_margin & 0xff);
...
int fs3001_haptic_init(struct fs3001 *fs3001)
{
	...
	hrtimer_init(&fs3001->haptic_audio.timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
	...
	for (i = 0; i < FS3001_SEQUENCER_SIZE; i++) {
		reg_val =
			fs3001_i2c_read_bits(fs3001, FS3001_WFSCFG1 + i, 6, 0);
		fs3001->seq[i] = reg_val;
	}
	...
	fs3001_haptic_offset_calibration(fs3001);
	//config auto_brake
	fs3001_haptic_auto_brk_enable(fs3001,
				      fs3001->dts_info.fs3001_auto_brake);

	//f0 calibration
	if (fs3001->dts_info.fs3001_f0_cali_data_mode ==
	    FS3001_F0_CALI_DATA_SELF_MODE) {
		fs3001_haptic_f0_calibration(fs3001);
	}
	/*zzzz
	...
	*/
	else if (fs3001->dts_info.fs3001_f0_cali_data_mode ==
		 FS3001_F0_CALI_DATA_DTS_MODE) {
	...
	if (reg_val & FS3001_INTSTAT2_MASK_B1_AE) {
		...
		if (fs3001->rtp_init) {
			while ((!fs3001_haptic_rtp_get_fifo_afs_0xAF(fs3001)) &&
				...
				sprintf(str,
					"%s,fs3001 rtp mode fifo update, cnt=%d\n",
					__func__, fs3001->rtp_cnt);
				fs3001_debug_message(fs3001, str);
				...
				DIGSTAT_OPS = DIGSTAT_OPS >>
					      4; //get 0x4 high 4 bits(7-4)
				if ((fs3001->rtp_cnt ==
				     fs3001->rtp_container->len) ||
				    ((DIGSTAT_OPS & 0x0f) == 0)) {
					if (fs3001->rtp_cnt ==
					...
					fs3001_haptic_raminit(fs3001, false);
					fs3001_haptic_set_rtp_aei(fs3001,
								  false);
					fs3001->rtp_cnt = 0;
					fs3001->rtp_init = 0;
					mutex_unlock(&fs3001->rtp_lock);
...
int fs3001_haptics_upload_effect(struct input_dev *dev,
				 ...
				 struct ff_effect *old)
{
	...
	sprintf(str,
		"%s,enter effect->type=0x%x,FF_CONSTANT=0x%x,FF_PERIODIC=0x%x\n",
	...
	while (atomic_read(&fs3001->exit_in_rtp_loop)) {
		...
		ret = wait_event_interruptible(
			fs3001->stop_wait_q,
			atomic_read(&fs3001->exit_in_rtp_loop) == 0);
	...
	if (fs3001->effect_type == FF_CONSTANT) {
		...
		fs3001->effect_id =
			data[0]; //yyyy_get effect_id from data[0] as effect_type is FF_PERIODIC
		...
		fs3001->play.vmax_mv =
			effect->u.periodic.magnitude; //vmax level

		if (fs3001->effect_id < 0 ||
		    fs3001->effect_id > fs3001->dts_info.fs3001_rtp_max) {
		...
		if (fs3001->effect_id <
			...
			sprintf(str,
				"%s,fs3001->effect_id=%d , fs3001->activate_mode = %d\n",
				__func__, fs3001->effect_id,
				fs3001->activate_mode);
			fs3001_debug_message(fs3001, str);
			data[1] = fs3001->predefined[fs3001->effect_id]
					  .play_rate_us /
				  1000000; //second data
			data[2] = fs3001->predefined[fs3001->effect_id]
					  .play_rate_us /
				  1000; //millisecond data
			sprintf(str,
				"%s,fs3001->predefined[fs3001->effect_id].play_rate_us/1000 = %d\n",
				__func__,
				fs3001->predefined[fs3001->effect_id]
						.play_rate_us /
					1000);
			fs3001_debug_message(fs3001, str);
		}
		...
		if (fs3001->effect_id >=
			...
			sprintf(str,
				...
				fs3001->activate_mode);
			fs3001_debug_message(fs3001, str);
			data[1] = fs3001->dts_info
					  .fs3001_rtp_time[fs3001->effect_id] /
				  1000; //second data
/* N6 code for HQ-352720 by p-dongfeiju at 2023/12/11 start */
			data[2] = fs3001->dts_info.fs3001_rtp_time[fs3001->effect_id] % 1000; //millisecond data
/* N6 code for HQ-352720 by p-dongfeiju at 2023/12/11 end */
	...
	sprintf(str,
		"%s,before mutex_unlock(&fs3001->lock)     fs3001->effect_type= 0x%x\n",
...
int fs3001_haptics_playback(struct input_dev *dev, int effect_id, int val)
{
	...
	sprintf(str,
		"%s,enter: fs3001->effect_id=%d , fs3001->activate_mode = %d, val=%d, duration=%d\n",
		__func__, fs3001->effect_id, fs3001->activate_mode, val,
		fs3001->duration);
	fs3001_debug_message(fs3001, str);
...
{
	struct fs3001 *fs3001 =
		container_of(work, struct fs3001, set_gain_work);
