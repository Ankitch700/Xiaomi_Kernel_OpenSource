	...
	for (i = 0; i < ARRAY_SIZE(awinic->pinctrl_state); i++) {
		...
		if (!strncmp(n, name, strlen(n))) {
			...
			if (rc)
				...
				aw_dev_err(awinic->dev, "%s: Selected '%s'\n",
					   __func__, name);
			goto exit;
		}
	}
...
static int awinic_read_chipid(struct awinic *awinic, unsigned char *reg,
			      unsigned char type)
{
	int ret = -1;
	unsigned char cnt = 0;

	aw_dev_info(awinic->dev, "%s: awinic i2c addr = 0x%02x", __func__,
		    awinic->i2c->addr);
	while (cnt < AW_I2C_RETRIES) {
		...
		if (ret < 0) {
			if (type == AW_FIRST_TRY) {
				...
				aw_dev_err(awinic->dev, "%s: type is error\n",
					   __func__);
			}
		} else {
			*reg = ret;
...
static int awinic_parse_chipid(struct awinic *awinic)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		switch (reg) {
		case AW8624_CHIP_ID:
			aw_dev_info(awinic->dev, "%s aw8624 detected\n",
				    __func__);
			awinic->name = AW8624;
			   ...
			   struct device_node *np)
{
	...
	if (awinic->reset_gpio >= 0) {
		aw_dev_err(awinic->dev, "%s: reset gpio provided ok\n",
			   __func__);
	} else {
		awinic->reset_gpio = -1;
		aw_dev_err(
	...
	if (awinic->irq_gpio < 0) {
		...
		aw_dev_err(awinic->dev, "%s: irq gpio provided ok.\n",
			   __func__);
		awinic->IsUsedIRQ = true;
	}

	val = of_property_read_u32(np, "aw8622x_i2c_addr",
				   &awinic->aw8622x_i2c_addr);
	if (val)
		aw_dev_err(awinic->dev, "%s:configure aw8622x_i2c_addr error\n",
			   __func__);
	else
		aw_dev_info(awinic->dev, "%s: configure aw8622x_i2c_addr ok\n",
			    __func__);
	return 0;
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (awinic->name == AW8624) {
		awinic->aw8624 = devm_kzalloc(&i2c->dev, sizeof(struct aw8624),
					      GFP_KERNEL);
		...
		if (gpio_is_valid(awinic->aw8624->irq_gpio) &&
			...
			ret = devm_request_threaded_irq(
				&i2c->dev,
				gpio_to_irq(awinic->aw8624->irq_gpio), NULL,
				aw8624_irq, irq_flags, "aw8624",
				awinic->aw8624);
		...
		if (rc < 0) {
			dev_err(awinic->aw8624->dev,
				"create FF input device failed, rc=%d\n", rc);
			return rc;
		}
		awinic->aw8624->work_queue = create_singlethread_workqueue(
			"aw8624_vibrator_work_queue");
		...
		INIT_WORK(&awinic->aw8624->set_gain_work,
			  aw8624_haptics_set_gain_work_routine);
		ff = input_dev->ff;
		...
		if (rc < 0) {
			dev_err(awinic->aw8624->dev,
				"register input device failed, rc=%d\n", rc);
			goto aw8624_destroy_ff;
		}

	} else if (awinic->name == AW86223 || awinic->name == AW86224_5) {
		awinic->aw8622x = devm_kzalloc(
			&i2c->dev, sizeof(struct aw8622x), GFP_KERNEL);
		...
		if (gpio_is_valid(awinic->aw8622x->irq_gpio) &&
			...
			ret = devm_request_threaded_irq(
				&i2c->dev,
				gpio_to_irq(awinic->aw8622x->irq_gpio), NULL,
				aw8622x_irq, irq_flags, "aw8622x",
				awinic->aw8622x);
			if (ret != 0) {
				aw_dev_err(
					&i2c->dev,
			...
			aw_dev_err(&i2c->dev, "%s skipping IRQ registration\n",
				   __func__);
			/* disable feature support if gpio was invalid */
			awinic->aw8622x->flags |= AW8622X_FLAG_SKIP_INTERRUPTS;
		}
		...
		rc = input_ff_create(input_dev, effect_count_max);
		if (rc < 0) {
			dev_err(awinic->aw8622x->dev,
				"create FF input device failed, rc=%d\n", rc);
			return rc;
		}
		awinic->aw8622x->work_queue = create_singlethread_workqueue(
			"aw8622x_vibrator_work_queue");
...
static const struct i2c_device_id awinic_i2c_id[] = { { AWINIC_I2C_NAME, 0 },
						      {} };
