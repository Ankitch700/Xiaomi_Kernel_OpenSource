			    ...
			    unsigned char reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(aw8624->i2c, reg_addr,
						reg_data);
		if (ret < 0) {
...
static int aw8624_i2c_writes(struct aw8624 *aw8624, unsigned char reg_addr,
...
int aw8624_ram_init(struct aw8624 *aw8624)
{
	...
	if (check_sum != (unsigned short)((aw8624_ram_data[0] << 8) |
					  (aw8624_ram_data[1]))) {
		pr_err("%s: check sum err: check_sum=0x%04x\n", __func__,
		       check_sum);
		return -1;
				   ...
				   unsigned char play_mode)
{
	...
	case AW8624_HAPTIC_CONT_MODE:
		...
		break;
	default:
		dev_err(aw8624->dev, "%s: play mode %d err", __func__,
			play_mode);
		break;
	}
	...
	} else if (!aw8624->irq_sts_flag &&
		   (play_mode != AW8624_HAPTIC_STANDBY_MODE)) {
...
static int aw8624_haptic_play_go(struct aw8624 *aw8624, bool flag)
{
	...
	if (flag == true) {
		aw8624_i2c_write_bits(aw8624, AW8624_REG_GO, AW8624_BIT_GO_MASK,
...
static int aw8624_haptic_set_wav_seq(struct aw8624 *aw8624, unsigned char wav,
				     unsigned char seq)
{
...
static int aw8624_haptic_set_wav_loop(struct aw8624 *aw8624, unsigned char wav,
				      unsigned char loop)
{
	unsigned char tmp = 0;
...
static int aw8624_haptic_set_gain(struct aw8624 *aw8624, unsigned char gain)
{
	...
	if (aw8624->ram_vbat_comp == AW8624_HAPTIC_RAM_VBAT_COMP_ENABLE) {
		...
		pr_debug(
			...
			__func__, aw8624->vbat, AW8624_VBAT_MIN,
			AW8624_VBAT_REFER);
					 ...
					 unsigned char flag)
{
	...
	if (flag) {
		...
		if (aw8624->activate_mode ==
			...
			aw8624_haptic_set_repeat_wav_seq(
				aw8624, (aw8624->info.effect_id_boundary + 1));
			aw8624_haptic_effect_strength(aw8624);
			aw8624_haptic_set_gain(aw8624, aw8624->level);
					     ...
					     unsigned long int real_time)
{
	...
	if (theory_time == real_time) {
		pr_info("aw_osctheory_time == real_time:%ld  theory_time = %ld not need to "
			"cali\n",
		...
		if ((theory_time - real_time) > (theory_time / 50)) {
			pr_info("aw_osc((theory_time - real_time) > (theory_time / 50)) not to "
				"cali\n");
			return DFT_LRA_TRIM_CODE;
		}
		if ((theory_time - real_time) <
...
static void aw8624_rtp_work_routine(struct work_struct *work)
{
	...
	if (aw8624->state) {
		...
		if (aw8624->rtp_file_num >
			...
			aw8624->rtp_file_num = (sizeof(aw8624_rtp_name) /
						AW8624_RTP_NAME_MAX) -
					       1;
...
aw8624_haptic_audio_timer_func(struct hrtimer *timer)
{
	...
	hrtimer_start(
		&aw8624->haptic_audio.timer,
		ktime_set(aw8624->haptic_audio.timer_val / 1000000,
			  (aw8624->haptic_audio.timer_val % 1000000) * 1000),
		HRTIMER_MODE_REL);
...
static void aw8624_haptic_audio_work_routine(struct work_struct *work)
{
	...
	pr_debug("%s: cnt=%d, cmd=%d, play=%d, wavseq=%d, loop=%d, gain=%d\n",
		 __func__, aw8624->haptic_audio.cnt,
		 aw8624->haptic_audio.ctr.cmd, aw8624->haptic_audio.ctr.play,
		 aw8624->haptic_audio.ctr.wavseq, aw8624->haptic_audio.ctr.loop,
		 aw8624->haptic_audio.ctr.gain);
	mutex_unlock(&aw8624->haptic_audio.lock);
	if (AW8624_HAPTIC_CMD_ENABLE == aw8624->haptic_audio.ctr.cmd) {
		if (AW8624_HAPTIC_PLAY_ENABLE ==
			...
			aw8624_haptic_set_wav_seq(
				aw8624, 0x00, aw8624->haptic_audio.ctr.wavseq);
			aw8624_haptic_set_wav_seq(aw8624, 0x01, 0x00);

			aw8624_haptic_set_wav_loop(
				aw8624, 0x00, aw8624->haptic_audio.ctr.loop);
		...
		mutex_lock(&aw8624->haptic_audio.lock);
		memset(&aw8624->haptic_audio.data[aw8624->haptic_audio.cnt], 0,
		       sizeof(struct haptic_ctr));
		mutex_unlock(&aw8624->haptic_audio.lock);
	}
...
static int aw8624_haptic_f0_calibration(struct aw8624 *aw8624)
{
	...
	if (aw8624_haptic_get_f0(aw8624)) {
		...
		/* calculate cali step */
		f0_cali_step = 100000 *
			       ((int)f0_limit - (int)aw8624->info.f0_pre) /
			       ((int)f0_limit * 25);
...
{
	struct aw8624 *aw8624 = container_of(timer, struct aw8624, stop_timer);
	int rc;

	pr_info("%s: enter\n", __func__);
	aw8624->play.length_us = 0;
	rc = aw8624_haptic_play_go(aw8624,
				   false); // qti_haptics_play(aw8624, false);
	if (rc < 0)
...
{
	struct aw8624 *aw8624 =
		container_of(timer, struct aw8624, hap_disable_timer);
	int rc;

	pr_info("%s: enter\n", __func__);
	rc = aw8624_haptic_play_go(
		aw8624,
		false); // qti_haptics_module_en(aw8624, false);
	if (rc < 0)
		    ...
		    struct device_node *np)
{
	...
	val = of_property_read_u32(np, "aw8624_vib_f0_pre",
				   &aw8624->info.f0_pre);
	if (val != 0)
		pr_err("%s: vib_f0_pre not found\n", __func__);
	val = of_property_read_u32(np, "aw8624_vib_f0_cali_percen",
				   &aw8624->info.f0_cali_percen);
	if (val != 0)
		pr_err("%s: vib_f0_cali_percen not found\n", __func__);
	val = of_property_read_u32(np, "aw8624_vib_cont_drv_lev",
				   &aw8624->info.cont_drv_lvl);
	if (val != 0)
		pr_err("%s: vib_cont_drv_lev not found\n", __func__);
	val = of_property_read_u32(np, "aw8624_vib_cont_drv_lvl_ov",
				   &aw8624->info.cont_drv_lvl_ov);
	if (val != 0)
		pr_err("%s: vib_cont_drv_lvl_ov not found\n", __func__);
	val = of_property_read_u32(np, "aw8624_vib_cont_td",
				   &aw8624->info.cont_td);
	if (val != 0)
		pr_err("%s: vib_cont_td not found\n", __func__);
	val = of_property_read_u32(np, "aw8624_vib_cont_zc_thr",
				   &aw8624->info.cont_zc_thr);
	if (val != 0)
		pr_err("%s: vib_cont_zc_thr not found\n", __func__);
	val = of_property_read_u32(np, "aw8624_vib_cont_num_brk",
				   &aw8624->info.cont_num_brk);
	if (val != 0)
		pr_err("%s: vib_cont_num_brk not found\n", __func__);
	val = of_property_read_u32(np, "aw8624_vib_f0_coeff",
				   &aw8624->info.f0_coeff);
	if (val != 0)
	...
	val = of_property_read_u32(np, "aw8624_vib_r_spare",
				   &aw8624->info.r_spare);
	if (val != 0)
	...
	val = of_property_read_u32(np, "aw8624_vib_gain_flag",
	...
	aw8624->constant.pattern = devm_kcalloc(
		aw8624->dev, HAP_WAVEFORM_BUFFER_MAX, sizeof(u8), GFP_KERNEL);
	if (!aw8624->constant.pattern)
		return -ENOMEM;

	tmp = of_get_available_child_count(np);
	aw8624->predefined = devm_kcalloc(
		aw8624->dev, tmp, sizeof(*aw8624->predefined), GFP_KERNEL);
	...
	for_each_available_child_of_node (np, child_node) {
		...
		effect->pattern =
			devm_kcalloc(aw8624->dev, effect->pattern_length,
	...
	for (i = 0; i < 3; i++)
		for (j = 0; j < 5; j++)
			printk("%s:       aw8624->info.trig_config[%d][%d]: %d\n",
			       __func__, i, j, aw8624->info.trig_config[i][j]);
	for (i = 0; i < ARRAY_SIZE(aw8624->info.rtp_time); i++)
		printk("%s:       aw8624->info.rtp_time[%d]: %d\n", __func__, i,
		       aw8624->info.rtp_time[i]);
	...
	if (aw8624->effect_type == FF_CONSTANT) {
		...
		if (aw8624->effect_id < aw8624->info.effect_id_boundary) {
			...
			pr_info("%s: aw8624->effect_id=%d , aw8624->activate_mode = %d\n",
				__func__, aw8624->effect_id,
				aw8624->activate_mode);
			data[1] = aw8624->predefined[aw8624->effect_id]
					  .play_rate_us /
				  1000000; /*second data */
			data[2] = aw8624->predefined[aw8624->effect_id]
					  .play_rate_us /
		...
		if (aw8624->effect_id >= aw8624->info.effect_id_boundary) {
			...
			pr_info("%s: aw8624->effect_id=%d , aw8624->activate_mode = %d\n",
				__func__, aw8624->effect_id,
				aw8624->activate_mode);
			data[1] = aw8624->info.rtp_time[aw8624->effect_id] /
				  1000; /*second data */
			data[2] = aw8624->info.rtp_time[aw8624->effect_id] %
				  1000; /*millisecond data */
			pr_debug("%s: data[1] = %d data[2] = %d, rtp_time %d\n",
				 __func__, data[1], data[2],
				 aw8624->info.rtp_time[aw8624->effect_id]);
		}
...
int aw8624_haptics_playback(struct input_dev *dev, int effect_id, int val)
{
	struct aw8624 *aw8624 = input_get_drvdata(dev);
	int rc = 0;
	// Daniel 20210526 modify start
...
void aw8624_haptics_set_gain_work_routine(struct work_struct *work)
{
	...
	pr_info("%s: set_gain queue work, new_gain = %x level = %x \n",
		__func__, aw8624->new_gain, aw8624->level);
	...
	if (aw8624->ram_vbat_comp == AW8624_HAPTIC_RAM_VBAT_COMP_ENABLE &&
		...
		pr_info("%s: enable vbat comp, level = %x comp level = %x",
			__func__, aw8624->level, comp_level);
				...
				struct device_attribute *attr, char *buf)
{
	...
	aw8624_haptic_get_f0(aw8624);
	mutex_unlock(&aw8624->lock);
	len += snprintf(buf + len, PAGE_SIZE - len, "aw8624 cali f0 = %d\n",
			aw8624->f0);
	return len;
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "aw8624 cont f0 = %d\n",
			aw8624->cont_f0);
	return len;
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < AW8624_TRIG_NUM; i++) {
		len += snprintf(
			...
			"trig%d: enable=%d, default_level=%d, dual_edge=%d, "
			"frist_seq=%d, second_seq=%d\n",
			i + 1, aw8624->trig[i].enable,
			aw8624->trig[i].default_level,
			aw8624->trig[i].dual_edge, aw8624->trig[i].frist_seq,
