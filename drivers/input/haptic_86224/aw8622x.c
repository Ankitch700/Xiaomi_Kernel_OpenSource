...
int aw8622x_i2c_read(struct aw8622x *aw8622x, unsigned char reg_addr,
		     unsigned char *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
...
int aw8622x_i2c_writes(struct aw8622x *aw8622x, unsigned char reg_addr,
		       unsigned char *buf, unsigned int len)
{
	...
	if (data == NULL) {
		aw_dev_err(aw8622x->dev, "%s: can not allocate memory\n",
			   __func__);
		return -ENOMEM;
	}
	data[0] = reg_addr;
				  ...
				  unsigned char reg_data)
{
	...
	if (ret < 0) {
		aw_dev_err(aw8622x->dev, "%s: i2c read error, ret=%d\n",
			   __func__, ret);
		return ret;
	}
	reg_val &= mask;
	...
	if (ret < 0) {
		aw_dev_err(aw8622x->dev, "%s: i2c write error, ret=%d\n",
			   __func__, ret);
		return ret;
	}
	return 0;
		     ...
		     struct device_node *np)
{
	...
	val = of_property_read_u32(np, "aw8622x_vib_mode",
				   &aw8622x->dts_info.mode);
	if (val != 0)
		aw_dev_err(aw8622x->dev, "%s aw8622x_vib_mode not found\n",
			   __func__);
	val = of_property_read_u32(np, "aw8622x_vib_f0_pre",
				   &aw8622x->dts_info.f0_ref);
	if (val != 0)
		aw_dev_err(aw8622x->dev, "%s vib_f0_ref not found\n", __func__);
	val = of_property_read_u32(np, "aw8622x_vib_f0_cali_percen",
	...
	val = of_property_read_u32_array(np, "aw8622x_vib_sine_array",
					 sine_array_temp,
	...
	aw8622x->dts_info.is_enabled_auto_bst =
		of_property_read_bool(np, "aw8622x_vib_is_enabled_auto_bst");
	aw_dev_err(aw8622x->dev, "%s aw8622x->info.is_enabled_auto_bst = %d\n",
		   __func__, aw8622x->dts_info.is_enabled_auto_bst);
	// yundi add
	...
	if (val != 0)
		aw_dev_err(aw8622x->dev,
			   "%s: vib_effect_id_boundary not found\n", __func__);
	...
	aw8622x->constant.pattern = devm_kcalloc(
		aw8622x->dev, HAP_WAVEFORM_BUFFER_MAX, sizeof(u8), GFP_KERNEL);
	if (!aw8622x->constant.pattern)
		return -ENOMEM;

	tmp = of_get_available_child_count(np);
	aw8622x->predefined = devm_kcalloc(
		aw8622x->dev, tmp, sizeof(*aw8622x->predefined), GFP_KERNEL);
	...
	for_each_available_child_of_node (np, child_node) {
		...
		effect->pattern =
			devm_kcalloc(aw8622x->dev, effect->pattern_length,
...
static int aw8622x_haptic_set_gain(struct aw8622x *aw8622x, unsigned char gain)
{
	...
	if (aw8622x->ram_vbat_compensate ==
		...
		pr_debug("%s: ref %d vbat %d ", __func__, AW8622X_VBAT_REFER,
			 aw8622x->vbat);
		comp_gain = aw8622x->gain * AW8622X_VBAT_REFER / aw8622x->vbat;
		if (comp_gain > (128 * AW8622X_VBAT_REFER / AW8622X_VBAT_MIN)) {
			comp_gain = 128 * AW8622X_VBAT_REFER / AW8622X_VBAT_MIN;
		...
		pr_debug(
			"%s: disable compsensation, vbat=%d, vbat_min=%d, vbat_ref=%d",
			__func__, aw8622x->vbat, AW8622X_VBAT_MIN,
			AW8622X_VBAT_REFER);
				    ...
				    unsigned char play_mode)
{
	...
	case AW8622X_HAPTIC_CONT_MODE:
		...
		break;
	default:
		aw_dev_err(aw8622x->dev, "%s: play mode %d error", __func__,
			   play_mode);
		break;
	}
	return 0;
...
static int aw8622x_haptic_cont_get_f0(struct aw8622x *aw8622x)
{
	...
	if (!aw8622x->dts_info.cont_track_margin) {
		aw_dev_err(aw8622x->dev,
			   "%s aw8622x->dts_info.cont_track_margin = 0!\n",
			   __func__);
	} else {
		aw8622x_i2c_write(
			aw8622x, AW8622X_REG_CONTCFG11,
   ...
   */
	...
	aw8622x_i2c_write_bits(aw8622x, AW8622X_REG_PLAYCFG3,
			       AW8622X_BIT_PLAYCFG3_BRK_EN_MASK, brk_en_temp);
	return ret;
}
...
static int aw8622x_haptic_rtp_init(struct aw8622x *aw8622x)
{
	...
	aw_dev_err(aw8622x->dev, "%s enter\n", __func__);
	cpu_latency_qos_add_request(&aw8622x_pm_qos_req_vb,
	...
	while ((!aw8622x_haptic_rtp_get_fifo_afs(aw8622x)) &&
		...
		if ((aw8622x->rtp_cnt == aw8622x->rtp_container->len) ||
			...
			if (aw8622x->rtp_cnt == aw8622x->rtp_container->len)
				aw_dev_err(
					aw8622x->dev,
					"%s: rtp load completely! glb_state_val=0x%02x "
					"aw8622x->rtp_cnt=%d\n",
...
static void aw8622x_rtp_work_routine(struct work_struct *work)
{
	...
	if (aw8622x->state) {
		...
		aw8622x->rtp_init = 1;
		aw8622x_haptic_upload_lra(aw8622x, AW8622X_OSC_CALI);
		aw8622x_haptic_set_pwm(aw8622x,
				       AW8622X_PWM_24K); // Daniel 20210601
		/* gain */
		aw8622x_haptic_ram_vbat_compensate(aw8622x, false);
		aw8622x_haptic_set_gain(
			aw8622x, aw8622x->level); // Daniel 20210716 modify
		/* rtp mode config */
...
static int aw8622x_rtp_osc_calibration(struct aw8622x *aw8622x)
{
	...
	aw_dev_err(aw8622x->dev, "%s: rtp file:[%s] size = %dbytes\n", __func__,
		   aw8622x_rtp_name[0], aw8622x->rtp_container->len);
	...
	while (1) {
		if (!aw8622x_haptic_rtp_get_fifo_afs(aw8622x)) {
			...
			if (aw8622x->rtp_cnt != aw8622x->rtp_container->len) {
				...
				aw8622x->rtp_update_flag = aw8622x_i2c_writes(
					aw8622x, AW8622X_REG_RTPDATA,
					&aw8622x->rtp_container
						 ->data[aw8622x->rtp_cnt],
					buf_len);
				aw8622x->rtp_cnt += buf_len;
			}
			mutex_unlock(&aw8622x->rtp_lock);
		...
		if (osc_int_state & AW8622X_BIT_SYSST2_FF_EMPTY) {
			...
			pr_info("%s osc trim playback done aw8622x->rtp_cnt= %d\n",
				__func__, aw8622x->rtp_cnt);
			break;
		}
		ktime_get_real_ts64(&aw8622x->end);
		aw8622x->microsecond =
			(aw8622x->end.tv_sec - aw8622x->start.tv_sec) *
				1000000 +
			(aw8622x->end.tv_nsec - aw8622x->start.tv_nsec) /
				1000000;
					...
					unsigned long int real_time)
{
	...
	if (theory_time == real_time) {
		...
		if ((theory_time - real_time) > (theory_time / 50)) {
			pr_err("%s (theory_time - real_time) > (theory_time / 50), can't "
			       "calibrate!\n",
			       __func__);
			return DFT_LRA_TRIM_CODE;
		}
				    ...
				    struct aw8622x_container *aw8622x_cont)
{
	...
	aw8622x_i2c_write_bits(
		aw8622x, AW8622X_REG_RTPCFG3, AW8622X_BIT_RTPCFG3_FIFO_AEH_MASK,
		(unsigned char)(((aw8622x->ram.base_addr >> 1) >> 4) & 0xF0));
	/* FIFO AEL */
	aw8622x_i2c_write(
		aw8622x, AW8622X_REG_RTPCFG4,
		(unsigned char)(((aw8622x->ram.base_addr >> 1) & 0x00FF)));
	/* FIFO_AFH */
	aw8622x_i2c_write_bits(
		aw8622x, AW8622X_REG_RTPCFG3, AW8622X_BIT_RTPCFG3_FIFO_AFH_MASK,
   ...
   */
	...
	for (i = shift; i < aw8622x_cont->len; i++) {
		aw8622x->ram_update_flag = aw8622x_i2c_write(
			aw8622x, AW8622X_REG_RAMDATA, aw8622x_cont->data[i]);
	}
#ifdef AW_CHECK_RAM_DATA
	shift = aw8622x->ram.baseaddr_shift;
	aw8622x_i2c_write_bits(aw8622x, AW8622X_REG_RAMADDRH,
     ...
     */
		if (reg_val != aw8622x_cont->data[i]) {
			aw_dev_err(
				aw8622x->dev,
				"%s: ram check error addr=0x%04x, file_data=0x%02X, "
				"ram_data=0x%02X\n",
...
static int aw8622x_rtp_trim_lra_calibration(struct aw8622x *aw8622x)
{
	...
	lra_trim_code =
		aw8622x_osc_trim_calculation(theory_time, aw8622x->microsecond);
	if (lra_trim_code >= 0) {
...
static int aw8622x_haptic_f0_calibration(struct aw8622x *aw8622x)
{
	...
	unsigned int f0_cali_min = aw8622x->dts_info.f0_ref *
				   (100 - aw8622x->dts_info.f0_cali_percent) /
				   100;
   ...
   */
	if (aw8622x_haptic_cont_get_f0(aw8622x)) {
		...
		aw_dev_err(
			aw8622x->dev,
			"%s f0_ref = %d, f0_cali_min = %d, f0_cali_max = %d, f0 = %d\n",
			__func__, aw8622x->dts_info.f0_ref, f0_cali_min,
			f0_cali_max, aw8622x->f0);
		...
		/* calculate cali step */
		f0_cali_step = 100000 *
			       ((int)f0_limit - (int)aw8622x->dts_info.f0_ref) /
			       ((int)f0_limit * 24);
...
aw8622x_haptic_brightness_get(struct led_classdev *cdev)
{
...
static void aw8622x_haptic_audio_work_routine(struct work_struct *work)
{
	struct aw8622x *aw8622x =
		container_of(work, struct aw8622x, haptic_audio.work);
	...
	if (aw8622x->haptic_audio.ctr.play) {
		aw_dev_err(
			...
			__func__, aw8622x->haptic_audio.ctr.cnt,
	...
	if (AW8622X_HAPTIC_CMD_ENABLE ==
		...
		if (aw8622x->haptic_audio.ctr.play ==
			...
			aw_dev_err(aw8622x->dev, "%s: normal haptic start\n",
				   __func__);
			mutex_lock(&aw8622x->lock);
			aw8622x_haptic_stop(aw8622x);
			aw8622x_haptic_play_mode(aw8622x,
						 AW8622X_HAPTIC_RAM_MODE);
			aw8622x_haptic_set_wav_seq(
				aw8622x, 0x00,
				aw8622x->haptic_audio.ctr.wavseq);
			aw8622x_haptic_set_wav_loop(
				aw8622x, 0x00, aw8622x->haptic_audio.ctr.loop);
				 ...
				 size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		if (databuf[0] > (AW8622X_SEQUENCER_SIZE - 1)) {
			aw_dev_err(aw8622x->dev,
				 ...
				 size_t count)
{
	...
	rc = kstrtouint(buf, 0, &val);
	if (rc < 0) {
		aw_dev_err(aw8622x->dev, "%s: kstrtouint fail\n", __func__);
		return rc;
	}
	mutex_lock(&aw8622x->lock);
	aw8622x_haptic_stop(aw8622x);
					  ...
					  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d %d %d %d %d", &databuf[0], &databuf[1],
		...
		if (databuf[2]) {
			...
			aw8622x_haptic_audio_ctr_list_insert(
				&aw8622x->haptic_audio, hap_ctr, aw8622x->dev);
			if (hap_ctr->cmd == 0xff) {
				aw_dev_err(aw8622x->dev,
					   "%s: haptic_audio stop\n", __func__);
				if (hrtimer_active(
					    &aw8622x->haptic_audio.timer)) {
					aw_dev_err(
						aw8622x->dev,
...
static struct attribute *aw8622x_vibrator_attributes[] = {
	&dev_attr_state.attr,	 &dev_attr_duration.attr,
	&dev_attr_activate.attr,      &dev_attr_activate_mode.attr,
	&dev_attr_index.attr,	 &dev_attr_gain.attr,
	&dev_attr_seq.attr,	   &dev_attr_loop.attr,
	&dev_attr_register.attr,      &dev_attr_rtp.attr,
	&dev_attr_ram_update.attr,    &dev_attr_f0.attr,
	&dev_attr_cali.attr,	  &dev_attr_f0_save.attr,
	&dev_attr_osc_save.attr,      &dev_attr_cont.attr,
	&dev_attr_cont_wait_num.attr, &dev_attr_cont_drv_lvl.attr,
	&dev_attr_cont_drv_time.attr, &dev_attr_cont_brk_time.attr,
	&dev_attr_vbat_monitor.attr,  &dev_attr_lra_resistance.attr,
	&dev_attr_prctmode.attr,      &dev_attr_sram_size.attr,
	&dev_attr_ram_vbat_comp.attr, &dev_attr_osc_cali.attr,
	&dev_attr_gun_type.attr,      &dev_attr_bullet_nr.attr,
	&dev_attr_haptic_audio.attr,  NULL
};
					  ...
					  unsigned char flag)
{
	...
	if (flag) {
		if (aw8622x->activate_mode ==
			...
			if (aw8622x->effect_id == 5 ||
			    aw8622x->effect_id == 6) {
				aw8622x->effect_id = 6;
			} else if (aw8622x->effect_id == 1) {
				aw8622x->effect_id = 3;
			} else if ((aw8622x->effect_id >= 2) &&
				   (aw8622x->effect_id <= 9)) {
				aw8622x->effect_id = 8;
			}
			// Daniel 20210719 modify end
			aw8622x_haptic_set_wav_seq(
				aw8622x, 0x00, (char)aw8622x->effect_id + 1);
			aw8622x_haptic_set_pwm(
				aw8622x, AW8622X_PWM_12K); // Daniel 20210601
			// modify
			aw8622x_haptic_set_wav_seq(aw8622x, 0x01, 0x00);
		...
		if (aw8622x->activate_mode ==
			...
			aw8622x_haptic_set_repeat_wav_seq(
				aw8622x,
				(aw8622x->dts_info.effect_id_boundary + 1));
			// Daniel 20210716 modify start
			aw8622x_i2c_write_bits(
				aw8622x, AW8622X_REG_SYSCTRL7,
...
{
	struct aw8622x *aw8622x =
		container_of(work, struct aw8622x, long_vibrate_work);

	aw_dev_dbg(aw8622x->dev, "%s enter\n", __func__);

	printk("%s: effect_id = %d state=%d activate_mode = %d duration = %d\n",
	       __func__, aw8622x->effect_id, aw8622x->state,
	       aw8622x->activate_mode, aw8622x->duration);
...
int aw8622x_vibrator_init(struct aw8622x *aw8622x)
{
	...
	ret = sysfs_create_group(&aw8622x->i2c->dev.kobj,
				 &aw8622x_vibrator_attribute_group);
	if (ret < 0) {
		aw_dev_err(aw8622x->dev,
			   "%s error creating bus sysfs attr files\n",
			   __func__);
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_err(aw8622x->dev,
			   "%s error creating class sysfs link attr files\n",
			   __func__);
		return ret;
	}
#endif
...
static void aw8622x_haptic_misc_para_init(struct aw8622x *aw8622x)
{
	...
	if (!aw8622x->dts_info.cont_bemf_set) {
		aw_dev_err(aw8622x->dev,
			   "%s aw8622x->dts_info.cont_bemf_set = 0!\n",
			   __func__);
	} else {
		aw8622x_i2c_write_bits(aw8622x, AW8622X_REG_CONTCFG13,
  ...
  */
	...
	if (!aw8622x->dts_info.cont_brk_gain) {
		aw_dev_err(aw8622x->dev,
			   "%s aw8622x->dts_info.cont_brk_gain = 0!\n",
			   __func__);
	} else {
		aw8622x_i2c_write_bits(aw8622x, AW8622X_REG_CONTCFG5,
...
{
	struct aw8622x *aw8622x =
		container_of(work, struct aw8622x, ram_work.work);
...
{
	struct aw8622x *aw8622x =
		container_of(timer, struct aw8622x, haptic_audio.timer);
...
static void aw8622x_haptic_auto_bst_enable(struct aw8622x *aw8622x,
					   unsigned char flag)
{
	if (flag) {
		aw8622x_i2c_write_bits(aw8622x, AW8622X_REG_PLAYCFG3,
...
int aw8622x_haptic_init(struct aw8622x *aw8622x)
{
	...
	hrtimer_init(&aw8622x->haptic_audio.timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
	...
	aw8622x_haptic_auto_bst_enable(aw8622x,
				       aw8622x->dts_info.is_enabled_auto_bst);
	/* vbat compensation */
	aw8622x_haptic_vbat_mode_config(
		aw8622x, AW8622X_HAPTIC_CONT_VBAT_HW_ADJUST_MODE);
	aw8622x->ram_vbat_compensate = AW8622X_HAPTIC_RAM_VBAT_COMP_ENABLE;

	/* f0 calibration */
	/*LRA trim source select register*/
	aw8622x_i2c_write_bits(aw8622x, AW8622X_REG_TRIMCFG1,
...
irqreturn_t aw8622x_irq(int irq, void *data)
{
	...
	if (reg_val & AW8622X_BIT_SYSINT_FF_AEI) {
		...
		if (aw8622x->rtp_init) {
			while ((!aw8622x_haptic_rtp_get_fifo_afs(aw8622x)) &&
				...
				if ((aw8622x->rtp_cnt ==
					...
					if (aw8622x->rtp_cnt ==
						...
						aw_dev_err(
							aw8622x->dev,
							"%s: rtp load completely! glb_state_val=%02x "
							"aw8622x->rtp_cnt=%d\n",
							__func__, glb_state_val,
							aw8622x->rtp_cnt);
					else
			...
			aw_dev_err(aw8622x->dev,
				   "%s: aw8622x rtp init = %d, init error\n",
	...
	if (reg_val & AW8622X_BIT_SYSINT_FF_AFI)
		aw_dev_err(aw8622x->dev,
			   "%s: aw8622x rtp mode fifo almost full!\n",
			   __func__);
				  ...
				  struct ff_effect *old)
{
	...
	aw_dev_dbg(aw8622x->dev,
		   "%s: effect->type=0x%x,FF_CONSTANT=0x%x,FF_PERIODIC=0x%x\n",
	...
	if (aw8622x->effect_type == FF_CONSTANT) {
		...
		if (aw8622x->effect_id < aw8622x->dts_info.effect_id_boundary) {
			...
			aw_dev_dbg(
				...
				__func__, aw8622x->effect_id,
				aw8622x->activate_mode);
			data[1] = aw8622x->predefined[aw8622x->effect_id]
					  .play_rate_us /
				  1000000; /*second data */
			data[2] = aw8622x->predefined[aw8622x->effect_id]
					  .play_rate_us /
				  1000; /*millisecond data */
			pr_debug(
				"%s: aw8622x->predefined[aw8622x->effect_id].play_rate_us/1000 "
				"= %d\n",
				__func__,
		...
		if (aw8622x->effect_id >=
		    aw8622x->dts_info.effect_id_boundary) {
			aw8622x->activate_mode =
				AW8622X_HAPTIC_ACTIVATE_RTP_MODE;
			pr_info("%s: aw8622x->effect_id=%d , aw8622x->activate_mode = %d\n",
				__func__, aw8622x->effect_id,
				aw8622x->activate_mode);
			data[1] =
				aw8622x->dts_info.rtp_time[aw8622x->effect_id] /
				1000; /*second data */
			data[2] =
				aw8622x->dts_info.rtp_time[aw8622x->effect_id] %
				1000; /*millisecond data */
			pr_debug(
				"%s: data[1] = %d data[2] = %d, rtp_time %d\n",
...
int aw8622x_haptics_playback(struct input_dev *dev, int effect_id, int val)
{
	struct aw8622x *aw8622x = input_get_drvdata(dev);
	int rc = 0;
	pr_debug("%s:  %d enter\n", __func__, __LINE__);
...
void aw8622x_haptics_set_gain_work_routine(struct work_struct *work)
{
	...
	pr_info("%s: set_gain queue work, new_gain = %x level = %x \n",
		__func__, aw8622x->new_gain, aw8622x->level);
	...
	if (aw8622x->ram_vbat_compensate ==
		...
		pr_info("%s: enable vbat comp, level = %x comp level = %x",
			__func__, aw8622x->level, comp_level);
		aw8622x_i2c_write(aw8622x, AW8622X_REG_PLAYCFG2,
				  comp_level); // Daniel 20210716
	} else {
		pr_debug(
			"%s: disable compsensation, vbat=%d, vbat_min=%d, vbat_ref=%d",
			__func__, aw8622x->vbat, AW8622X_VBAT_MIN,
