...
#define AF_DRVNAME "EMERALD_GT9764VAF_DRV"
...
static int s4AF_ReadReg(u8 a_uAddr, u8 *a_uData)
{
	...
	if (i2c_master_send(g_pstAF_I2Cclient, &a_uAddr, 1) < 0) {
		LOG_ERR("ReadI2C send failed!! a_uAddr = %d\n", a_uAddr);
		return -1;
	}

	if (i2c_master_recv(g_pstAF_I2Cclient, a_uData, 1) < 0) {
		LOG_ERR("ReadI2C recv failed!!a_uAddr = %d\n", a_uAddr);
		return -1;
	}
...
static int s4AF_WriteReg(u8 a_uLength, u8 a_uAddr, u16 a_u2Data)
{
	...
	if (a_uLength == 0) {
		if (i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2) < 0) {
			LOG_ERR("WriteI2C failed!! a_uAddr=%d, a_u2Data=%d\n", a_uAddr, a_u2Data);
			return -1;
		}
	} else if (a_uLength == 1) {
		if (i2c_master_send(g_pstAF_I2Cclient, puSendCmd2, 3) < 0) {
			LOG_ERR("WriteI2C 2 failed!!  a_uAddr=%d, a_u2Data=%d\n", a_uAddr, a_u2Data);
			return -1;
		}
	}
...
static int setPosition(unsigned short UsPosition)
{
	...
	if (i4RetValue != 0) {
		LOG_ERR("DAC set Reg 0x03 , UcPosH = %d failed", UcPosH);
		return -1;
	}
	i4RetValue = s4AF_WriteReg(0, 0x04, UcPosL); //VCM_LSB
	if (i4RetValue != 0) {
		LOG_ERR("DAC set Reg 0x04, UcPosL = %d failed", UcPosL);
	}
...
/*N6 code for HQ-322763 by huabinchen at 20230824 start*/
		u16 puSendCmdArray[][2] = {
			{0x02, 0x02}, {0x06, 0x40}, {0x07, 0x0F},
		};
/*N6 code for HQ-322763 by huabinchen at 20230824 end*/
/*N6 code for HQ-306051 by huabinchen at 20230711 end*/
		...
		for (cmd_number = 0; cmd_number < sizeof(puSendCmdArray)/sizeof(puSendCmdArray[0]); cmd_number++) {
			ret = s4AF_WriteReg(0, (u8)puSendCmdArray[cmd_number][0], puSendCmdArray[cmd_number][1]);
			if (ret < 0) {
				LOG_ERR("init register failed %x\n");
				return -1;
			}
		}
...
/* ////////////////////////////////////////////////////////////// */
long EMERALD_GT9764VAF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
		      unsigned long a_u4Param)
{
	long i4RetValue = 0;

	LOG_INF("%s: a_u4Command = %d, a_u4Param = %ul", __func__, a_u4Command, a_u4Param);
	switch (a_u4Command) {
...
int EMERALD_GT9764VAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	int Ret = 0;
...
int EMERALD_GT9764VAF_PowerDown(struct i2c_client *pstAF_I2Cclient,
			int *pAF_Opened)
{
...
int EMERALD_GT9764VAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int EMERALD_GT9764VAF_GetFileName(unsigned char *pFileName)
{
