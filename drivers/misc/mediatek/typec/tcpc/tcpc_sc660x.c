...
static int sc660x_read_device(void *client, u32 reg, int len, void *dst)
{
	...
	while (1) {
		...
		t2 = local_clock();
		SC660X_INFO("%s del = %lluus, reg = 0x%02X, len = %d, val = 0x%08X\n",
...
#endif /* CONFIG_RT_REGMAP */
	if (ret < 0) {
		dev_err(sc->dev, "sc660x reg read fail\n");
		return ret;
	}
	return val;
}
...
#endif /* CONFIG_RT_REGMAP */
	if (ret < 0)
		dev_err(sc->dev, "sc660x reg write fail\n");
	return ret;
}
			...
			u8 reg, int len, void *dst)
{
	...
	if (ret < 0)
		dev_err(sc->dev, "sc660x block read fail\n");
	return ret;
}
...
#endif /* #ifdef CONFIG_RT_REGMAP */
	if (ret < 0)
		dev_err(sc->dev, "sc660x block write fail\n");
	return ret;
}
...
int chg_sc660x_set_continuous_time(struct tcpc_device *tcpc)
{
	...
	ret = sc660x_write_device(sc->client, 0xE7, 1, &data[0]);
	ret |= sc660x_write_device(sc->client, 0xE8, 1, &data[1]);
	ret |= sc660x_write_device(sc->client, 0xE9, 1, &data[2]);
	ret |= sc660x_write_device(sc->client, 0xEA, 1, &data[3]);
	ret |= sc660x_write_device(sc->client, 0xEB, 1, &data[4]);

	pr_info("%s set_continuous_time ret = %d\n", __func__, ret);
	return ret;
}
...
static int sc660x_set_dis_vconn_ov(struct tcpc_device *tcpc, bool en)
{
	int ret;
	ret = sc660x_i2c_read8(tcpc, TCPC_V10_REG_FAULT_CTRL);
	if (ret < 0)
		return ret;
	if (en) {
		ret &= ~TCPC_V10_REG_FAULT_CTRL_DIS_VCONN_OV;
		ret |= TCPC_V10_REG_FAULT_CTRL_DIS_VCONN_OV;
		pr_info("%s TCPC_V10_REG_FAULT_CTRL_DIS_VCONN_OV true !!\n", __func__);
	} else {
		ret &= ~TCPC_V10_REG_FAULT_CTRL_DIS_VCONN_OV;
		pr_info("%s TCPC_V10_REG_FAULT_CTRL_DIS_VCONN_OV false !!\n", __func__);
	}
	sc660x_i2c_write8(tcpc, TCPC_V10_REG_FAULT_CTRL, ret);
	pr_err("ret = %d!!\n", ret);
	return 0;
}
			...
			uint16_t *msg_head, enum tcpm_transmit_type *frame_type)
{
	...
	*msg_head = le16_to_cpu(*(uint16_t *)&buf[2]);
	...
	if (PD_DATA_BIST == PD_HEADER_TYPE(*msg_head) && cnt > 3)
		sc660x_set_bist_test_mode(tcpc, true);
...
static struct tcpc_ops sc660x_tcpc_ops = {
	...
	.set_dis_vconn_ov = sc660x_set_dis_vconn_ov,
...
#endif	/* CONFIG_USB_POWER_DELIVERY */
	/* N19A code for HQHW-6756 by wuwencheng at 20240712 start */
	.set_continuous_time = chg_sc660x_set_continuous_time,
};
static int sc660x_parse_dt(struct sc660x *sc, struct device *dev)
{
	...
	int ret = 0;
	pr_info("%s\n", __func__);
	ret = of_get_named_gpio(np, "sc660x,intr_gpio_num", 0);
	if (ret < 0) {
...
/* N6R code for HQ-429228 at 2024/11/07 by p-mazhuang3 start */
  	/* mediatek boot mode */
  	boot_node = of_parse_phandle(dev->of_node, "sc,bootmode", 0);
  	if (!boot_node)
  		SC660X_INFO("%s: failed to get boot mode phandle\n", __func__);
  	else {
  		tag = (struct tag_bootmode *)of_get_property(boot_node,
  							"atag,boot", NULL);
  		if (!tag)
  			SC660X_INFO("%s: failed to get atag,boot\n", __func__);
  		else {
  			SC660X_INFO("%s: size:0x%x tag:0x%x bootmode:0x%x boottype:0x%x\n",
  				__func__, tag->size, tag->tag, tag->bootmode, tag->boottype);

		sc->tcpc->bootmode = tag->bootmode;
		sc->tcpc->boottype = tag->bootmode;
  		// info->bootmode = tag->bootmode;
  		// info->boottype = tag->boottype;
  		}
  	}
/* N6R code for HQ-429228 at 2024/11/07 by p-mazhuang3 end */
	return 0;
}
				...
				const struct i2c_device_id *id)
{
	...
	tcpc_schedule_init_work(sc->tcpc);
