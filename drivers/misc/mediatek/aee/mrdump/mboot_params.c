...
#define array_size_aee(x) (sizeof(x) / sizeof((x)[0]))
...
int aee_rr_reboot_reason_show(struct seq_file *m, void *v)
{
	...
	if (mboot_params_check_header(mboot_params_old)) {
		...
		for (i = 0; i < array_size_aee(aee_rr_last_xxx); i++)
			aee_rr_last_xxx[i] (m);
		return 0;
	}
	for (i = 0; i < array_size_aee(aee_rr_show); i++)
		aee_rr_show[i] (m);

	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		seq_printf(m, "CPU %d\n", cpu);
		for (i = 0; i < array_size_aee(aee_rr_show_cpu); i++)
