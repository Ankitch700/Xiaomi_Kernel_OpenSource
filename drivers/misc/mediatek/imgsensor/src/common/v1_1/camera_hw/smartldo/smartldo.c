...
wl2866_ldo_pin_map_t wl2866_ldo_pin_map[] = {
	{ IMGSENSOR_SENSOR_IDX_MAIN, IMGSENSOR_HW_PIN_AVDD, WL2866_LDO_AVDD1},//2.8 ok
	{ IMGSENSOR_SENSOR_IDX_MAIN, IMGSENSOR_HW_PIN_DVDD, WL2866_LDO_DVDD1},//1.0 1.1 V ok

	{ IMGSENSOR_SENSOR_IDX_SUB, IMGSENSOR_HW_PIN_AVDD, WL2866_LDO_AVDD2},//2.8V ok
	{ IMGSENSOR_SENSOR_IDX_SUB, IMGSENSOR_HW_PIN_DVDD, WL2866_LDO_DVDD2},//1.2V  ok

	{ IMGSENSOR_SENSOR_IDX_MAIN2, IMGSENSOR_HW_PIN_AVDD, WL2866_LDO_AVDD2},//2.8V  ULTRA Depth ok
	{ IMGSENSOR_SENSOR_IDX_MAIN2, IMGSENSOR_HW_PIN_DVDD, WL2866_LDO_DVDD2},//1.2V ULTRA  this?

	{ IMGSENSOR_SENSOR_IDX_SUB2, IMGSENSOR_HW_PIN_AVDD, WL2866_LDO_AVDD2},//2.8V Depth ok
	{ IMGSENSOR_SENSOR_IDX_SUB2, IMGSENSOR_HW_PIN_DVDD, WL2866_LDO_DVDD2},//2.8V Marco or this?
};
...
wl2866_ldo_votage_map_t wl2866_ldo_avdd_votage_map[] = {
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_2200, 0x50},//2.2V
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_2800, 0x80},//2.8V
...
wl2866_ldo_votage_map_t wl2866_ldo_dvdd_votage_map[] = {
	...
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_900, 0x32},// 0.900V
/*N6 code for  HQ-308044 by hudongjiao at 20230718 start*/
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_1100, 0x54},// 1.1V
/*N6 code for  HQ-308044 by hudongjiao at 20230718 end*/
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_1200, 0x64},//1.2V
...
/*N6 code for HQ-306051 by huabinchen at 20230710 start*/
	if (pin > IMGSENSOR_HW_PIN_DOVDD   ||
	    ...
	    pin_state > IMGSENSOR_HW_PIN_STATE_LEVEL_2200 ||
	    sensor_idx < 0)
		return ret;
/*N6 code for HQ-306051 by huabinchen at 20230710 end*/
...
static int wl2866_ldo_set_value(cam_cell_id_t ldo_id, uint32_t value)
{
	...
	if (ret == 0) {
		...
		printk("%s: read ldo_id =%d register value = %d\n", __func__, value, wl2866_map[ldo_id].reg_addr);
	}
	return ret;
}
...
// echo "sensorIdx pin pinstate" > /proc/ldo_camera_status
// MAIN AVDD -> enable: echo " 0 3 10" > /proc/ldo_camera_status     disable:echo " 0 3 0" > /proc/ ldo_camera_status     2.8V  avdd1
// MAIN DVDD ->  enable: echo " 0 6 2" > /proc/ldo_camera_status     disable:echo " 0 4 0" > /proc/ ldo_camera_status    1.1V   dvdd1
			 ...
			 const struct i2c_device_id *id)
{
	...
	/*ret = smartldo_set(NULL, IMGSENSOR_SENSOR_IDX_MAIN, IMGSENSOR_HW_PIN_DVDD, IMGSENSOR_HW_PIN_STATE_LEVEL_900);
	printk("%s: set IMGSENSOR_SENSOR_IDX_MAIN IMGSENSOR_HW_PIN_DVDD IMGSENSOR_HW_PIN_STATE_LEVEL_900 ret = %d\n", __func__,ret);
	ret = smartldo_set(NULL, IMGSENSOR_SENSOR_IDX_MAIN, IMGSENSOR_HW_PIN_AVDD, 0);
	printk("%s: set IMGSENSOR_SENSOR_IDX_MAIN IMGSENSOR_HW_PIN_AVDD IMGSENSOR_HW_PIN_STATE_LEVEL_2800 ret = %d\n", __func__,ret);
	ret = smartldo_set(NULL, IMGSENSOR_SENSOR_IDX_SUB, IMGSENSOR_HW_PIN_AVDD, 0);
	printk("%s: set IMGSENSOR_SENSOR_IDX_SUB, IMGSENSOR_HW_PIN_AVDD, IMGSENSOR_HW_PIN_STATE_LEVEL_2800 ret = %d\n", __func__,ret);
	ret = smartldo_set(NULL, IMGSENSOR_SENSOR_IDX_SUB, IMGSENSOR_HW_PIN_DVDD, 0);
	printk("%s: set IMGSENSOR_SENSOR_IDX_SUB, IMGSENSOR_HW_PIN_DVDD, IMGSENSOR_HW_PIN_STATE_LEVEL_1200 ret = %d\n", __func__,ret);
	*/
