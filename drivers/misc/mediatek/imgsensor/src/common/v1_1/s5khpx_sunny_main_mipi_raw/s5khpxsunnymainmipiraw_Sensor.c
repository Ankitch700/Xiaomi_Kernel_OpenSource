...
#include "s5khpxsunnymainmipiraw_Sensor.h"
...
#define LOG_TAG "s5khpxsunny"
#define S5KHPXSUNNY_LOG_INF(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
#define S5KHPXSUNNY_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
static unsigned char s5khpx_QSC_data[QSC_DATA_LEN];
/*N6 code for HQ-323769 by HQ camera at 20230901 start*/
#define ISZ_IS_ON 1 //add by stan
/*N6 code for HQ-323769 by HQ camera at 20230901 end*/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHPX_SUNNY_MAIN_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h

	.checksum_value = 0x47a75476,		//checksum value for Camera Auto Test

/*N6 code for HQ-311278 by HQ camera at 20230823 start*/
	.pre = {
		.pclk = 2000000000,
		.linelength  = 10560,
		.framelength = 6302,
...
/*N6 code for HQ-328697 by p-tuping at 20230913 start*/
	.slim_video = {
		.pclk = 2000000000,
		.linelength = 10560,
		.framelength = 3152,
...
/*N6 code for HQ-328697 by p-tuping at 20230913 end*/
...
/*N6 code for HQ-311278 by HQ camera at 20230823 end*/

	.margin = 164,			//sensor framelength & shutter margin
	...
	.gain_step = 1,
	.gain_type = 2,
	.max_frame_length = 0xBC16,//max framelength by sensor register's limitation
	...
	.custom6_delay_frame = 2,	//enter custom5 delay frame num
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[11] = {
	...
	{ 16320,12288,    0,     0,  16320,12288,  4080, 3072,    0,    0,  4080, 3072,    0,    0,  4080, 3072}, // capture 50M 30fps
	{ 16320,12288,    0,  1552,  16320, 9184,  4080, 2296,    0,    0,  4080, 2296,    0,    0,  4080, 2296}, // normal video 4080x2296 30fps
	{ 16320,12288, 2400,  2904,  11520, 6480,  1920, 1080,    0,    0,  1920, 1080,    0,    0,  1920, 1080}, // hs video as 240fps
	{ 16320,12288,    0,  1552,  16320, 9184,  4080, 2296,    0,    0,  4080, 2296,    0,    0,  4080, 2296}, // 12M @ 60fps
	{ 16320,12288,    0,     0,  16320,12288,  4080, 3072,    0,    0,  4080, 3072,    0,    0,  4080, 3072}, // custom1 stereo 24fps
	...
	{ 16320,12288,    0,     0,  16320,12288,  8160, 6144,    0,    0,  8160, 6144,    0,    0,  8160, 6144}, // custom4 full size capture
	{ 16320,12288, 4080,  3072,   8160, 6144,  4080, 3072,    0,    0,  4080, 3072,    0,    0,  4080, 3072}, // custom5 full size capture
	{ 16320,12288,    0,     0, 16320,12288,  16320,12288,    0,    0,  16320,12288,   0,    0,  16320, 12288}, // custom6 as full size
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4BlockNumY = 0,
	.i4LeFirst = 0,
/*N6 code for HQ-323257 by HQ camera at 20230829 start*/
	.i4Crop = {
		{0, 0}, {0, 0}, {0, 388}, {0, 388}, {0, 388},
		{0, 0}, {60, 228}, {0, 388}, {0, 0}, {2040, 1536}
	},
/*N6 code for HQ-323257 by HQ camera at 20230829 end*/
	.iMirrorFlip = 0,
};
static struct SENSOR_VC_INFO2_STRUCT SENSOR_VC_INFO2[4] = {
	...
	{
		0x02, 0x0a, 0x00, 0x08, 0x40, 0x00,//zoom2x
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0xff0, 0xC00},//4080*3072
...
/*N6 code for HQ-306077 by HQ camera at 20230713 start*/
	...
	{
		S5KHPXSUNNY_LOG_INF("not match vendor id:0x%x\n",vendor_id);
		sensor_id = 0xFFFF;
	}

	S5KHPXSUNNY_LOG_INF("[%s] vendor_id: 0x%x, sensor_id: 0x%x,sensor_version:0x%x\n", __func__, vendor_id, sensor_id,sensor_version);
/*N6 code for HQ-306077 by HQ camera at 20230713 end*/
	return sensor_id;
}
...
{
	S5KHPXSUNNY_LOG_DBG("dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	S5KHPXSUNNY_LOG_DBG("framerate = %d, min framelength =%d, should enable? \n", framerate, min_framelength_en);
...
static void write_shutter(kal_uint32 shutter)
{
	...
	S5KHPXSUNNY_LOG_DBG("Exit! shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length, kal_bool auto_extend_en)
{
	...
	S5KHPXSUNNY_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
...
static void set_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    ...
    S5KHPXSUNNY_LOG_INF("write awb gain r:g:b %d:%d:%d \n", r_Gain, g_Gain, b_Gain);
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	S5KHPXSUNNY_LOG_INF("set_gain %d \n", gain);
	if (gain < imgsensor_info.min_gain || gain > imgsensor_info.max_gain) {
		S5KHPXSUNNY_LOG_INF("Error gain setting");
...
{
	S5KHPXSUNNY_LOG_DBG("le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
...
static kal_uint32 streaming_control(struct imgsensor_struct imgsensor,kal_bool enable)
{
	...
	int i=0;
	int framecnt = 0;
	S5KHPXSUNNY_LOG_INF("streaming_enable(0=sw standby,1=streaming):%d\n",enable);
	if (enable) {
/*N6 code for HQ-323769 by HQ camera at 20230901 start*/
#if ISZ_IS_ON
		write_cmos_sensor(0xFCFC, 0x2000);
		if((imgsensor.sensor_mode == IMGSENSOR_MODE_PREVIEW) || (imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE))
		{
			write_cmos_sensor(0x13D8, 0x2003);
			write_cmos_sensor(0x13DA, 0x0000);
		}
		else if(imgsensor.sensor_mode == IMGSENSOR_MODE_CUSTOM5)
		{
			write_cmos_sensor(0x13D8, 0x2003);
			write_cmos_sensor(0x13DA, 0x0508);
		}
		else
		{
...
/*N6 code for HQ-323769 by HQ camera at 20230901 end*/
		...
		S5KHPXSUNNY_LOG_INF("stream off fail! framecnt = %d\n",framecnt);
	}
	return ERROR_NONE;
}
...
static void sensor_init(void)
{
	...
	// Global
	S5KHPXSUNNY_LOG_INF("loading global setting+\n");
	table_write_cmos_sensor(s5khpxsunny_init_setting_global,
			sizeof(s5khpxsunny_init_setting_global) / sizeof(kal_uint16));
	S5KHPXSUNNY_LOG_INF("loading global setting-\n");
}	/*	sensor_init  */
...
{
	S5KHPXSUNNY_LOG_INF("+\n");
	table_write_cmos_sensor(s5khpxsunny_preview_setting,
			sizeof(s5khpxsunny_preview_setting) / sizeof(kal_uint16));
...
{
	S5KHPXSUNNY_LOG_INF("capture setting E\n");
	table_write_cmos_sensor(s5khpxsunny_preview_setting,
			sizeof(s5khpxsunny_preview_setting) / sizeof(kal_uint16));
}	/*	capture_setting  */
...
{
	S5KHPXSUNNY_LOG_INF("E!\n");
	table_write_cmos_sensor(s5khpxsunny_normal_video_setting,
			sizeof(s5khpxsunny_normal_video_setting) / sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(s5khpxsunny_hs_video_setting,
		sizeof(s5khpxsunny_hs_video_setting) / sizeof(kal_uint16));
}
...
static void slim_video_setting(void)
{
	S5KHPXSUNNY_LOG_INF("E\n");
/*N6 code for HQ-346208 by HQ camera at 20231108 start*/
	table_write_cmos_sensor(s5khpxsunny_slim_video_setting,
		sizeof(s5khpxsunny_slim_video_setting) / sizeof(kal_uint16));
/*N6 code for HQ-346208 by HQ camera at 20231108 end*/
}
...
{
	S5KHPXSUNNY_LOG_INF("E\n");
	table_write_cmos_sensor(s5khpxsunny_custom1_setting,
		sizeof(s5khpxsunny_custom1_setting) / sizeof(kal_uint16));
}

static void custom2_setting(void)
{
	S5KHPXSUNNY_LOG_INF("E\n");
	table_write_cmos_sensor(s5khpxsunny_custom2_setting,
			sizeof(s5khpxsunny_custom2_setting) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	table_write_cmos_sensor(s5khpxsunny_custom3_setting,
			sizeof(s5khpxsunny_custom3_setting) / sizeof(kal_uint16));
}

static void custom4_setting(void)
{
	S5KHPXSUNNY_LOG_INF("E\n");
	table_write_cmos_sensor(s5khpxsunny_custom4_setting,
			sizeof(s5khpxsunny_custom4_setting) / sizeof(kal_uint16));
}
...
static void custom5_setting(void)
{
	...
	table_write_cmos_sensor(s5khpxsunny_custom5_setting,
			sizeof(s5khpxsunny_custom5_setting) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 3;
	S5KHPXSUNNY_LOG_INF("S5KHPXSUNNY get_imgsensor_id in\n");
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				S5KHPXSUNNY_LOG_INF("probe success, i2c write id: 0x%x, sensor id: 0x%x\n",
				...
				S5KHPXSUNNY_LOG_INF("Read sensor id fail,i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			}
			retry--;
		} while (retry > 0);
		i++;
...
static void read_QSC_data(void)
{
	...
	S5KHPXSUNNY_LOG_INF("read_QSC_data E");
	if(1 == read_cmos_eeprom_8(addr_qsc_flag))
	{
		iReadRegI2C(pusendcmd, 2, &s5khpx_QSC_data[0], read_qsc_len_1, EEPROM_SLAVE_ID);
		...
		{
			iReadRegI2C(pusendcmd, 2, &s5khpx_QSC_data[read_qsc_len_1 + read_qsc_len_2], 64, EEPROM_SLAVE_ID);
			read_qsc_len_2 += 64;
			pusendcmd[0] = (char)((addr_qsc + read_qsc_len_1 + read_qsc_len_2) >> 8);
			pusendcmd[1] = (char)((addr_qsc + read_qsc_len_1 + read_qsc_len_2) & 0xFF);
		}

		iReadRegI2C(pusendcmd, 2, &s5khpx_QSC_data[QSC_DATA_LEN-((QSC_DATA_LEN - read_qsc_len_1) % 64)], (QSC_DATA_LEN - read_qsc_len_1) % 64, EEPROM_SLAVE_ID);

	}else {
		S5KHPXSUNNY_LOG_INF("read_QSC_data error invalid flag");
	}

	for (idx = 0; idx < QSC_DATA_LEN; idx++)
	{
		S5KHPXSUNNY_LOG_DBG("read_QSC_data[%d] = 0x%x",idx,s5khpx_QSC_data[idx]);
	}
	S5KHPXSUNNY_LOG_INF("read_QSC_data X");
...
// 	iReadRegI2CTiming(pu_send_cmd, 2, get_byte, 2, imgsensor.i2c_write_id, 400);
// 	return (get_byte[0] << 8) | (get_byte[1]);
// }

// static void read_OTP_page(kal_uint32 page, unsigned char* buf)
// {
// 	kal_uint16 idx = 0;
// 	kal_uint16 temp = 0;
...
// 	{
// 		temp = read_cmos_sensor16(0x0A04 + idx);
// 		buf[idx] = temp >> 8;
// 		S5KHPXSUNNY_LOG_DBG("read page %d data[0x%x] = 0x%x\n", page, 0x0A04 + idx, buf[idx]);
// 		buf[++idx] = temp & 0xFF;
// 		S5KHPXSUNNY_LOG_DBG("read page %d data[0x%x] = 0x%x\n", page, 0x0A04 + idx, buf[idx]);
// 	}
// 	mdelay(5);
...
static kal_uint32 open(void)
{
	...
	S5KHPXSUNNY_LOG_INF("PLATFORM:MT6877, MIPI 3LANE\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				S5KHPXSUNNY_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			S5KHPXSUNNY_LOG_INF("Read sensor id fail,i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
...
{
	S5KHPXSUNNY_LOG_INF("loading preview_setting\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading capture_setting\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading normal_video_setting\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading hs_video_setting\n");
...
static kal_uint32 custom1(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading custom1_setting\n");
...
static kal_uint32 custom2(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading custom2_setting\n");
...
static kal_uint32 custom3(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading custom3_setting\n");
...
static kal_uint32 custom4(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading custom4_setting\n");
...
static kal_uint32 custom5(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading custom5_setting\n");
...
static kal_uint32 custom6(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("loading custom6_setting\n");
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHPXSUNNY_LOG_INF("scenario_id = %d\n", scenario_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_CUSTOM6:
		...
		break;
	default:
		S5KHPXSUNNY_LOG_INF("Error ScenarioId setting");
		preview(image_window, sensor_config_data);
...
static kal_uint32 seamless_switch(enum MSDK_SCENARIO_ID_ENUM scenario_id, kal_uint32 shutter, kal_uint32 gain,kal_uint32 shutter_2ndframe, kal_uint32 gain_2ndframe)
{
	...
	S5KHPXSUNNY_LOG_INF("seamless switch to %d!\n",scenario_id);
	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	{
		mdelay(30);
		S5KHPXSUNNY_LOG_INF("seamless switch to preview! gain(%d), shutter(%d)\n", gain, shutter);
		spin_lock(&imgsensor_drv_lock);
		...
		//imgsensor.extend_frame_length_en = KAL_FALSE;
		spin_unlock(&imgsensor_drv_lock);
		S5KHPXSUNNY_LOG_INF("seamless switch preview start 1-exp!\n");
		
		write_cmos_sensor_8(0x0104, 0x01);
		preview_setting();
		write_cmos_sensor_8(0x0104, 0x00);
		
		S5KHPXSUNNY_LOG_INF("seamless switch preview end 1-exp!\n");
	...
	case MSDK_SCENARIO_ID_CUSTOM5:
	{
		S5KHPXSUNNY_LOG_INF("seamless switch to 2x mode! gain(%d), shutter(%d)\n", gain, shutter);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.sensor_mode = scenario_id;
		imgsensor.autoflicker_en = KAL_FALSE;
		imgsensor.pclk = imgsensor_info.custom5.pclk;
		...
		//imgsensor.extend_frame_length_en = KAL_FALSE;
		spin_unlock(&imgsensor_drv_lock);
		S5KHPXSUNNY_LOG_INF("seamless switch CUSTOM5 start 2-exp!\n");
		
		write_cmos_sensor_8(0x0104, 0x01);
		custom5_setting();
		write_cmos_sensor_8(0x0104, 0x00);
		
		S5KHPXSUNNY_LOG_INF("seamless switch CUSTOM5 end 2-exp!\n");
		if (gain != 0) {
			set_gain(gain_2ndframe);
		}
		if (shutter != 0) {
	...
	S5KHPXSUNNY_LOG_INF("%s success, scenario is switched to %d", __func__, scenario_id);
	return 0;
}
...
{
	S5KHPXSUNNY_LOG_DBG("framerate = %d\n ", framerate);
...
{
	S5KHPXSUNNY_LOG_DBG("enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	S5KHPXSUNNY_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		if (imgsensor.frame_length > imgsensor.shutter) {
			...
			/*No need to set*/
			S5KHPXSUNNY_LOG_DBG("frame_length %d < shutter %d", imgsensor.frame_length, imgsensor.shutter);
		}
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		if (framerate == 0)
	...
	default:  //coding with  preview scenario by default
		...
		if (imgsensor.frame_length > imgsensor.shutter) {
			...
			/*No need to set*/
			S5KHPXSUNNY_LOG_INF("frame_length %d < shutter %d", imgsensor.frame_length, imgsensor.shutter);
		}
		S5KHPXSUNNY_LOG_INF("error scenario_id = %d, we use preview scenario \n", scenario_id);
		break;
	}
	return ERROR_NONE;
}
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	S5KHPXSUNNY_LOG_DBG("feature_id = %d\n", feature_id);
	switch (feature_id) {
	...
	case SENSOR_FEATURE_SET_TEST_PATTERN:
		/*N6 code for HQ-397244 by chenxiaoyong at 20240727 start*/
		streaming_control(imgsensor,KAL_FALSE);
		set_test_pattern_mode((BOOL)*feature_data);
		streaming_control(imgsensor,KAL_TRUE);
		/*N6 code for HQ-397244 by chenxiaoyong at 20240727 end*/
		break;
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		S5KHPXSUNNY_LOG_INF("current fps :%d\n", (UINT32)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.current_fps = *feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_SET_HDR:
		S5KHPXSUNNY_LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.ihdr_en = (BOOL)*feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_GET_CROP_INFO:
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		S5KHPXSUNNY_LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n", (UINT16)*feature_data, (UINT16)*(feature_data + 1), (UINT16)*(feature_data + 2));
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(imgsensor,KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
		streaming_control(imgsensor,KAL_TRUE);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
	...
	case SENSOR_FEATURE_GET_VC_INFO2:
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_GET_VC_INFO %d\n",
			(UINT16) *feature_data);
		pvcinfo2 = (struct SENSOR_VC_INFO2_STRUCT *) (uintptr_t) (*(feature_data + 1));
		/*N6 code for HQ-323257 by HQ camera at 20230831 start*/
		switch (*feature_data_32) {
		case MSDK_SCENARIO_ID_CUSTOM1:
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		...
		/*N6 code for HQ-323257 by HQ camera at 20230831 end*/
		break;
#if ENABLE_PDAF

	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5KHPXSUNNY_LOG_INF("odin GET_PDAF_DATA EEPROM\n");
		break;

	case SENSOR_FEATURE_GET_PDAF_INFO:
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%d\n",
			(UINT16) *feature_data);
...
/*N6 code for HQ-346208 by HQ camera at 20231108 end*/
	...
	case SENSOR_FEATURE_SET_PDAF:
		...
		S5KHPXSUNNY_LOG_INF("[odin] pdaf mode : %d \n", imgsensor.pdaf_mode);
		break;
...
/*N6 code for HQ-330628 by HQ camera at 20231027 end*/
	...
	case SENSOR_FEATURE_SEAMLESS_SWITCH:
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_SEAMLESS_SWITCH");
		...
		if ((feature_data + 1) != NULL) {
			...
			S5KHPXSUNNY_LOG_INF("ae_ctrl(%d) input", *pAeCtrls);
		}
		else
			S5KHPXSUNNY_LOG_INF("warning! no ae_ctrl input");
		if (feature_data == NULL) {
			S5KHPXSUNNY_LOG_INF("error! input scenario is null!");
			return ERROR_INVALID_SCENARIO_ID;
		}
		S5KHPXSUNNY_LOG_INF("call seamless_switch");
		if (pAeCtrls != NULL) {
		S5KHPXSUNNY_LOG_INF("aectrlcall seamless_switch_arctrl");
		seamless_switch((*feature_data),*pAeCtrls, *(pAeCtrls + 1),
				*(pAeCtrls + 4), *(pAeCtrls + 5));
		} else {
		S5KHPXSUNNY_LOG_INF("call seamless_switch_null");
	...
	case SENSOR_FEATURE_GET_SEAMLESS_SCENARIOS:
		if ((feature_data + 1) != NULL) {
			...
			S5KHPXSUNNY_LOG_INF("feature_data(%d), *pScenarios(%d)", *feature_data, *pScenarios);
		}
		else
		{
			S5KHPXSUNNY_LOG_INF("input pScenarios vector is NULL!\n");
			return ERROR_INVALID_SCENARIO_ID;
		}
		switch (*feature_data)
		{
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			*pScenarios = MSDK_SCENARIO_ID_CUSTOM5;//9
			S5KHPXSUNNY_LOG_INF("MSDK_SCENARIO_ID_CAMERA_PREVIEW");
		break;
		case MSDK_SCENARIO_ID_CUSTOM5:
		...
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_GET_SEAMLESS_SCENARIOS %d %d\n",
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/*get 4 cell data from eeprom*/
	{
		...
		S5KHPXSUNNY_LOG_INF("SENSOR_FEATURE_GET_4CELL_DATA");

		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			data[0] = 0x00;
			data[1] = 0x24;
			memcpy(data + 2, s5khpx_QSC_data, QSC_DATA_LEN);
			S5KHPXSUNNY_LOG_INF("Read FOUR_CELL_CAL_TYPE_XTALK_CAL = %02x %02x %02x %02x %02x %02x\n",
			...
			data[0] = 0x80;
			data[1] = 0x95;
			// memcpy(data + 2, s5khpx_OTP_data, OTP_DATA_LEN);
			S5KHPXSUNNY_LOG_INF("Read FOUR_CELL_CAL_TYPE_DPC = %02x %02x %02x %02x %02x %02x\n",
				(UINT16)data[0], (UINT16)data[1],
				(UINT16)data[2], (UINT16)data[3],
				(UINT16)data[4], (UINT16)data[5]);
...
UINT32 S5KHPX_SUNNY_MAIN_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    S5KHPXSUNNY_LOG_INF("S5KHPX_SUNNY_MAIN_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	S5KHPX_SUNNY_MAIN_MIPI_RAW_SensorInit	*/