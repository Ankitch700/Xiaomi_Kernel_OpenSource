...
#include "s5khp3_ofilm_main_mipi_raw.h"
...
#define LOG_TAG "s5khp3ofilm"
#define S5KHP3OFILM_LOG_INF(format, args...) pr_err(LOG_TAG "[%s] " format, __func__, ##args)
#define S5KHP3OFILM_LOG_DBG(format, args...) pr_debug(LOG_TAG "[%s] " format, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KHP3_OFILM_MAIN_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
/*N6 code for HQ-309652 by HQ camera at 20230729 end*/
	...
	.custom7 = {/*4080*3072 24fps sensor zoom2x*/
		.pclk = 2000000000,
		.linelength  = 14848,
		.framelength = 5600,
		.startx = 0,
		...
		.max_framerate = 240,
		.mipi_pixel_rate = 1357056000,
	},/*4080*3072 24fps sensor zoom2x*/
		.custom8 = {//dual camera 1080p 30fps
		.pclk = 2000000000,
		.linelength  = 8192,
		.framelength = 8128,
		.startx = 0,
	...
	.custom7_delay_frame    = 2, /* enter custom7delay frame num */
	.custom8_delay_frame    = 2, /* enter custom8delay frame num */	
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[13] = {
	...
	{ 16320,12288, 4080,  3072,  8160,6144,  4080, 3072,    0,    0,  4080, 3072,    0,    0,  4080, 3072}, // custom7 stereo as Preview 24fps zoom2x
	{ 16320,12288, 2400,  2904,  11520, 6480,  1920, 1080,    0,    0,  1920, 1080,    0,    0,  1920, 1080}, // custom8 stereo as dual video 1080p 30fps  
};
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4Crop = {
		...
		{0, 0}, {0, 0}, {6120, 4608}, {0, 0}, {2040, 1536}, {0, 0}
	},//{0, 1632}
	.iMirrorFlip = 0,
};
static struct SENSOR_VC_INFO2_STRUCT SENSOR_VC_INFO2[6] = {
	...
	},
	{
		...
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0xff0, 0xC00},//4080*3072
			{VC_PDAF_STATS_NE_PIX_1, 0x01, 0x30, 0x4ec, 0xc0},//1272*192
		},
		1
	},
	{
		...
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0xff0, 0xC00},//4080*3072
			{VC_PDAF_STATS_NE_PIX_1, 0x01, 0x30, 0x9ec, 0x180},//2552*384
		},
		1
	},
	{
		...
		{
			{VC_STAGGER_NE, 0x00, 0x2b, 0xff0, 0xC00},//4080*3072
			{VC_PDAF_STATS_NE_PIX_1, 0x01, 0x30, 0x9ec, 0x180},//2032*384
		},
		1
	},
};
...
/*N6 code for HQ-306077 by HQ camera at 20230713 start*/
	...
	{
		S5KHP3OFILM_LOG_INF("not match vendor id:0x%x\n",vendor_id);
		sensor_id = 0xFFFF;
	}

	S5KHP3OFILM_LOG_INF("[%s] vendor_id: 0x%x, sensor_id: 0x%x,sensor_version:0x%x\n", __func__, vendor_id, sensor_id,sensor_version);
/*N6 code for HQ-306077 by HQ camera at 20230713 end*/
	return sensor_id;
}
...
{
	S5KHP3OFILM_LOG_DBG("dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	S5KHP3OFILM_LOG_DBG("framerate = %d, min framelength =%d, should enable? \n", framerate, min_framelength_en);
...
static void write_shutter(kal_uint32 shutter)
{
	...
	S5KHP3OFILM_LOG_DBG("Exit! shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length, kal_bool auto_extend_en)
{
	...
	S5KHP3OFILM_LOG_DBG("set shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}
...
static void set_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
    ...
    S5KHP3OFILM_LOG_INF("write awb gain r:g:b %d:%d:%d \n", r_Gain, g_Gain, b_Gain);
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	S5KHP3OFILM_LOG_INF("set_gain %d \n", gain);
	if (gain < imgsensor_info.min_gain || gain > imgsensor_info.max_gain) {
		S5KHP3OFILM_LOG_INF("Error gain setting");
	...
	spin_unlock(&imgsensor_drv_lock);
	S5KHP3OFILM_LOG_DBG("gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
	...
	if (imgsensor.current_scenario_id == MSDK_SCENARIO_ID_CUSTOM4 ||
        ...
        imgsensor.current_scenario_id == MSDK_SCENARIO_ID_CUSTOM7) {
...
{
	S5KHP3OFILM_LOG_DBG("le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
...
static kal_uint32 streaming_control(struct imgsensor_struct imgsensor,kal_bool enable)
{
	...
	S5KHP3OFILM_LOG_INF("streaming_enable(0=sw standby,1=streaming):%d\n",enable);
	if (enable) {
		write_cmos_sensor(0xFCFC, 0x4000);
		...
		S5KHP3OFILM_LOG_INF("stream off fail! framecnt = %d\n",framecnt);
	}
	 return ERROR_NONE;
}
/*N6 code for HQ-309652 by HQ camera at 20230729 end*/
...
static void sensor_init(void)
{
	...
	// Global
	S5KHP3OFILM_LOG_INF("loading global setting+\n");
	table_write_cmos_sensor(s5khp3ofilm_init_setting_global,
			sizeof(s5khp3ofilm_init_setting_global) / sizeof(kal_uint16));
	S5KHP3OFILM_LOG_INF("loading global setting-\n");
}	/*	sensor_init  */
...
{
	S5KHP3OFILM_LOG_INF("+\n");
	table_write_cmos_sensor(s5khp3ofilm_preview_setting,
			sizeof(s5khp3ofilm_preview_setting) / sizeof(kal_uint16));
...
{
	S5KHP3OFILM_LOG_INF("capture setting E\n");
	table_write_cmos_sensor(s5khp3ofilm_capture_setting,
			sizeof(s5khp3ofilm_capture_setting) / sizeof(kal_uint16));
}	/*	capture_setting  */


static void normal_video_setting(void)
{
	S5KHP3OFILM_LOG_INF("E!\n");
	table_write_cmos_sensor(s5khp3ofilm_normal_video_setting,
			sizeof(s5khp3ofilm_normal_video_setting) / sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(s5khp3ofilm_hs_video_setting,
		sizeof(s5khp3ofilm_hs_video_setting) / sizeof(kal_uint16));
}

static void slim_video_setting(void)
{
	S5KHP3OFILM_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3ofilm_slim_video_setting,
		sizeof(s5khp3ofilm_slim_video_setting) / sizeof(kal_uint16));
}

static void custom1_setting(void)
{
	S5KHP3OFILM_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3ofilm_custom1_setting,
		sizeof(s5khp3ofilm_custom1_setting) / sizeof(kal_uint16));
}

static void custom2_setting(void)
{
	S5KHP3OFILM_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3ofilm_custom2_setting,
			sizeof(s5khp3ofilm_custom2_setting) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	table_write_cmos_sensor(s5khp3ofilm_custom3_setting,
			sizeof(s5khp3ofilm_custom3_setting) / sizeof(kal_uint16));
}

static void custom4_setting(void)
{
	S5KHP3OFILM_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3ofilm_custom4_setting,
			sizeof(s5khp3ofilm_custom4_setting) / sizeof(kal_uint16));
}

static void custom5_setting(void)
{
	S5KHP3OFILM_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3ofilm_custom5_setting,
			sizeof(s5khp3ofilm_custom5_setting) / sizeof(kal_uint16));
}
...
{
	S5KHP3OFILM_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3ofilm_custom7_setting,
		sizeof(s5khp3ofilm_custom7_setting) / sizeof(kal_uint16));
}

static void custom8_setting(void)
{
	S5KHP3OFILM_LOG_INF("E\n");
	table_write_cmos_sensor(s5khp3ofilm_custom8_setting,
		sizeof(s5khp3ofilm_custom8_setting) / sizeof(kal_uint16));
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 3;
	S5KHP3OFILM_LOG_INF("S5KHP3OFILM get_imgsensor_id in\n");
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				S5KHP3OFILM_LOG_INF("probe success, i2c write id: 0x%x, sensor id: 0x%x\n",
				...
				S5KHP3OFILM_LOG_INF("Read sensor id fail,i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			}
			retry--;
		} while (retry > 0);
		i++;
...
static void read_QSC_data(void)
{
	...
	S5KHP3OFILM_LOG_INF("read_QSC_data E");
	if(1 == read_cmos_eeprom_8(addr_qsc_flag))
	{
		...
		S5KHP3OFILM_LOG_INF("read_QSC_data error invalid flag");
	}

	for (idx = 0; idx < QSC_DATA_LEN; idx++)
	{
		S5KHP3OFILM_LOG_DBG("read_QSC_data[%d] = 0x%x",idx,s5khp3_QSC_data[idx]);
	}
	S5KHP3OFILM_LOG_INF("read_QSC_data X");
...
static kal_uint32 open(void)
{
	...
	S5KHP3OFILM_LOG_INF("PLATFORM:MT6877, MIPI 3LANE\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				S5KHP3OFILM_LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			S5KHP3OFILM_LOG_INF("Read sensor id fail,i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
...
{
	S5KHP3OFILM_LOG_INF("loading preview_setting\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading capture_setting\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading normal_video_setting\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading hs_video_setting\n");
...
static kal_uint32 custom1(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading custom1_setting\n");
...
static kal_uint32 custom2(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading custom2_setting\n");
...
static kal_uint32 custom3(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading custom3_setting\n");
...
static kal_uint32 custom4(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading custom4_setting\n");
...
static kal_uint32 custom5(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading custom5_setting\n");
...
static kal_uint32 custom6(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading custom6_setting\n");
...
static kal_uint32 custom7(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading custom7_setting\n");
...
static kal_uint32 custom8(
	MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("loading custom8_setting\n");
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHP3OFILM_LOG_INF("scenario_id = %d\n", scenario_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_CUSTOM8:
		...
		break;
	default:
		S5KHP3OFILM_LOG_INF("Error ScenarioId setting");
		preview(image_window, sensor_config_data);
...
static kal_uint32 seamless_switch(enum MSDK_SCENARIO_ID_ENUM scenario_id, kal_uint32 shutter, kal_uint32 gain,kal_uint32 shutter_2ndframe, kal_uint32 gain_2ndframe)
{
	...
	S5KHP3OFILM_LOG_INF("seamless switch to %d!\n",scenario_id);
	switch (scenario_id) {
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	{
		mdelay(30);
		S5KHP3OFILM_LOG_INF("seamless switch to preview! gain(%d), shutter(%d)\n", gain, shutter);
		spin_lock(&imgsensor_drv_lock);
		...
		//imgsensor.extend_frame_length_en = KAL_FALSE;
		spin_unlock(&imgsensor_drv_lock);
		S5KHP3OFILM_LOG_INF("seamless switch 1-exp!\n");
		preview_setting();
	...
	case MSDK_SCENARIO_ID_CUSTOM3:
	{
		S5KHP3OFILM_LOG_INF("MSDK_SCENARIO_ID_CUSTOM3 seamless switch to 4x mode! gain(%d), shutter(%d)\n", gain, shutter);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.sensor_mode = scenario_id;
		imgsensor.autoflicker_en = KAL_FALSE;
		imgsensor.pclk = imgsensor_info.custom3.pclk;
		...
		//imgsensor.extend_frame_length_en = KAL_FALSE;
		spin_unlock(&imgsensor_drv_lock);
		S5KHP3OFILM_LOG_INF("seamless switch 4-exp!\n");
		custom3_setting();
		if (gain_2ndframe != 0) {
			set_gain(gain_2ndframe);
		}
	...
	case MSDK_SCENARIO_ID_CUSTOM5:
	{
		S5KHP3OFILM_LOG_INF("MSDK_SCENARIO_ID_CUSTOM5 seamless switch to 2x mode! gain(%d), shutter(%d)\n", gain, shutter);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.sensor_mode = scenario_id;
		imgsensor.autoflicker_en = KAL_FALSE;
		imgsensor.pclk = imgsensor_info.custom5.pclk;
		...
		//imgsensor.extend_frame_length_en = KAL_FALSE;
		spin_unlock(&imgsensor_drv_lock);
		S5KHP3OFILM_LOG_INF("seamless switch 2-exp!\n");
		custom5_setting();
		if (gain_2ndframe != 0) {
			set_gain(gain_2ndframe);
		}
		...
		break;
	default:
		S5KHP3OFILM_LOG_INF("error! wrong setting in set_seamless_switch = %d", scenario_id);
		return 0xff;
	}
	imgsensor.fast_mode_on = KAL_TRUE;
	S5KHP3OFILM_LOG_INF("%s success, scenario is switched to %d", __func__, scenario_id);
	return 0;
}
...
{
	S5KHP3OFILM_LOG_DBG("framerate = %d\n ", framerate);
...
{
	S5KHP3OFILM_LOG_DBG("enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	S5KHP3OFILM_LOG_DBG("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		if (imgsensor.frame_length > imgsensor.shutter) {
			...
			/*No need to set*/
			S5KHP3OFILM_LOG_DBG("frame_length %d < shutter %d", imgsensor.frame_length, imgsensor.shutter);
		}
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		if (framerate == 0)
	...
	default:  //coding with  preview scenario by default
		...
		if (imgsensor.frame_length > imgsensor.shutter) {
			...
			/*No need to set*/
			S5KHP3OFILM_LOG_INF("frame_length %d < shutter %d", imgsensor.frame_length, imgsensor.shutter);
		}
		S5KHP3OFILM_LOG_INF("error scenario_id = %d, we use preview scenario \n", scenario_id);
		break;
	}
	return ERROR_NONE;
}
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	S5KHP3OFILM_LOG_DBG("feature_id = %d\n", feature_id);
	switch (feature_id) {
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		S5KHP3OFILM_LOG_INF("current fps :%d\n", (UINT32)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.current_fps = *feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_SET_HDR:
		S5KHP3OFILM_LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.ihdr_en = (BOOL)*feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_GET_CROP_INFO:
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		S5KHP3OFILM_LOG_INF("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n", (UINT16)*feature_data, (UINT16)*(feature_data + 1), (UINT16)*(feature_data + 2));
	...
	case SENSOR_FEATURE_SET_STREAMING_SUSPEND:
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_SET_STREAMING_SUSPEND\n");
		streaming_control(imgsensor,KAL_FALSE);
		break;
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_SET_STREAMING_RESUME, shutter:%llu\n",
			*feature_data);
		if (*feature_data != 0)
			set_shutter(*feature_data);
	...
	case SENSOR_FEATURE_GET_BINNING_TYPE:
		...
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_GET_BINNING_TYPE AE_binning_type:%d,\n",
	...
	case SENSOR_FEATURE_GET_VC_INFO2:
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_GET_VC_INFO %d\n",
			(UINT16) *feature_data);
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5KHP3OFILM_LOG_INF("odin GET_PDAF_DATA EEPROM\n");
		break;

	case SENSOR_FEATURE_GET_PDAF_INFO:
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%d\n",
			(UINT16) *feature_data);
...
/*N6 code for HQHW-4840 by huabinchen at 20230810 end*/
	...
	case SENSOR_FEATURE_SET_PDAF:
		...
		S5KHP3OFILM_LOG_INF("[odin] pdaf mode : %d \n", imgsensor.pdaf_mode);
		break;
	case SENSOR_FEATURE_GET_AWB_REQ_BY_SCENARIO:
		switch (*feature_data) {
/*N6 code for HQ-330628 by HQ camera at 20231025 start*/
		case MSDK_SCENARIO_ID_CUSTOM3:
		case MSDK_SCENARIO_ID_CUSTOM4:
		case MSDK_SCENARIO_ID_CUSTOM5:
		case MSDK_SCENARIO_ID_CUSTOM7:
/*N6 code for HQ-330628 by HQ camera at 20231025 end*/
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 1;
			break;
		default:
			*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 0;
	...
	case SENSOR_FEATURE_SEAMLESS_SWITCH:
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_SEAMLESS_SWITCH");
		switch (*feature_data)
		{
		case MSDK_SCENARIO_ID_CUSTOM3:
		...
		case MSDK_SCENARIO_ID_CUSTOM5:
			...
			break;
		default:
			S5KHP3OFILM_LOG_INF("fpsOV feature_data(%d)", *feature_data);
			break;
		}
		if ((feature_data + 1) != NULL)
			pAeCtrls = (MUINT32 *)((uintptr_t)(*(feature_data + 1)));
		else
			S5KHP3OFILM_LOG_INF("warning! no ae_ctrl input");
		if (feature_data == NULL) {
			S5KHP3OFILM_LOG_INF("error! input scenario is null!");
			return ERROR_INVALID_SCENARIO_ID;
		}
		S5KHP3OFILM_LOG_INF("call seamless_switch");
		if (pAeCtrls != NULL) {
			S5KHP3OFILM_LOG_INF("aectrlcall seamless_switch_arctrl");
			seamless_switch((*feature_data),*pAeCtrls, *(pAeCtrls + 1), *(pAeCtrls + 4), *(pAeCtrls + 5));
		} else {
			S5KHP3OFILM_LOG_INF("call seamless_switch_null");
	...
	case SENSOR_FEATURE_GET_SEAMLESS_SCENARIOS:
		if ((feature_data + 1) != NULL) {
			...
			S5KHP3OFILM_LOG_INF("feature_data(%d), *pScenarios(%d)", *feature_data, *pScenarios);
		}
		else
		{
			S5KHP3OFILM_LOG_INF("input pScenarios vector is NULL!\n");
			return ERROR_INVALID_SCENARIO_ID;
		}
		switch (*feature_data)
		{
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			...
			*(pScenarios + 1) = MSDK_SCENARIO_ID_CUSTOM3;//7
			S5KHP3OFILM_LOG_INF("MSDK_SCENARIO_ID_CAMERA_PREVIEW to MSDK_SCENARIO_ID_CUSTOM5 + MSDK_SCENARIO_ID_CUSTOM3");
			break;
		...
		case MSDK_SCENARIO_ID_CUSTOM6://full size
			*pScenarios = MSDK_SCENARIO_ID_CAMERA_PREVIEW;//0
			S5KHP3OFILM_LOG_INF("MSDK_SCENARIO_ID_CUSTOM3 or MSDK_SCENARIO_ID_CUSTOM5 to MSDK_SCENARIO_ID_CAMERA_PREVIEW\n");
			break;
		default:
			*pScenarios = 0xff;
			break;
		}
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_GET_SEAMLESS_SCENARIOS %d %d\n", *feature_data, *pScenarios);
		break;
	//N6 code for HQ-305655, by p-zhaobeidou3, 20230906, end
	case SENSOR_FEATURE_GET_4CELL_DATA:/*get 4 cell data from eeprom*/
	{
		...
		S5KHP3OFILM_LOG_INF("SENSOR_FEATURE_GET_4CELL_DATA");

		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			data[0] = 0x00;
			data[1] = 0x24;
			memcpy(data + 2, s5khp3_QSC_data, QSC_DATA_LEN);
			S5KHP3OFILM_LOG_INF("Read FOUR_CELL_CAL_TYPE_XTALK_CAL = %02x %02x %02x %02x %02x %02x\n",
			...
			data[0] = 0x80;
			data[1] = 0x95;
			S5KHP3OFILM_LOG_INF("Read FOUR_CELL_CAL_TYPE_DPC = %02x %02x %02x %02x %02x %02x\n",
				(UINT16)data[0], (UINT16)data[1],
				(UINT16)data[2], (UINT16)data[3],
				(UINT16)data[4], (UINT16)data[5]);
...
{
    S5KHP3OFILM_LOG_INF("S5KHP3_OFILM_MAIN_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	S5KHP3_OFILM_MAIN_MIPI_RAW_SensorInit	*/
