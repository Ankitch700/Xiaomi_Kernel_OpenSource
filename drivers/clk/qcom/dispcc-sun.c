...
static struct qcom_cc_desc disp_cc_sun_desc_no_cesta = {
	.config = &disp_cc_sun_regmap_config,
	.clks = disp_cc_sun_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_sun_clocks),
	.resets = disp_cc_sun_resets,
...
static bool disp_cc_disable_cesta = false;
...
static int disp_cc_sun_probe(struct platform_device *pdev)
{
	...
	disp_cc_disable_cesta = of_property_read_bool(of_node, "dispcc,disable_sde_cesta");

	if (disp_cc_disable_cesta) {
		regmap = qcom_cc_map(pdev, &disp_cc_sun_desc_no_cesta);
		if (IS_ERR(regmap))
			return PTR_ERR(regmap);

		ret = qcom_cc_runtime_init(pdev, &disp_cc_sun_desc_no_cesta);
		if (ret)
			return ret;
	} else {
	...
	if (disp_cc_disable_cesta)
		ret = qcom_cc_really_probe(pdev, &disp_cc_sun_desc_no_cesta, regmap);
	else
...
{
	if (disp_cc_disable_cesta)
		qcom_cc_sync_state(dev, &disp_cc_sun_desc_no_cesta);
	else
