...
void fsm_speaker_shutdown(void)
{
	struct list_head *pos = NULL;
	struct fsm_dev *fsm_dev = NULL;

	pr_info("fsm_speaker_shutdown enter");

	list_for_each (pos, &fsm_dev_list) {
		fsm_dev = list_entry(pos, struct fsm_dev, list);
		if (fsm_dev == NULL) {
			pr_err("struct fsm_dev not ready");
			return;
		}
		...
		pr_info("fsm_speaker_shutdown fsm_dev[id %d]", fsm_dev->id);
	}
}
EXPORT_SYMBOL_GPL(fsm_speaker_shutdown);
