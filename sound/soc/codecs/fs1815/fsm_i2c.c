			...
			const struct i2c_device_id *id)
{
	...
	fsm_dev->audio_nb.notifier_call = shipmode_notifier_callback;
	shipmode_shutdown_notifier_register_client(&fsm_dev->audio_nb);
...
static int fsm_i2c_remove(struct i2c_client *i2c)
{
	...
	if (fsm_dev->audio_nb.notifier_call != NULL) {
		shipmode_shutdown_notifier_unregister_client(&fsm_dev->audio_nb);
	}
