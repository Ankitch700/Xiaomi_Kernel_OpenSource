...
extern int shipmode_shutdown_notifier_register_client(struct notifier_block *nb);
extern int shipmode_shutdown_notifier_unregister_client(struct notifier_block *nb);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (acf_info == NULL) {
		AW_LOGE("get struct aw87xxx profile failed");
		return -EINVAL;
	}
...
static void aw87xxx_i2c_shutdown_extern(void)
{
	struct list_head *pos = NULL;
	struct aw87xxx *aw87xxx = NULL;
	...
	list_for_each (pos, &g_aw87xxx_list) {
		...
		AW_LOGI("aw87xxx[dev_index %d] shutdown", aw87xxx->dev_index);
	}
}

static int shipmode_notifier_callback(struct notifier_block *self,
 						unsigned long event, void *data)
{
	aw87xxx_i2c_shutdown_extern();
				...
				const struct i2c_device_id *id)
{
	...
	aw87xxx->audio_nb.notifier_call = shipmode_notifier_callback;
	shipmode_shutdown_notifier_register_client(&aw87xxx->audio_nb);
...
static int aw87xxx_i2c_remove(struct i2c_client *client)
{
	...
	if (aw87xxx->audio_nb.notifier_call != NULL) {
		shipmode_shutdown_notifier_unregister_client(&aw87xxx->audio_nb);
	}
