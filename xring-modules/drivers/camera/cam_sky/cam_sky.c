...
static uid_t s_last_caller_uid;
static pid_t s_last_caller_pid;

static uid_t s_last_reader_uid;
static pid_t s_last_reader_pid;
...
static long cameralog_ioctl(struct file *filp, unsigned int cmd,  unsigned long arg)
{
	...
	case IOCTL_CMD_SET_DEBUG_LEVEL: {
		int data;
		/* Copy debug level from user space */
	...
	case IOCTL_CMD_READ_KFIFO_CNT: {
		...
		/* Copy FIFO length to user space */
	...
	case IOCTL_CMD_READ_LAST_MSG: {
		/* Copy last message to user space */
...
static ssize_t cameralog_read(struct file *fp, char __user *buff,  size_t length, loff_t *ppos)
{
	...
	if (debugLevel > 0)
		pr_debug("%s: %s!\n", __func__, "cameralog_read .");

	mutex_lock(&camlog_message_lock);
	memset(&s_read_buf, 0, sizeof(struct camlog_dev));
	if (debugLevel > 0)
		pr_debug("%s: %s, kfifo len:%lu", __func__, "cameralog_read", kfifo_len(&message_kfifo) / sizeof(struct camlog_dev));


	ret = kfifo_out(&message_kfifo, &s_read_buf, sizeof(struct camlog_dev));
	if (ret != sizeof(struct camlog_dev)) {
		if (debugLevel > 0)
			pr_debug("%s: not read anything from kfifo %d, expect: %lu\n", __func__, ret, sizeof(struct camlog_dev));
	...
	if (debugLevel > 0)
		pr_debug("%s message to user:%s   len:%d", __func__, s_read_buf.m_camlog_message, message_len);
	...
	if (debugLevel > 0)
		pr_debug("return len:%zd", bytes_read);
...
{
	pr_debug("%s: %s", __func__, "camlog_send_message ss .");
}
...
static ssize_t cameralog_write(struct file *fp, const char *buff,   size_t length, loff_t *ppos)
{
	...
	if (debugLevel > 0)
		pr_debug("%s: %s", __func__, "cameralog_write .");
	...
	s_write_buf.m_camlog_message[MESSAGE_MAX-1] = '\0';
	if (ret == 0)
		bytes_wirte = strlen(s_write_buf.m_camlog_message);

	if (debugLevel > 0)
		pr_debug("%s valid_len:%d  strlen:%zd.", __func__, valid_len, bytes_wirte);
	...
	if (debugLevel > 0)
		pr_debug("%s input  msg : %s", __func__, s_write_buf.m_camlog_message);
...
{
	pr_debug("%s: %s", __func__, "cameralog_release .");
	wake_up_interruptible(&camlog_is_not_empty);
	return 0;
}
...
static int __init cameralog_init(void)
{
	...
	if (ret) {
		pr_err("cameralog message kfifo: Failed to allocate kfifo\n");
		return ret;
	}
